/* E-REPORT/SAGS V4.8.10B-HF1 · ARR→DEP MY FLIGHT READY FIX · SHA-256 DELTA CACHE */
const CACHE_NAME="sags-app-shell-v4.8.10b-hf1";
const META_CACHE_NAME="sags-app-meta-v4.8.10b-hf1";
const ASSET_MANIFEST_URL="./asset-manifest.json";
const BUILD="V4.8.10B-HF1-ARR-DEP-MYFLIGHT-READY";
const DISPLAY_VERSION="V4.8.10B-HF1";
...
async function migrateLegacyCaches(){/* V4.8.4: executable caches are build-scoped; never copy old build assets forward. */}
...
async function cleanupLegacyCaches(){const keys=await caches.keys(),legacy=keys.filter(k=>k!==CACHE_NAME&&k!==META_CACHE_NAME&&(k.startsWith("sags-app-shell-")||k.startsWith("sags-app-meta-")||k.startsWith("sags-v")||k.startsWith("sags-cache")));await Promise.all(legacy.map(k=>caches.delete(k).catch(()=>false)))}
