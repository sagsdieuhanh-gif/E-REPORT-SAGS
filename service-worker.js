/* E-REPORT/SAGS V4.2.52 · LEFT-ONLY + FAST PDF RENDER */
const CACHE_NAME="sags-v4.2.52-left-only-pdf-fast";
const BUILD="V4.2.52-LEFT-ONLY-PDF-FAST";
const DISPLAY_VERSION="V4.2.52";
...
async function safePut(cache,key,response){
  ...
  catch(e){console.info("V4.2.52 cache put skipped",key,e?.name||e?.message||e)}
}
...
async function validateRelease(){
  ...
  if(!pt.includes("V2.2.18-LEFT-ONLY-PDF-FAST"))throw new Error(PATCH_V2218+" marker mismatch");
  ...
  if(![2,3].includes(Number(cj?.schema))||!cj?.pages)throw new Error("fsags-display-coordinates.json invalid");
...
function patchRuntime17CoordinateBootstrap(response){
  ...
  return response.text().then(text=>{
    ...
    if(!out.includes("__SAGS_COORD49_RUNTIME_BOOTSTRAP")){
      out+=`
;/* V4.2.52 left-only fast coordinate runtime fallback loader */
(function(root){
  "use strict";
  ...
  function boot(){
    ...
    s.src="./v2.2.18-runtime-patch.js?v=V4.2.52";
...
function patchShiftReportCoreStrict(response){
  ...
  return response.text().then(text=>{
    ...
    if(changed<3)console.warn("V4.2.52: strict core patch applied partially",changed);
