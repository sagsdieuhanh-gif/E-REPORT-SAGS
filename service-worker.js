/* E-REPORT/SAGS V4.2.51 · LEFT-ONLY DISPLAY RENDER */
const CACHE_NAME="sags-v4.2.51-left-only-render";
const BUILD="V4.2.51-LEFT-ONLY-RENDER";
const DISPLAY_VERSION="V4.2.51";
...
async function safePut(cache,key,response){
  ...
  catch(e){console.info("V4.2.51 cache put skipped",key,e?.name||e?.message||e)}
}
...
async function validateRelease(){
  ...
  if(!pt.includes("V2.2.18-LEFT-ONLY-RENDER"))throw new Error(PATCH_V2218+" marker mismatch");
...
function patchRuntime17CoordinateBootstrap(response){
  ...
  return response.text().then(text=>{
    ...
    if(!out.includes("__SAGS_COORD49_RUNTIME_BOOTSTRAP")){
      out+=`
;/* V4.2.51 left-only coordinate runtime fallback loader */
(function(root){
  "use strict";
  ...
  function boot(){
    ...
    s.src="./v2.2.18-runtime-patch.js?v=V4.2.51";
...
function patchShiftReportCoreStrict(response){
  ...
  return response.text().then(text=>{
    ...
    if(changed<3)console.warn("V4.2.51: strict core patch applied partially",changed);
