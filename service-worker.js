/* E-REPORT/SAGS V4.2.46 · MULTISELECT + LEFT/CENTER/RIGHT ALIGN */
const CACHE_NAME="sags-v4.2.46-multiselect-align";
const BUILD="V4.2.46-MULTISELECT-ALIGN";
const DISPLAY_VERSION="V4.2.46";
...
async function safePut(cache,key,response){
  ...
  catch(e){console.info("V4.2.46 cache put skipped",key,e?.name||e?.message||e)}
}
...
async function validateRelease(){
  ...
  if(!pt.includes("V2.2.18-AD-FSAGS-BBBT-MULTISELECT-ALIGN"))throw new Error(PATCH_V2218+" marker mismatch");
...
function patchShiftReportCoreStrict(response){
  ...
  return response.text().then(text=>{
    ...
    if(changed<3)console.warn("V4.2.46: strict core patch applied partially",changed);
