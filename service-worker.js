const CACHE_NAME="sags-v2.1-myflight-realtime-adaptive-share";
const BUILD="V2.1-MYFLIGHT-REALTIME-ADAPTIVE-SHARE";
const DISPLAY_VERSION="V2.1";
const PATCH_FILE="./v2.1-runtime-patch.js";
const APP_SHELL=["./9Gfinal.png","./CBTT.png","./KTTB.png","./LNF.png","./PVHK.png","./PVHLNG.png","./VHTTB.png","./VJfinal.png","./VJfinal2.png","./VUfinal.png","./ai.js","./alert-crosscheck-complete.mp3","./alert-ket-so-moi.mp3","./alert-ket-so-thay-doi.mp3","./alert-mva.mp3","./alert-mvt.mp3","./alert-pushback-missing.mp3","./alert-read-sign.mp3","./app.js","./v2.1-runtime-patch.js","./apple-touch-icon.png","./favicon-16.png","./favicon-32.png","./fsags13-official-continuation.png","./fsags13-official-page1.png","./icon-192.png","./icon-512.png","./index.html","./ios-export.js","./ket-so.wav","./login-bg.jpg","./login-logo-10years.png","./manifest.webmanifest","./page1.png","./page10.png","./page11.png","./page12.png","./page13.png","./page2.png","./page4.png","./page6.png","./page7.png","./page9.png","./report.css","./report.js","./rns-ke-continuation.png","./rns-ke-page1.png","./rns-lj-continuation.png","./rns-lj-page1.png","./rns-tw-page1.png","./rns-tw-participants.png","./sags-logo.png","./theme.css","./ui.css","./ui.js","./daily-roster.js","./version.json","./ĐH.png"];
const FRESH_SUFFIXES=["/version.json","/manifest.webmanifest","/index.html","/app.js","/ai.js","/ui.css","/ui.js","/ios-export.js","/report.css","/report.js","/theme.css","/daily-roster.js","/v2.1-runtime-patch.js"];
...
function patchIndexHtml(html){
  ...
  if(!out.includes("v2.1-runtime-patch.js")){
    const tag=`<script src="./v2.1-runtime-patch.js?v=${encodeURIComponent(DISPLAY_VERSION)}"></script>`;
...
async function cacheCriticalFresh(dst){
  ...
  const sourceHtml=await ir.clone().text();
  const html=patchIndexHtml(sourceHtml);
  const bm=html.match(/const\s+APP_BUILD_VERSION\s*=\s*["']([^"']+)["']/);
  const dm=html.match(/const\s+APP_DISPLAY_VERSION\s*=\s*["']([^"']+)["']/);
  if(String(bm?.[1]||"").trim()!==BUILD||String(dm?.[1]||"").trim()!==DISPLAY_VERSION||!html.includes("daily-roster.js")||!html.includes("v2.1-runtime-patch.js"))throw new Error("index.html chưa vá đúng "+DISPLAY_VERSION);
  ...
  if(!appText.includes("E-REPORT/SAGS V1.1.103 RTDB ANCESTOR HOTFIX ONLY"))throw new Error("app.js chưa đúng V1.1.103");
  ...
  const pr=await fetchNoStore(PATCH_FILE+"?swcheck="+Date.now());
  if(!pr.ok)throw new Error("V2.1 patch HTTP "+pr.status);
  const patchText=await pr.clone().text();
  if(!patchText.includes(BUILD))throw new Error("V2.1 runtime patch chưa đúng BUILD "+BUILD);
  await dst.put(PATCH_FILE,pr.clone());
}

async function installDirectCache(){
  const dst=await caches.open(CACHE_NAME);
  await cacheCriticalFresh(dst);
  const critical=new Set(["./index.html","./version.json","./app.js","./ios-export.js","./daily-roster.js",PATCH_FILE]);
...
self.addEventListener("fetch",event=>{
  ...
  const isNavigation=event.request.mode==="navigate",versionFresh=url.pathname.endsWith("/version.json");
  if(versionFresh){
    event.respondWith((async()=>{
      const current=await caches.open(CACHE_NAME);
      try{
        const network=await fetch(event.request,{cache:"no-store"});
        if(network&&network.ok)current.put("./version.json",network.clone()).catch(()=>{});
        return network;
      }catch(_e){
        return (await current.match("./version.json"))||new Response("OFFLINE",{status:503,statusText:"Offline"});
      }
    })());
    return;
  }
  if(isNavigation){
    event.respondWith((async()=>{
      const current=await caches.open(CACHE_NAME),cached=await current.match("./index.html");
      if(cached)return cached;
      try{
        const network=await fetch(event.request,{cache:"no-store"});
        if(network&&network.ok){
          const html=patchIndexHtml(await network.clone().text());
          const patched=new Response(html,{status:network.status,statusText:network.statusText,headers:{"Content-Type":"text/html; charset=utf-8","Cache-Control":"no-cache"}});
          current.put("./index.html",patched.clone()).catch(()=>{});
          return patched;
        }
        return network;
      }catch(_e){
  ...
  if(isFreshPath(url.pathname)){
    event.respondWith((async()=>{
      const current=await caches.open(CACHE_NAME),hit=await current.match(event.request);
      if(hit)return hit;
      const canonical=await current.match(url.pathname.split('/').pop()?"./"+url.pathname.split('/').pop():event.request);
      if(canonical)return canonical;
      try{
        const r=await fetch(event.request,{cache:"no-store"});
        if(r&&r.ok)current.put(event.request,r.clone()).catch(()=>{});
        return r;
      }catch(_e){
        return new Response("OFFLINE",{status:503,statusText:"Offline"});
      }
