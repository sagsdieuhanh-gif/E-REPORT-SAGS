...
window.__SAGS_RUNTIME_BUILD__="V5.0.3B-HF2-LAYERED-PDF-AD-AUTH-FIX-NEW-UI-V1-QUICK-HANDBOOK-20260922-02";
...
(function(root){
  ...
  function install(){
    ...
    // The obsolete home report card is removed. Keep other report workflows intact.
    // No report-button handler is installed because the home button no longer exists.
...
#v451NoticeAdminModal{position:fixed;inset:0;z-index:2147482500;display:none;align-items:center;justify-content:center;padding:12px;background:rgba(5,20,36,.72);box-sizing:border-box;font-family:Arial,sans-serif}#v451NoticeAdminModal.show{display:flex}.v451NoticeCard{width:min(94vw,720px);max-height:90dvh;overflow:auto;background:#fff;border-radius:16px;box-shadow:0 20px 58px rgba(0,0,0,.42);padding:14px;color:#173d67}.v451NoticeHead{display:flex;align-items:center;gap:10px;border-bottom:1px solid #dbe5ee;padding-bottom:10px}.v451NoticeHead b{font-size:18px}.v451NoticeHead span{flex:1}.v451NoticeHead button{width:42px;height:42px;border:1px solid #cbd5e1;border-radius:10px;background:#fff;font-weight:900}.v451NoticeInfo{margin:12px 0;padding:11px;border-radius:11px;background:#eef5fb;color:#36566f;font:700 12px/1.5 Arial}.v451NoticeBlock{border:1px solid #d6e2eb;border-radius:12px;padding:12px;margin-top:10px}.v451NoticeBlock h4{margin:0 0 5px;font-size:14px}.v451NoticeBlock p{margin:0 0 10px;color:#617486;font:700 11px/1.45 Arial}.v451NoticeActions{display:flex;gap:8px;flex-wrap:wrap}.v451NoticeActions button{min-height:44px;border:1px solid #cbd5e1;border-radius:10px;padding:8px 12px;background:#f8fafc;color:#173d67;font-weight:900}.v451NoticeActions #v451ClearCloseoutBtn{background:#b42318;border-color:#b42318;color:#fff}.v451NoticeActions #v451OpenActionCenter{background:#0b67b2;border-color:#0b67b2;color:#fff}
  ...
  function badgeNumber(){
    const ids=["v342Badge","fs09NotifyBadge","khNotifyBadge"];
...
(function(root){
  ...
  function installBusinessHooks(){/* V5.0.2: direct roster opens use the canonical v324 handler; DEP choices retired. */setTimeout(()=>showDepBadge(activeMeta()),100)}
  ...
  /* Photos have a verified owner/flight primary; checkpoint holds only text
     and revision metadata, not a second multi-megabyte image payload. */
  const sagsV623PhotoKeys=['attachments','fsags421Attachments','fsags551Attachments','bbbtAttachments'];
  function writeCheckpoint(meta,env,reason){
    const now=Math.max(Date.now(),Number(env?.v22LocalRevision||0),
      Number(readCheckpoint(meta?.id)?.localRevision||0)+1),copy=clone(env)||{};
    copy.v22LocalRevision=now;copy.v22LocalSavedAtMs=Date.now();copy.v22LocalSaveReason=S(reason);
    if(meta?.rosterAssignmentId&&!copy.rosterAssignmentId)copy.rosterAssignmentId=S(meta.rosterAssignmentId);
    const pk=primaryKey(meta?.id);if(!pk)return null;
    try{
      const value=JSON.stringify(copy);localStorage.setItem(pk,value);
      if(localStorage.getItem(pk)!==value)throw Error('Không xác minh được bản lưu chuyến');
    }catch(e){lastCloudError=S(e?.message||e);console.warn('V6.1.23 primary save failed',e);return null;}
    const slim=clone(copy)||{},counts={};
    if(slim.state&&typeof slim.state==='object')for(const key of sagsV623PhotoKeys){
      if(Object.prototype.hasOwnProperty.call(slim.state,key)){
        counts[key]=Array.isArray(slim.state[key])?slim.state[key].length:0;
        delete slim.state[key];
      }
    }
    const cp={schema:2,build:BUILD,sessionId:S(meta?.id),
      rosterAssignmentId:S(meta?.rosterAssignmentId||copy.rosterAssignmentId),
      atMs:now,localRevision:now,reason:S(reason),pendingSync:!!meta?.rosterAssignmentId,
      envelope:slim,photoCountsV623:counts,photosInPrimaryV623:true};
    try{
      const value=JSON.stringify(cp),key=checkpointKey(meta?.id);
      localStorage.setItem(key,value);
      if(localStorage.getItem(key)!==value)throw Error('Không xác minh được checkpoint');
    }catch(e){cp.checkpointFailedV623=true;console.warn('V6.1.23 checkpoint save failed; verified primary remains',e);}
    return cp;
  }
  ...
  function persistNow(reason='event'){
    ...
    try{if(typeof root.persist==='function')root.persist();}
    catch(e){lastCloudError=S(e?.message||e);setSaveBadge('⚠ CHƯA LƯU ĐƯỢC',true);console.warn('V6.1.23 base persist failed',e);return false;}
    const env=currentEnvelope(meta);if(!env)return false;
    const cp=writeCheckpoint(meta,env,reason);
    if(!cp){setSaveBadge('⚠ CHƯA LƯU ĐƯỢC',true);return false;}
    lastSavedAtMs=cp.atMs;lastReason=S(reason);saveCount++;
    setSaveBadge(cp.checkpointFailedV623?'✓ ĐÃ LƯU HỒ SƠ TRÊN MÁY':
      (cp.pendingSync?'⏳ ĐÃ LƯU TRÊN MÁY':'✓ ĐÃ LƯU'),cp.pendingSync);
    Promise.resolve(flushCloud(cp,meta)).catch(()=>{});return true;
  }
  ...
  async function restoreIfNewer(){
    ...
    if(!useful(cur)||lr>cr){
      const merged=clone(cp.envelope)||{};merged.state={...(merged.state||{})};
      if(cp.photosInPrimaryV623){
        const current=cur?.state||{},counts=cp.photoCountsV623||{};
        for(const key of sagsV623PhotoKeys){
          if(Object.prototype.hasOwnProperty.call(merged.state,key))continue;
          const need=Number(counts[key]||0),photos=current[key];
          // Never overwrite a primary containing photos with a text-only
          // checkpoint when its backing photo array has disappeared.
          if(need&&(!Array.isArray(photos)||photos.length<need)){
            console.warn('V6.1.23 photo backing unavailable; checkpoint not applied',key);
            setSaveBadge('⚠ CẦN KIỂM TRA ẢNH ĐÃ LƯU',true);return false;
          }
          if(Object.prototype.hasOwnProperty.call(current,key))merged.state[key]=clone(photos);
        }
      }
      const pk=primaryKey(meta.id);
      try{if(pk){const value=JSON.stringify(merged);localStorage.setItem(pk,value);
        if(localStorage.getItem(pk)!==value)throw Error('Không xác minh được bản khôi phục');
        restored=true;}}catch(e){console.warn('V6.1.23 restore failed; original retained',e);}
    }
    if(cp.pendingSync)await flushCloud(cp,meta);return restored;
  }

  document.addEventListener('input',e=>{
    // #entryText is a temporary editor. Its value is not copied into state until commitEntry(),
    // so saving here only serializes/syncs the same old envelope while the user types.
    if(e?.target?.id==='entryText')return;
    scheduleSave('input',180);
  },true);
  ...
  // V5.0.2 retired DEP hook patching; installBusinessHooks now only refreshes the badge.
  // Avoid the former 250 ms / ~10 s polling loop on every app start.
...
(function(root){
  ...
  function install(){ /* V5.0.2: obsolete ARR→DEP named handover disabled. */ }
  install();
...
(function(root){
  ...
  function install(){ /* V5.0.2: roster is authoritative; no automatic DEP claim/reassignment. */ }
...
${files.map((f,i)=>String(i+1)+'. '+f.name).join('\n')}`;}
  ...
  async function loadCleaning(){try{const v=(await db(CLEAN_PUBLIC).once('value')).val();applyCleanCatalog(v||{});cleanLoaded=true;cleanServerReady=true}catch(e){console.info('V503 cleaning server load unavailable; skip stale alerts',e?.message||e);cleanCatalog={version:0,items:[]};cleanQueue=[];cleanLoaded=false;cleanServerReady=false}}
  ...
  function contextFor(sessionId,st,meta){
    ...
    const date=normalizeISO(identity.dateToken||pick(st,'date','f421_date','f551_date','f09_date'))||'';
  ...
  function contexts(){
    const map=new Map();
    const add=(sid,st,meta)=>{
      const c=contextFor(sid,st,meta),key=[c.date,c.flights.slice().sort().join('+'),c.reg].join('|');
      // No reliable flight number = do not infer from registration alone.
      if(!c.flights.length||!c.date||key==='||')return;
      const old=map.get(key),quality=Number(!!c.sta)+Number(!!c.chockOn)*2+Number(!!c.pushback)*4;
      if(!old||quality>old.quality)map.set(key,{...c,key,quality});
    };
    try{add(S(root.activeFlightSessionId),root.state||{},root.currentFlightSessionMeta?.()||null)}catch(_){}
    try{for(const meta of (root.readFlightSessionList?.()||[])){
      const sid=S(meta?.id);if(!sid)continue;
      const env=root.readFlightSessionEnvelope?.(sid)||{};add(sid,env.state||{},meta);
    }}catch(_){}
    return [...map.values()];
  }
  function cleanMatch(x,c){
    if(x.active===false||S(x.date)!==c.date)return false;
    const flights=[x.arrivalFlight,x.departureFlight,...(x.flights||[])].map(normFlight).filter(Boolean);
    if(!flights.length||!flights.some(f=>c.flights.includes(f)))return false;
    const expectedReg=normReg(x.acReg);if(expectedReg&&c.reg&&expectedReg!==c.reg)return false;
    return true;
  }
  function ackStore(){try{return JSON.parse(localStorage.getItem(CLEAN_ACK)||'{}')||{}}catch(_){return {}}}
  function alertKey(item,c,trigger){return `${S(item.id)}|${c.key}|${trigger}`}
  ...
  function evaluateCleaning(){
    if(role()!=='DH'||!cleanCatalog.version||!cleanServerReady)return;
    const now=Date.now(),acks=ackStore(),validKeys=new Set();
    for(const c of contexts()){
      const matches=cleanCatalog.items.filter(x=>cleanMatch(x,c));if(!matches.length)continue;
      const latest=matches.slice().sort((a,b)=>Number(b.updatedAtMs||0)-Number(a.updatedAtMs||0))[0];
      if(!latest)continue;
      // Arriving late / old saved flights must not re-trigger an STA reminder.
      const due=clockMs(c.date,c.sta),chock=clockMs(c.date,c.chockOn),triggers=[];
      if(!c.pushback&&!c.chockOn&&due&&now>=due-10*60000&&now<=due+15*60000)triggers.push('STA_T10');
      if(!c.pushback&&chock&&now>=chock&&now<=chock+12*60000)triggers.push('CHOCK_ON');
      for(const trigger of triggers){
        const key=alertKey(latest,c,trigger);validKeys.add(key);
        if(acks[key]||cleanCurrent?.key===key||cleanQueue.some(a=>a.key===key))continue;
        cleanQueue.push({key,item:latest,ctx:c,trigger});
      }
    }
    cleanQueue=cleanQueue.filter(a=>validKeys.has(a.key));
    if(cleanCurrent&&!validKeys.has(cleanCurrent.key)){
      const panel=$('v2211CleaningAlert');if(panel)panel.style.display='none';cleanCurrent=null;
    }
    tryShowCleaning();
  }
...
const BUILD='V6.2.9-FORM-MANAGER-LIVE-REGISTRY',REG_URL='./forms.registry.json',LOCAL_KEY='sagsFormRegistryDraftV450',OLD_LOCAL_KEY='sagsFormRegistryDraftV440',DB_NAME='sags-form-assets-v440',STORE='assets';
const $=id=>document.getElementById(id),esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const clone=v=>{try{return JSON.parse(JSON.stringify(v))}catch(_){return v}};
let published={schema:2,forms:[]},draft=null,currentId='',pageIndex=0,selectedKey='',selectedMany=new Set(),history=[],future=[],objectUrls=[],testMode=false,testEditMode=true,testValuesV462={},directEditV627=true;
...
const FM_TYPES_V487=new Set(['text','number','time','date','checkbox','select','textarea','signature','computed','static-text']);
const FM_FONTS_V487=['Times New Roman','Arial','system-ui','Courier New'];
let styleClipboardV487=null;
function fmClampV487(v,min,max,fallback){const n=Number(v);return Number.isFinite(n)?Math.max(min,Math.min(max,n)):fallback}
function fmFontV487(v){const s=String(v||'Times New Roman').trim();return FM_FONTS_V487.includes(s)?s:'Times New Roman'}
function fmColorV487(v){const s=String(v||'').trim();return /^#[0-9a-f]{6}$/i.test(s)?s:'#003b8e'}
function fmOptionsV487(v){const a=Array.isArray(v)?v:String(v||'').split(/\r?\n|,/);return [...new Set(a.map(x=>String(x||'').trim()).filter(Boolean))].slice(0,80)}
function fmNormalizeFieldV487(x){if(!x||typeof x!=='object')return x;x.key=fieldKey(x.key||x.bind||'field');x.bind=fieldKey(x.bind||x.key)||x.key;x.label=String(x.label||x.key||'').slice(0,160);x.type=FM_TYPES_V487.has(String(x.type||'').toLowerCase())?String(x.type).toLowerCase():'text';x.x=fmClampV487(x.x,0,1,0);x.y=fmClampV487(x.y,0,1,0);x.w=fmClampV487(x.w,.005,1,.12);x.h=fmClampV487(x.h,.005,1,.035);if(x.x+x.w>1)x.w=Math.max(.005,1-x.x);if(x.y+x.h>1)x.h=Math.max(.005,1-x.y);x.fontSize=fmClampV487(x.fontSize,6,80,14);x.fontFamily=fmFontV487(x.fontFamily);x.fontWeight=String(['400','500','600','700','800','900'].includes(String(x.fontWeight))?x.fontWeight:'700');x.fontStyle=(String(x.fontStyle||'').toLowerCase()==='italic'||x.italic===true)?'italic':'normal';x.underline=!!x.underline;x.lineHeight=fmClampV487(x.lineHeight,.8,4,1.08);x.letterSpacing=fmClampV487(x.letterSpacing,-2,10,0);x.textColor=fmColorV487(x.textColor);x.align=['left','center','right'].includes(String(x.align))?String(x.align):'left';x.valign=['top','middle','bottom'].includes(String(x.valign))?String(x.valign):'middle';x.wrapMode=fmIsMultiV489(x)?'auto':'single';if(fmIsMultiV489(x)){if(!Number.isFinite(Number(x.rowCount))||Number(x.rowCount)<1)x.rowCount=fmInferRowsV490(x);x.rowCount=Math.max(1,Math.min(30,Math.round(Number(x.rowCount)||1)));if(x.rowFit===undefined)x.rowFit=true;else x.rowFit=!!x.rowFit;if(x.rowFit)fmApplyRowFitV490(x)}else{x.rowFit=false;x.rowCount=1}if(x.type==='select')x.options=fmOptionsV487(x.options);return x}
function fmNormalizeFormV487(f){if(!f||typeof f!=='object')return f;f.pages=Array.isArray(f.pages)?f.pages:[];f.fields=Array.isArray(f.fields)?f.fields:[];f.fields.forEach(fmNormalizeFieldV487);return f}
function fmNormalizeRegistryV487(r){if(!r||typeof r!=='object')return r;r.forms=Array.isArray(r.forms)?r.forms:[];r.forms.forEach(fmNormalizeFormV487);return r}
function fmStyleV487(fld){fmNormalizeFieldV487(fld);return {fontSize:Number(fld.fontSize),fontFamily:fmFontV487(fld.fontFamily),fontWeight:String(fld.fontWeight||'700'),fontStyle:String(fld.fontStyle||'normal'),underline:!!fld.underline,lineHeight:Number(fld.lineHeight||1.08),letterSpacing:Number(fld.letterSpacing||0),textColor:fmColorV487(fld.textColor),align:String(fld.align||'left'),valign:String(fld.valign||'middle'),rowFit:!!fld.rowFit,rowCount:Math.max(1,Math.round(Number(fld.rowCount)||1))}}
function fmCopyStyleV487(src,dst){const s=fmStyleV487(src);Object.assign(dst,s);return fmNormalizeFieldV487(dst)}
function fmCssV487(fld,scaled=false){const s=fmStyleV487(fld),fs=scaled?`calc(${s.fontSize}px * var(--v451-page-scale,1))`:`${s.fontSize}px`;return `font-family:${s.fontFamily};font-size:${fs};font-weight:${s.fontWeight};font-style:${s.fontStyle};text-decoration:${s.underline?'underline':'none'};line-height:${s.lineHeight};letter-spacing:${s.letterSpacing}px;color:${s.textColor};text-align:${s.align}`}
function fmIsMultiV489(fld){const t=String(fld?.type||'').toLowerCase();return t==='textarea'||t==='static-text'}
function fmPageForFieldV490(fld){try{const f=form(),id=String(fld?.pageId||'');return (f?.pages||[]).find(p=>String(p?.id||'')===id)||page()||null}catch(_){return null}}
function fmInferRowsV490(fld,p=null){const pg=p||fmPageForFieldV490(fld),pageH=Math.max(1,Number(pg?.height||1755)),boxH=Math.max(1,Number(fld?.h||.035)*pageH),fs=Math.max(6,Number(fld?.fontSize||14));return Math.max(2,Math.min(30,Math.round(boxH/Math.max(fs*1.8,1))))}
function fmApplyRowFitV490(fld,p=null){if(!fmIsMultiV489(fld))return fld;const pg=p||fmPageForFieldV490(fld),pageH=Math.max(1,Number(pg?.height||1755)),boxH=Math.max(1,Number(fld.h||.035)*pageH),rows=Math.max(1,Math.min(30,Math.round(Number(fld.rowCount)||fmInferRowsV490(fld,pg)))),fs=Math.max(6,Number(fld.fontSize||14));fld.rowCount=rows;fld.lineHeight=fmClampV487(boxH/(rows*fs),.8,4,1.5);return fld}
function fmLineCapacityV489(fld,p=null){fmNormalizeFieldV487(fld);if(!fmIsMultiV489(fld))return 1;if(fld.rowFit)return Math.max(1,Math.min(30,Math.round(Number(fld.rowCount)||1)));const pageH=Math.max(1,Number(p?.height||page()?.height||1755)),boxH=Math.max(1,Number(fld.h||.035)*pageH),lh=Math.max(1,Number(fld.fontSize||14)*Number(fld.lineHeight||1.08));return Math.max(1,Math.min(40,Math.floor(Math.max(lh,boxH-4)/lh)))}
function fmCanvasBaselineV490(st,py,ph,count,lh){const n=Math.max(1,count),blockH=n*lh,top=st.valign==='bottom'?py+Math.max(0,ph-blockH):st.valign==='middle'?py+Math.max(0,(ph-blockH)/2):py,lead=Math.max(0,(lh-st.fontSize)/2);return top+lead+st.fontSize*.82}
function validateFormV487(f){const errors=[],warnings=[];if(!f)return {errors:['Chưa chọn form.'],warnings};const pages=Array.isArray(f.pages)?f.pages:[],fields=Array.isArray(f.fields)?f.fields:[],pageIds=new Set(),keys=new Set(),binds=new Map();for(const p of pages){const id=String(p?.id||'').trim();if(!id)errors.push('Trang thiếu ID.');else if(pageIds.has(id))errors.push('Trùng pageId: '+id);else pageIds.add(id);if(!(Number(p?.width)>0&&Number(p?.height)>0))errors.push('Trang '+(id||'?')+' thiếu kích thước hợp lệ.')}for(const raw of fields){const x=fmNormalizeFieldV487(raw),k=String(x.key||'');if(!k)errors.push('Field thiếu KEY.');else if(keys.has(k))errors.push('Trùng KEY: '+k);else keys.add(k);if(!pageIds.has(String(x.pageId||'')))errors.push('Field '+k+' trỏ tới trang không tồn tại: '+String(x.pageId||''));if(x.x<0||x.y<0||x.w<=0||x.h<=0||x.x+x.w>1.00001||x.y+x.h>1.00001)errors.push('Field '+k+' vượt khỏi trang.');if(x.type==='select'&&!x.options?.length)warnings.push('Select '+k+' chưa có OPTIONS.');const b=String(x.bind||'');if(b){if(binds.has(b)&&binds.get(b)!==k)warnings.push('BIND '+b+' đang dùng chung: '+binds.get(b)+' + '+k);else binds.set(b,k)}}return {errors:[...new Set(errors)],warnings:[...new Set(warnings)]}}
function validateCurrentFormV487(){const f=form(),r=validateFormV487(f);const msg=[r.errors.length?`❌ ${r.errors.length} lỗi`:'✓ Không có lỗi cấu trúc',r.warnings.length?`⚠ ${r.warnings.length} cảnh báo`:'✓ Không có cảnh báo',...r.errors.slice(0,8),...r.warnings.slice(0,8)].join('\n');status(msg,!!r.errors.length);if(r.errors.length||r.warnings.length)alert(msg);return !r.errors.length}
function normalizeCurrentFormV487(){const f=form();if(!f)return;pushHistory();fmNormalizeFormV487(f);f.updatedAtMs=Date.now();saveLocal();renderAll();status('✓ Đã chuẩn hóa schema + typography toàn form. Tọa độ và dữ liệu bind được giữ nguyên.')}
...
function syncLegacyToLive(redraw=false){const gs=globalFieldsV45();if(!gs.length)return false;const source=draft?.forms?.length?draft:published?.forms?.length?published:null;if(!source)return false;
  ...
  for(const fm of source.forms||[]){if(!fm?.legacy)continue;const pageById=new Map((fm.pages||[]).map(p=>[String(p.id),Number(p.sourcePage||pageNoV45(p)||0)]));
    for(const x of fm.fields||[]){
      ...
      if(original){original.vx=Number(x.x);original.vy=Number(x.y);original.vw=Number(x.w);original.vh=Number(x.h);if(Number.isFinite(Number(x.fontSize)))original.font=Number(x.fontSize);original.align=String(x.align||'left');original.valign=String(x.valign||'middle');original.leftValue=original.align==='left';if(original.leftValue)original.manualInset=0;const wantsMulti=String(x.type||'').toLowerCase()==='textarea',nativeMulti=!!x.sourceMultiline;original.multiline=wantsMulti;original.autoFitLines=0;
      // On 42.1, the published Form Manager registry is the geometry authority.
      // Old hand-tuned dot rectangles/vertical lifts belong to an earlier template.
      const canonical421=(Number(original.page)===6||Number(original.page)===7)&&String(x.key||'').startsWith('f421_')&&!['check','displayCheck','signature'].includes(String(original.type||''));
      if(canonical421){
        original.__formManagerCanonical421=true;
        original.displayDy=0;original.aboveDots=false;original.writeOnLine=false;
        if(!wantsMulti){original.x=Number(x.x);original.y=Number(x.y);original.w=Number(x.w);original.h=Number(x.h);original.dotRects=null;}
      }
      if(wantsMulti&&(!nativeMulti||canonical421)){original.__formManagerExactTextV490=true;original.fmLineHeight=fmClampV487(x.lineHeight,.8,3,1.08);original.fmFontFamily=fmFontV487(x.fontFamily);original.fmFontWeight=String(x.fontWeight||'700');original.fmFontStyle=String(x.fontStyle||'normal');original.fmUnderline=!!x.underline;original.fmLetterSpacing=fmClampV487(x.letterSpacing,-2,10,0);original.fmTextColor=fmColorV487(x.textColor)}else{delete original.__formManagerExactTextV490;delete original.fmLineHeight;delete original.fmFontFamily;delete original.fmFontWeight;delete original.fmFontStyle;delete original.fmUnderline;delete original.fmLetterSpacing;delete original.fmTextColor}continue}
      ...
      gs.push({page:pg,key,type:rt,label:String(x.label||x.key||key),x:gx,y:gy,w:gw,h:gh,vx:gx,vy:gy,vw:gw,vh:gh,tickX:gx+gw/2,tickY:gy+gh/2,font:Math.max(6,Number(x.fontSize||14)),align:al,valign:va,multiline:String(x.type||'')==='textarea',inputMode:String(x.type||'')==='number'?'numeric':'text',filter:String(x.type||'')==='number'?'number':String(x.type||'')==='time'?'time':String(x.type||'')==='date'?'date':null,manualInset:0,leftValue:al==='left',dotRects:null,displayDy:0,autoFitLines:0,aboveDots:false,writeOnLine:false,lineRatio:null,hitPadX:0,hitPadY:0,__formManagerExactTextV490:String(x.type||'')==='textarea',fmLineHeight:fmClampV487(x.lineHeight,.8,3,1.08),fmFontFamily:fmFontV487(x.fontFamily),fmFontWeight:String(x.fontWeight||'700'),fmFontStyle:String(x.fontStyle||'normal'),fmUnderline:!!x.underline,fmLetterSpacing:fmClampV487(x.letterSpacing,-2,10,0),fmTextColor:fmColorV487(x.textColor),__formManagerCustomV463:true,__formManagerFieldKey:String(x.key||key),__formManagerFormId:String(fm.id||''),__formManagerOriginalType:String(x.type||'text')});
    }}
  if(redraw){try{if(typeof draw==='function')draw()}catch(_){}}return true}
function saveLocal(){try{fmNormalizeRegistryV487(draft);const cf=form();if(cf)cf.updatedAtMs=Date.now();localStorage.setItem(LOCAL_KEY,JSON.stringify(draft||{schema:2,forms:[]}))}catch(e){console.warn('V4.5 form local save',e)}try{syncLegacyToLive(false)}catch(_){}}
...
function mergeRegistry(){
  const local=isAD()?loadLocal():null;
  // V6.2.1: published SYSTEM geometry is authoritative on every device.
  // Older AD-local Form Manager drafts may contain obsolete 3/4-row limits or
  // signature boxes. Preserve only AD-created CUSTOM forms/fields; never let a
  // stale local copy replace official legacy-field geometry.
  draft=clone(published);draft.schema=2;draft.version='V4.8.10';draft.build=BUILD;draft.forms=Array.isArray(draft.forms)?draft.forms:[];
  if(local?.forms?.length){
    for(const lf of local.forms){
      if(!lf?.legacy){
        const i=draft.forms.findIndex(x=>String(x?.id)===String(lf.id));
        if(i>=0)draft.forms[i]=clone(lf);else draft.forms.push(clone(lf));
        continue;
      }
      const pf=draft.forms.find(x=>String(x?.id)===String(lf.id));if(!pf)continue;
      pf.fields=Array.isArray(pf.fields)?pf.fields:[];
      const known=new Set(pf.fields.map(x=>String(x.key)));
      for(const x of (lf.fields||[])){
        if(!(x?.customFieldV463===true||x?.generatedFromLegacy===false))continue;
        const k=String(x.key||'');if(!k||known.has(k))continue;
        pf.fields.push(clone(x));known.add(k);
      }
    }
  }
  hydrateLegacyForms();fmNormalizeRegistryV487(draft);syncLegacyToLive(false);return draft
}
...
#v440Fm{position:fixed;inset:0;z-index:16050;background:rgba(11,18,32,.72);display:none;align-items:stretch;justify-content:center;padding:8px;box-sizing:border-box;font:13px Arial;color:#172033}#v440Fm.show{display:flex}#v440FmCard{width:min(99vw,1500px);height:calc(100vh - 16px);background:#f8fafc;border-radius:15px;overflow:hidden;display:grid;grid-template-rows:auto 1fr;box-shadow:0 18px 50px rgba(0,0,0,.38)}.v440FmHead{display:flex;align-items:center;gap:8px;padding:9px 12px;background:#0b4a7b;color:#fff}.v440FmHead b{font-size:15px}.v440FmHead .grow{flex:1}.v440Btn{border:1px solid #cbd5e1;border-radius:8px;background:#fff;padding:7px 9px;font-weight:800;cursor:pointer}.v440Btn.primary{background:#0569ad;color:#fff;border-color:#0569ad}.v440Btn.green{background:#087a55;color:#fff;border-color:#087a55}.v440Btn.danger{background:#fff1f2;color:#b42318;border-color:#fda4af}.v440Btn:disabled{opacity:.45}.v440Btn.active{background:#075ea8;color:#fff;border-color:#075ea8}.v461TypographyBox{margin:10px 0;padding:12px;border:1px solid #d9ccff;border-radius:14px;background:linear-gradient(180deg,#fcfbff 0%,#f7f3ff 100%);box-shadow:0 2px 10px rgba(124,58,237,.06)}.v440FmBody{min-height:0;display:grid;grid-template-columns:260px 1fr 300px;gap:8px;padding:8px}.v440Pane{min-height:0;overflow:auto;background:#fff;border:1px solid #d0d5dd;border-radius:11px;padding:9px;box-sizing:border-box}.v440FormItem{display:block;width:100%;text-align:left;border:1px solid #e4e7ec;background:#fff;border-radius:9px;padding:8px;margin-bottom:6px;font-weight:800}.v440FormItem.active{border-color:#7c3aed;background:#f5f3ff}.v440Tag{display:inline-block;font-size:9px;border-radius:999px;padding:2px 5px;margin-left:5px;background:#eef2ff;color:#4338ca}.v440Row{display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin:6px 0}.v440Field{display:grid;gap:3px;margin:7px 0}.v440Field label{font-size:10px;font-weight:900;color:#475467}.v440Field input,.v440Field select,.v440Field textarea{width:100%;box-sizing:border-box;border:1px solid #cbd5e1;border-radius:10px;padding:8px 10px;background:#fff}.v440EditorViewport{width:100%;height:calc(100vh - 155px);min-height:420px;overflow:auto;background:#e9eef3;border:1px solid #d0d5dd;border-radius:10px;padding:12px;box-sizing:border-box;overscroll-behavior:contain}.v440EditorWrap{position:relative;margin:0 auto;width:max-content;max-width:none;background:#e5e7eb;box-shadow:0 0 0 1px #cbd5e1;user-select:none;transform-origin:top left}.v440EditorWrap img{display:block;width:100%;height:auto;max-width:none;max-height:none}.v440Overlay{position:absolute;inset:0}.v451ZoomTools{display:flex;align-items:center;gap:5px;flex-wrap:wrap;position:sticky;top:0;z-index:12;background:#fff8d8;border:2px solid #f59e0b;border-radius:10px;padding:6px 8px;box-shadow:0 2px 8px rgba(0,0,0,.08)}.v453ZoomTitle{font-weight:1000;color:#8a4b00;white-space:nowrap}.v451ZoomTools .v451ZoomLabel{min-width:58px;text-align:center;font-weight:900;color:#173d67;background:#eef5fb;border-radius:8px;padding:7px 8px}.v451ZoomTools input[type=range]{width:130px;accent-color:#0b67b2}.v461TextAlignBox{margin:10px 0;padding:12px;border:1px solid #bfdbfe;border-radius:14px;background:linear-gradient(180deg,#f8fbff 0%,#eff6ff 100%);box-shadow:0 2px 10px rgba(11,74,123,.05)}.v461TextAlignTitle{font-weight:1000;color:#0b4a7b;margin-bottom:4px;letter-spacing:.2px}.v487SectionSub{font-size:10px;line-height:1.35;color:#667085;font-weight:800;margin-bottom:8px}.v487ControlGrid2{display:grid;grid-template-columns:1fr 1fr;gap:8px}.v487ControlGrid4{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.v487ActionGrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:8px}.v487ActionGrid.three{grid-template-columns:repeat(3,minmax(0,1fr))}.v487ActionGrid .v440Btn{min-height:40px;display:flex;align-items:center;justify-content:center;text-align:center;line-height:1.15;padding:8px 10px}.v487ActionGrid.tight .v440Btn{min-height:36px}.v461TextAlignRow{display:grid;grid-template-columns:60px 1fr;gap:8px;align-items:start;margin:7px 0}.v461TextAlignRow>span{font-size:10px;font-weight:1000;color:#475467;padding-top:11px}.v487AlignGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}.v461AlignBtn{min-height:40px;border:1px solid #b7c6d7;border-radius:10px;background:#fff;color:#173d67;font-weight:900;cursor:pointer;padding:8px 6px}.v461AlignBtn.active{background:#075ea8;border-color:#075ea8;color:#fff;box-shadow:0 0 0 2px #bfdbfe}.v461CenterBoth{width:100%;margin-top:8px;min-height:42px;border:1px solid #087a55;border-radius:12px;background:#ecfdf3;color:#087a55;font-weight:1000;cursor:pointer}.v461AlignHint{margin-top:7px;font-size:10px;line-height:1.45;color:#52687a;font-weight:700}.v461FieldPositionTitle{margin-top:10px;font-size:10px;font-weight:1000;color:#7c3aed;text-transform:uppercase;letter-spacing:.4px}.v487Divider{height:1px;background:#e5e7eb;margin:10px 0}.v487CompactHelp{font-size:10px;color:#52687a;font-weight:800;margin-top:6px;line-height:1.4}.v440Box{position:absolute;border:2px solid #7c3aed;background:rgba(124,58,237,.08);box-sizing:border-box;min-width:8px;min-height:8px;cursor:move}.v440Box.ai{border-color:#0284c7;background:rgba(14,165,233,.09)}.v440Box.ai>span{background:#0369a1}.v440Box.ai-preview{border:2px dashed #16a34a;background:rgba(34,197,94,.10);pointer-events:none;z-index:8}.v440Box.ai-preview>span{background:#166534}.v460AiModal{position:fixed;inset:0;z-index:19550;background:#0b1220cc;display:none;align-items:center;justify-content:center;padding:12px}.v460AiModal.show{display:flex}.v460AiCard{width:min(94vw,760px);max-height:90vh;overflow:auto;background:#fff;border-radius:16px;padding:16px;box-shadow:0 22px 70px #0007}.v460AiHead{display:flex;align-items:center;gap:8px}.v460AiHead b{font-size:18px;color:#075ea8}.v460AiInfo{margin:9px 0;padding:10px;border-radius:10px;background:#eef7ff;color:#244761;font-weight:700}.v460AiGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.v460AiProgress{height:10px;border-radius:999px;background:#e5e7eb;overflow:hidden;margin:9px 0}.v460AiProgress>i{display:block;height:100%;width:0;background:#0b67b2;transition:width .15s}.v460AiStatus{white-space:pre-wrap;min-height:42px;padding:9px;border-radius:9px;background:#f8fafc;border:1px solid #e2e8f0;font-weight:700}.v460AiList{max-height:250px;overflow:auto;border:1px solid #e2e8f0;border-radius:9px;margin-top:8px}.v460AiRow{display:grid;grid-template-columns:64px 1fr 100px 100px;gap:6px;padding:7px;border-bottom:1px solid #eef2f7;font-size:11px;align-items:center}.v460AiRow:last-child{border-bottom:0}.v460AiScore{font-weight:900;color:#067647}.v460AiWarn{color:#b54708}.v460AiBtn{border:1px solid #cbd5e1;background:#fff;border-radius:9px;padding:8px 10px;font-weight:900;cursor:pointer}.v460AiBtn.primary{background:#075ea8;color:#fff;border-color:#075ea8}.v460AiBtn.good{background:#087a55;color:#fff;border-color:#087a55}.v460AiBtn.danger{background:#fff1f2;color:#b42318;border-color:#fda4af}.v460AiActions{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}@media(max-width:700px){.v460AiGrid{grid-template-columns:1fr}.v460AiRow{grid-template-columns:54px 1fr 80px}}.v440Box.sel{border-color:#dc2626;background:rgba(220,38,38,.10);z-index:3}.v440Box.multi{border-color:#f59e0b}.v450TestValue{position:absolute;inset:0;display:flex;align-items:center;overflow:hidden;padding:1px 3px;box-sizing:border-box;color:#003b8e;pointer-events:none;white-space:pre-wrap}.v450TestMode .v440Box{border-color:rgba(16,185,129,.28);background:rgba(255,255,255,.02)}.v450TestMode .v440Box>span,.v450TestMode .v440Resize{display:none!important}.v462TestModeBtn{display:none!important;min-width:190px}.v462TestModeBtn.show{display:inline-flex!important;align-items:center;justify-content:center}.v462TestModeBtn.edit{background:#ecfdf3;color:#067647;border-color:#6ce9a6}.v462TestModeBtn.interact{background:#eff8ff;color:#175cd3;border-color:#84caff}.v462TestHint{margin:6px 0;padding:7px 9px;border-radius:9px;font-size:10px;font-weight:900;line-height:1.35}.v462TestHint.edit{background:#ecfdf3;color:#067647}.v462TestHint.interact{background:#eff8ff;color:#175cd3}.v450TestMode.v462TestEdit .v440Box{border-color:rgba(124,58,237,.55);background:rgba(255,255,255,.03);cursor:move}.v450TestMode.v462TestEdit .v440Box.sel{border-color:#dc2626;background:rgba(220,38,38,.08)}.v450TestMode.v462TestEdit .v440Box.sel>span,.v450TestMode.v462TestEdit .v440Box.sel>.v440Resize{display:block!important}.v450TestMode.v462TestInteract .v440Box{border-color:transparent!important;background:transparent!important;cursor:default}.v450TestMode.v462TestInteract .v440Box.sel{outline:1px dashed rgba(37,99,235,.4)}.v462TestControl{width:100%;height:100%;box-sizing:border-box;border:0!important;border-radius:0!important;background:transparent!important;color:#003b8e;padding:0 2px!important;margin:0;font-family:'Times New Roman';font-weight:700;outline:none;box-shadow:none;min-width:0;min-height:0}.v462TestControl:focus{box-shadow:inset 0 0 0 1px rgba(37,99,235,.38)!important;background:rgba(255,255,255,.08)!important}.v462TestControl:is(textarea){display:block;resize:none!important;overflow:hidden!important;scrollbar-width:none}.v462TestControl:is(textarea)::-webkit-scrollbar{display:none}.v450TestValue,.v462TestStatic{padding:0 2px!important}.v462TestCheck{width:100%;height:100%;margin:0;accent-color:#075ea8}.v462TestStatic{position:absolute;inset:0;display:flex;overflow:hidden;padding:1px 3px;box-sizing:border-box;color:#003b8e}.v462KeyboardHelp{font-size:10px;color:#52687a;font-weight:800;margin-top:5px}.v450Perf{font:800 10px/1.35 Arial;color:#dff7ea;white-space:nowrap}.v440Box span{position:absolute;left:0;top:-17px;background:#4c1d95;color:#fff;font:800 9px Arial;padding:2px 4px;white-space:nowrap;max-width:180px;overflow:hidden;text-overflow:ellipsis}.v440Resize{position:absolute;right:-5px;bottom:-5px;width:11px;height:11px;border-radius:50%;background:#dc2626;border:2px solid #fff;cursor:nwse-resize}
.v627DirectBtn.active{background:#7c3aed!important;color:#fff!important;border-color:#6d28d9!important;box-shadow:0 0 0 2px #ddd6fe inset!important}
.v627DirectOn .v440Box{background:rgba(255,255,255,.025);border-color:rgba(124,58,237,.72)}
.v627DirectOn .v440Box.sel{border-color:#dc2626;background:rgba(255,255,255,.04);box-shadow:0 0 0 1px rgba(220,38,38,.2)}
.v627DirectOn .v450TestValue{opacity:.96;z-index:1}
.v627Handle{position:absolute;z-index:12;width:14px;height:14px;border:2px solid #fff;background:#dc2626;border-radius:50%;box-sizing:border-box;touch-action:none}
.v627Handle[data-resize=n]{left:50%;top:-8px;transform:translateX(-50%);cursor:ns-resize}.v627Handle[data-resize=s]{left:50%;bottom:-8px;transform:translateX(-50%);cursor:ns-resize}
.v627Handle[data-resize=e]{right:-8px;top:50%;transform:translateY(-50%);cursor:ew-resize}.v627Handle[data-resize=w]{left:-8px;top:50%;transform:translateY(-50%);cursor:ew-resize}
.v627Handle[data-resize=nw]{left:-9px;top:-9px;cursor:nwse-resize}.v627Handle[data-resize=ne]{right:-9px;top:-9px;cursor:nesw-resize}.v627Handle[data-resize=sw]{left:-9px;bottom:-9px;cursor:nesw-resize}.v627Handle[data-resize=se]{right:-9px;bottom:-9px;cursor:nwse-resize}
.v627RowGuides{position:absolute;inset:0;pointer-events:none;z-index:7;overflow:visible}.v627RowGuides i{position:absolute;left:0;right:0;border-top:1px dashed rgba(220,38,38,.72);height:0}.v627RowGuides i em{position:absolute;right:2px;top:-12px;background:#fff7ed;color:#9a3412;border:1px solid #fdba74;border-radius:4px;padding:1px 3px;font:800 8px Arial;font-style:normal}.v628RowCal{position:absolute;inset:0;z-index:8;width:100%;height:100%;overflow:visible;pointer-events:none}.v628RowCal .rule{stroke:#e11d48;stroke-width:1;stroke-dasharray:5 4;vector-effect:non-scaling-stroke}.v628RowCal .base{stroke:#2563eb;stroke-width:.8;stroke-dasharray:2 3;opacity:.55;vector-effect:non-scaling-stroke}.v628RowCal text{fill:#003b8e}.v628RowMode{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:7px}.v628RowMode .v440Btn{min-height:38px;font-size:10px;padding:6px}.v628RowMode .active{background:#7c3aed!important;color:#fff!important;border-color:#6d28d9!important}
.v627Geo{position:absolute;left:2px;bottom:2px;z-index:11;max-width:calc(100% - 4px);padding:2px 4px;border-radius:5px;background:rgba(17,24,39,.86);color:#fff;font:800 8px/1.2 Arial;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;pointer-events:none}
.v627DirectOn .v440Box:not(.sel) .v627Handle,.v627DirectOn .v440Box:not(.sel) .v627Geo,.v627DirectOn .v440Box:not(.sel) .v627RowGuides{display:none!important}
.v450TestMode.v462TestInteract .v627Handle,.v450TestMode.v462TestInteract .v627Geo,.v450TestMode.v462TestInteract .v627RowGuides{display:none!important}
@media(max-width:700px){.v627Handle{width:18px;height:18px}.v627Handle[data-resize=n]{top:-10px}.v627Handle[data-resize=s]{bottom:-10px}.v627Handle[data-resize=e]{right:-10px}.v627Handle[data-resize=w]{left:-10px}.v627Handle[data-resize=nw]{left:-11px;top:-11px}.v627Handle[data-resize=ne]{right:-11px;top:-11px}.v627Handle[data-resize=sw]{left:-11px;bottom:-11px}.v627Handle[data-resize=se]{right:-11px;bottom:-11px}.v627Geo{font-size:9px}}
.v440PageTabs{display:flex;gap:5px;flex-wrap:wrap;justify-content:center;margin:4px 0 8px}.v440PageTabs button.active{background:#0b4a7b;color:#fff}.v440Empty{padding:30px;text-align:center;color:#667085;font-weight:800}.v440Status{font-size:11px;color:#067647;font-weight:800;min-height:15px;margin-top:5px}.v440Grid2{display:grid;grid-template-columns:1fr 1fr;gap:6px}.v440Grid4{display:grid;grid-template-columns:repeat(4,1fr);gap:5px}.v440Runtime{position:fixed;inset:0;z-index:15500;background:#eef2f6;display:none;overflow:auto;padding:8px}.v440Runtime.show{display:block}.v440RuntimeTop{position:sticky;top:0;z-index:20;background:#fff;border:1px solid #d0d5dd;border-radius:10px;padding:8px;display:flex;gap:8px;align-items:center;margin-bottom:8px}.v440RuntimePage{position:relative;width:min(100%,900px);margin:0 auto 12px;background:#fff;box-shadow:0 2px 10px rgba(0,0,0,.16)}.v440RuntimePage>img{display:block;width:100%;height:auto}.v440RuntimeField{position:absolute;box-sizing:border-box}.v440RuntimeField input,.v440RuntimeField textarea,.v440RuntimeField select,.v440RuntimeField button{width:100%;height:100%;box-sizing:border-box;border:0;background:transparent;font:inherit;padding:0 2px;outline:none;border-radius:0;box-shadow:none}.v440RuntimeField input:focus,.v440RuntimeField textarea:focus,.v440RuntimeField select:focus{box-shadow:inset 0 0 0 1px rgba(0,82,160,.32);background:rgba(255,255,255,.08)}.v440RuntimeField textarea{display:block;resize:none;overflow:hidden;scrollbar-width:none}.v440RuntimeField textarea::-webkit-scrollbar{display:none}.v440RuntimeField input[type=checkbox]{width:100%;height:100%}@media(max-width:900px){.v440FmBody{grid-template-columns:1fr}.v440Pane.left,.v440Pane.right{max-height:34vh}.v440EditorViewport{height:58vh;min-height:360px}}@media print{body.v440Printing>*:not(#v440CustomRuntime){display:none!important}body.v440Printing #v440CustomRuntime{display:block!important;position:static!important;padding:0!important;background:#fff!important}.v440RuntimeTop{display:none!important}.v440RuntimePage{break-after:page;box-shadow:none;width:100%!important;margin:0!important}}
`;document.head.appendChild(st)}
function ensureUiPolishV489(){if($('v489FormManagerPolish'))return;const st=document.createElement('style');st.id='v489FormManagerPolish';st.textContent=`
#v440Fm{padding:6px}#v440FmCard{border-radius:16px;background:#f5f7fa}.v440FmHead{gap:6px;padding:8px 10px;flex-wrap:wrap;box-shadow:0 1px 0 #ffffff22}.v440FmHead>b{font-size:14px;letter-spacing:.15px;min-width:245px}.v440FmHead .v440Btn{min-height:36px;padding:7px 9px;font-size:11px}.v440FmBody{grid-template-columns:minmax(210px,240px) minmax(0,1fr) minmax(320px,360px);gap:7px;padding:7px}.v440Pane{border-radius:13px;padding:8px;box-shadow:0 1px 4px rgba(15,23,42,.04)}.v440Pane.left{background:#fbfdff}.v440Pane.right{background:#fbfcfe}.v440FormItem{border-radius:10px;padding:9px}.v440FormItem.active{box-shadow:0 0 0 2px #ddd6fe inset}.v451ZoomTools{border-width:1px;border-color:#f4c66a;background:#fffaf0;box-shadow:none}.v451ZoomTools .v440Btn{min-height:34px;padding:6px 8px}.v440EditorViewport{border-radius:12px;background:#e8edf3;padding:10px}.v440Status{position:sticky;bottom:0;background:#fffffff0;backdrop-filter:blur(5px);padding:6px 2px 2px;z-index:6}.v489InspectorSection{border:1px solid #d9e1e8;border-radius:12px;background:#fff;margin:0 0 8px;overflow:hidden}.v489InspectorSection>summary{list-style:none;cursor:pointer;padding:10px 11px;background:#f8fafc;color:#28445e;font:900 11px/1.2 Arial;display:flex;align-items:center;gap:6px;border-bottom:1px solid transparent}.v489InspectorSection[open]>summary{border-bottom-color:#e6ebf0;background:#f4f8fb}.v489InspectorSection>summary::-webkit-details-marker{display:none}.v489InspectorSection>summary:after{content:'▾';margin-left:auto;color:#64748b}.v489InspectorSection:not([open])>summary:after{content:'▸'}.v489InspectorBody{padding:9px}.v489FieldTitle{display:flex;gap:7px;align-items:center;flex-wrap:wrap;margin-bottom:6px}.v489FieldTitle b{color:#0b4a7b}.v489TypeHint{font:800 10px/1.35 Arial;color:#52687a;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:7px 8px;margin-top:5px}.v489WrapGood{background:#ecfdf3!important;border-color:#86efac!important;color:#067647!important}.v489WrapSingle{background:#f8fafc!important;color:#475467!important}.v489RowFitBox{display:grid;grid-template-columns:1fr;gap:6px;margin-top:7px;padding:8px;border:1px solid #bfdbfe;border-radius:10px;background:#f8fbff}.v489RowFitBox>label{display:grid;grid-template-columns:auto 90px;align-items:center;gap:8px;font:900 10px Arial;color:#28445e}.v489RowFitBox input[type=number]{width:100%;box-sizing:border-box;border:1px solid #b8c7d6;border-radius:8px;padding:7px;text-align:center;font-weight:900}.v489RowFitCheck{grid-template-columns:auto 1fr!important;justify-content:start}.v489RowFitCheck input{width:18px;height:18px;accent-color:#087a55}.v489GeoGrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px}.v489GeoGrid .v440Field{margin:3px 0}.v489MultiActions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px}.v489MultiActions .v440Btn{min-height:38px;padding:7px 6px;font-size:10.5px}.v489SaveBar{display:grid;grid-template-columns:1fr 1fr;gap:7px}.v489SaveBar .v440Btn{min-height:42px}.v450TestValue,.v462TestStatic{overflow-wrap:anywhere;word-break:break-word}.v440RuntimeField textarea{resize:none;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word}.v489MobileTabs{display:none}
@media(max-width:1050px){.v440FmBody{grid-template-columns:210px minmax(0,1fr) 320px}.v440FmHead .v440Btn{font-size:10.5px}}
@media(max-width:900px){#v440Fm{padding:0}#v440FmCard{width:100vw;height:100dvh;border-radius:0}.v440FmHead{padding:7px 8px;max-height:92px;overflow-x:auto;overflow-y:hidden;flex-wrap:nowrap}.v440FmHead>b{min-width:max-content}.v440FmHead .grow{display:none}.v440FmHead .v440Btn{flex:0 0 auto;min-height:34px}.v489MobileTabs{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;padding:6px 7px;background:#e9eef4;border-bottom:1px solid #d4dde6}.v489MobileTabs button{border:1px solid #cbd5e1;border-radius:9px;background:#fff;color:#39546c;min-height:38px;font-weight:900}.v489MobileTabs button.active{background:#0b67b2;border-color:#0b67b2;color:#fff}.v440FmBody{display:block;padding:6px;overflow:hidden}.v440Pane{display:none;height:calc(100dvh - 145px);max-height:none!important;overflow:auto}.v440Pane.v489MobileActive{display:block}.v440Pane.center{padding:6px}.v440EditorViewport{height:calc(100dvh - 230px);min-height:300px}.v451ZoomTools{position:static;display:flex;overflow-x:auto;flex-wrap:nowrap;width:100%;box-sizing:border-box}.v451ZoomTools input[type=range]{min-width:100px}.v440Pane.right{padding-bottom:70px}.v489GeoGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:560px){.v440FmHead>b{font-size:12px}.v440FmHead .v440Btn{padding:6px 8px}.v440Pane{height:calc(100dvh - 137px)}.v440EditorViewport{height:calc(100dvh - 220px)}.v487ControlGrid2,.v487ControlGrid4{grid-template-columns:1fr 1fr}.v461TextAlignRow{grid-template-columns:52px 1fr}.v487AlignGrid{gap:5px}.v461AlignBtn{min-height:38px;padding:6px 3px;font-size:10.5px}.v489MultiActions{grid-template-columns:1fr 1fr}}
`;document.head.appendChild(st)}
function setMobilePaneV489(name){const fm=$('v440Fm');if(!fm)return;fm.dataset.v489Pane=name||'center';document.querySelectorAll('#v440Fm .v440Pane').forEach(x=>x.classList.remove('v489MobileActive'));const target=name==='left'?fm.querySelector('.v440Pane.left'):name==='right'?fm.querySelector('.v440Pane.right'):fm.querySelector('.v440Pane.center');target?.classList.add('v489MobileActive');document.querySelectorAll('#v440Fm [data-v489-pane]').forEach(b=>b.classList.toggle('active',b.dataset.v489Pane===(name||'center')))}

function ensureUi(){ensureStyle();ensureUiPolishV489();if($('v440Fm'))return;const m=document.createElement('div');m.id='v440Fm';m.innerHTML=`<div id="v440FmCard"><div class="v440FmHead"><b>🧩 FORM MANAGER · NGUỒN LAYOUT DUY NHẤT</b><span id="v450Perf" class="v450Perf"></span><span class="grow"></span><button class="v440Btn" id="v450Test">👁 TEST HIỂN THỊ</button><button class="v440Btn primary v627DirectBtn active" id="v627Direct" type="button">🎯 CHỈNH TRỰC TIẾP</button><button class="v440Btn v462TestModeBtn" id="v462TestMode" type="button">🔓 TEST · CHỈNH VỊ TRÍ</button><button class="v440Btn" id="v450PdfPreview">📄 PDF PREVIEW</button><button class="v440Btn" id="v460AiRecognize">🤖 AI NHẬN DẠNG</button><button class="v440Btn" id="v440Undo">↶ UNDO</button><button class="v440Btn" id="v440Redo">↷ REDO</button><button class="v440Btn" id="v487Validate">✓ KIỂM TRA</button><button class="v440Btn" id="v487Normalize">⚙ CHUẨN HÓA</button><button class="v440Btn green" id="v440Export">📦 XUẤT GÓI FORM</button><button class="v440Btn" id="v440ExportRegistry">⬇ forms.registry.json</button><button class="v440Btn green" id="v627ApplyExport">✓ ÁP DỤNG + XUẤT JSON</button><button class="v440Btn" id="v440Close">ĐÓNG</button></div><div class="v489MobileTabs"><button type="button" data-v489-pane="left">☰ FORM</button><button type="button" data-v489-pane="center" class="active">▣ TRANG</button><button type="button" data-v489-pane="right">⚙ THUỘC TÍNH</button></div><div class="v440FmBody"><div class="v440Pane left"><div class="v440Row"><button class="v440Btn primary" id="v440NewForm">+ FORM MỚI</button><button class="v440Btn" id="v440RefreshReg">↻</button></div><div id="v440FormList"></div></div><div class="v440Pane center v489MobileActive"><div class="v440Row" style="justify-content:space-between"><div id="v440PageTabs" class="v440PageTabs"></div><div class="v451ZoomTools"><span class="v453ZoomTitle">🔎 ZOOM FORM</span><button class="v440Btn" id="v451ZoomFit" type="button">VỪA KHUNG</button><button class="v440Btn" id="v451ZoomOut" type="button">−</button><span id="v451ZoomLabel" class="v451ZoomLabel">100%</span><input id="v451ZoomRange" type="range" min="50" max="400" step="25" value="100" aria-label="Zoom biểu mẫu"><button class="v440Btn" id="v451ZoomIn" type="button">+</button><button class="v440Btn" id="v440AddPage">+ TRANG</button><button class="v440Btn danger" id="v440DelPage">XÓA TRANG</button></div></div><div id="v440Editor" class="v440Empty">Chọn biểu mẫu.</div></div><div class="v440Pane right"><div id="v440Inspector"></div><div class="v440Status" id="v440Status"></div></div></div></div>`;document.body.appendChild(m);document.querySelectorAll('#v440Fm [data-v489-pane]').forEach(b=>b.onclick=()=>setMobilePaneV489(b.dataset.v489Pane));setMobilePaneV489('center');
$('v440Close').onclick=close;$('v450Test').onclick=toggleTestV45;$('v627Direct').onclick=toggleDirectV627;$('v462TestMode').onclick=toggleTestSubModeV462;$('v450PdfPreview').onclick=previewPdfV45;$('v460AiRecognize').onclick=aiOpenV460;$('v440NewForm').onclick=createForm;$('v440RefreshReg').onclick=async()=>{published={schema:1,forms:[]};await loadPublished(true);mergeRegistry();renderAll()};$('v440Undo').onclick=undo;$('v440Redo').onclick=redo;$('v487Validate').onclick=validateCurrentFormV487;$('v487Normalize').onclick=normalizeCurrentFormV487;$('v440AddPage').onclick=addPage;$('v440DelPage').onclick=deletePage;$('v440Export').onclick=exportCurrent;$('v440ExportRegistry').onclick=exportRegistry;$('v627ApplyExport').onclick=applyAndExportV627;$('v451ZoomFit').onclick=()=>setEditorZoom(1);$('v451ZoomOut').onclick=()=>setEditorZoom(editorZoom-.25);$('v451ZoomIn').onclick=()=>setEditorZoom(editorZoom+.25);$('v451ZoomRange').oninput=e=>setEditorZoom(Number(e.target.value)/100);renderZoomUi();if(!root.__SAGS_V462_KEYBOARD_NUDGE){root.__SAGS_V462_KEYBOARD_NUDGE=true;document.addEventListener('keydown',e=>{const fm=$('v440Fm');if(!fm?.classList.contains('show'))return;const tag=String(e.target?.tagName||'').toUpperCase();if(['INPUT','TEXTAREA','SELECT','BUTTON'].includes(tag)||e.target?.isContentEditable)return;if(testMode&&!testEditMode)return;const map={ArrowLeft:[-1,0],ArrowRight:[1,0],ArrowUp:[0,-1],ArrowDown:[0,1]},d=map[e.key];if(!d||!selectedFields().length)return;e.preventDefault();const mul=e.shiftKey?5:1;nudge(d[0]*mul,d[1]*mul)},{capture:true})}}
function status(t,err=false){const e=$('v440Status');if(e){e.textContent=String(t||'');e.style.color=err?'#b42318':'#067647'}}
function open(){if(!isAD())return alert('Chỉ AD được phép quản lý biểu mẫu.');directEditV627=true;testMode=false;ensureUi();loadPublished().then(()=>{mergeRegistry();if(!currentId)currentId=draft.forms?.[0]?.id||'';$('v440Fm').classList.add('show');renderAll();status('🎯 CHỈNH TRỰC TIẾP đang bật · kéo field/cạnh/góc ngay trên ảnh form · snap 1 px · xuất forms.registry.json khi hoàn tất.')})}
...
function renderAll(){renderList();renderPages();renderEditor();renderInspector();$('v440Undo').disabled=!history.length;$('v440Redo').disabled=!future.length;if($('v450Test'))$('v450Test').textContent=testMode?'✕ THOÁT TEST':'👁 TEST HIỂN THỊ';renderTestModeUiV462();renderDirectModeUiV627();renderPerfV45();renderZoomUi()}
...
function rowCalibrationTextV628(fld){const rows=Math.max(1,Math.min(9,Math.round(Number(fld?.rowCount)||1)));return Array.from({length:rows},(_,i)=>(i+1)+' · CÂU NÀY ĐỂ CĂN DÒNG').join('\n')}
function testValueV45(fld){const key=fld.bind||fld.key;if(Object.prototype.hasOwnProperty.call(testValuesV462,key))return testValuesV462[key];const st=liveStateV45(),v=st?.[key];if(v!==undefined&&v!==null&&String(v)!=='')return v;if(fld.type==='checkbox')return true;if(fld.type==='date')return '2026-09-16';if(fld.type==='time')return '08:25';if(fld.type==='number')return '123';if(fld.type==='signature')return '';return fld.type==='textarea'?rowCalibrationTextV628(fld):'TEST'}
function testHtmlV45(fld){fmNormalizeFieldV487(fld);const v=testValueV45(fld),st=fmStyleV487(fld),al=st.align,va=st.valign,just=al==='center'?'center':al==='right'?'flex-end':'flex-start',items=va==='top'?'flex-start':va==='bottom'?'flex-end':'center',multi=fmIsMultiV489(fld),txt=esc(fld.type==='static-text'?(fld.text||fld.label||''):v);if(fld.type==='signature'){if(typeof v==='string'&&v.startsWith('data:image/'))return `<div class="v450TestValue" style="justify-content:center;align-items:center"><img src="${esc(v)}" style="max-width:100%;max-height:100%;object-fit:contain"></div>`;return '<div class="v450TestValue" style="justify-content:center;align-items:center;color:#64748b">CHỮ KÝ</div>'}if(fld.type==='checkbox')return `<div class="v450TestValue" style="justify-content:center;align-items:center;font-size:calc(${st.fontSize}px * var(--v451-page-scale,1))">${v?'✓':''}</div>`;if(multi){const vj=va==='bottom'?'flex-end':va==='middle'?'center':'flex-start';return `<div class="v450TestValue" style="display:flex;flex-direction:column;justify-content:${vj};align-items:stretch;padding:0 2px;${fmCssV487(fld,true)};white-space:normal;overflow:hidden"><div style="width:100%;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;text-align:${al}">${txt}</div></div>`}return `<div class="v450TestValue" style="justify-content:${just};align-items:${items};${fmCssV487(fld,true)};white-space:nowrap">${txt}</div>`}
function testControlHtmlV462(fld){fmNormalizeFieldV487(fld);const v=testValueV45(fld),key=esc(fld.bind||fld.key),common=fmCssV487(fld,true);if(fld.type==='checkbox')return `<input class="v462TestCheck" data-v462-test-bind="${key}" type="checkbox" ${v?'checked':''}>`;if(fld.type==='textarea')return `<textarea class="v462TestControl" data-v462-test-bind="${key}" wrap="soft" style="${common};padding:0 2px;border:0;background:transparent;resize:none;overflow:hidden;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word">${esc(v??'')}</textarea>`;if(fld.type==='select'){const opts=Array.isArray(fld.options)&&fld.options.length?fld.options:[v||'TEST','A','B'];return `<select class="v462TestControl" data-v462-test-bind="${key}" style="${common}">${opts.map(x=>`<option ${String(x)===String(v)?'selected':''}>${esc(x)}</option>`).join('')}</select>`}if(fld.type==='signature')return `<div class="v462TestStatic" style="align-items:center;justify-content:center;color:#64748b">${typeof v==='string'&&v.startsWith('data:image/')?`<img src="${esc(v)}" style="max-width:100%;max-height:100%;object-fit:contain">`:'CHỮ KÝ'}</div>`;if(fld.type==='computed'||fld.type==='static-text')return `<div class="v462TestStatic" style="align-items:center;justify-content:${fld.align==='center'?'center':fld.align==='right'?'flex-end':'flex-start'};${common}">${esc(fld.type==='static-text'?(fld.text||fld.label||v):v)}</div>`;const typ=['number','time','date'].includes(fld.type)?fld.type:'text';return `<input class="v462TestControl" data-v462-test-bind="${key}" type="${typ}" value="${esc(v??'')}" style="${common}">`}
...
function previewPdfV45(){const f=form(),p=page();if(!f||!p)return;return previewCustomPageV45(f,p)}
function fmWrapCanvasV487(ctx,text,maxWidth){const out=[],fitToken=tok=>{const parts=[];let cur='';for(const ch of String(tok||'')){const t=cur+ch;if(cur&&ctx.measureText(t).width>maxWidth){parts.push(cur);cur=ch}else cur=t}if(cur)parts.push(cur);return parts.length?parts:['']};for(const para of String(text??'').replace(/\r\n/g,'\n').split('\n')){if(!para){out.push('');continue}const words=para.split(/\s+/).filter(Boolean);let line='';for(const word of words){let chunks=ctx.measureText(word).width>maxWidth?fitToken(word):[word];for(const w of chunks){const test=line?line+' '+w:w;if(!line||ctx.measureText(test).width<=maxWidth)line=test;else{out.push(line);line=w}}}if(line||!out.length)out.push(line)}return out}
// V6.1.19 unified render standard:
// - A printed row may absorb up to ~33% extra natural text width by horizontal fitting.
// - Multiline row count remains the hard safety limit.
// - Row-fit text sits 4px above each printed/dotted rule.
const SAGS_V6119_SOFT_FIT_RATIO=.75,SAGS_V6119_RULE_GAP=4;
function fmV6119SoftWidth(width){return Math.max(8,Number(width)||8)/SAGS_V6119_SOFT_FIT_RATIO}
function fmV6119Measure(ctx,text,letterSpacing=0){
  const raw=String(text??''),m=ctx.measureText(raw);
  return Math.max(0,Number(m.width)||0)+(('letterSpacing' in ctx)?0:Math.max(0,Number(letterSpacing)||0)*Math.max(0,raw.length-1));
}
function fmV6119FitLine(ctx,text,st,width){
  const available=Math.max(8,Number(width)||8),preferred=Math.max(6,Number(st.fontSize)||14);
  const setFont=fs=>{ctx.font=`${st.fontStyle} ${st.fontWeight} ${fs}px '${st.fontFamily}'`;};
  let fontSize=preferred;setFont(fontSize);
  let natural=fmV6119Measure(ctx,text,st.letterSpacing),ratio=natural>0?available/natural:1;
  // Keep the published font first. Only when horizontal compression would be
  // stronger than 75% do we lower the font by at most 1px, then fit horizontally.
  if(ratio<SAGS_V6119_SOFT_FIT_RATIO&&fontSize>8){
    fontSize=Math.max(8,fontSize-1);setFont(fontSize);
    natural=fmV6119Measure(ctx,text,st.letterSpacing);ratio=natural>0?available/natural:1;
  }
  return {fontSize,natural,scaleX:Math.min(1,ratio),available};
}
function fmV6119DrawLine(ctx,text,tx,baseline,st,width){
  const fit=fmV6119FitLine(ctx,text,st,width),line=String(text??'');
  ctx.font=`${st.fontStyle} ${st.fontWeight} ${fit.fontSize}px '${st.fontFamily}'`;
  if(line)ctx.fillText(line,tx,baseline,fit.natural>fit.available?fit.available:undefined);
  if(st.underline&&line){
    const drawn=Math.min(fit.natural,fit.available),ux=st.align==='right'?tx-drawn:st.align==='center'?tx-drawn/2:tx;
    ctx.fillRect(ux,baseline+2,drawn,Math.max(1,fit.fontSize/16));
  }
  return fit;
}
function fmV6119LegacyField(pageNo,key){
  try{return globalFieldsV45().find(g=>Number(g?.page)===Number(pageNo)&&String(g?.key)===String(key))||null}catch(_){return null}
}
function fmV6119Baseline(fld,st,pageNo,py,phField,visible,lh,index){
  if(fmIsMultiV489(fld)&&fld.rowFit&&String(fld.rowBaselineMode||'')==='box'){
    const rows=Math.max(1,Math.round(Number(fld.rowCount)||1)),rowH=phField/rows;
    return py+(Math.max(0,index)+1)*rowH-SAGS_V6119_RULE_GAP;
  }
  const legacy=fmV6119LegacyField(pageNo,fld.key);
  if(legacy){
    const dots=Array.isArray(legacy.dotRects)?legacy.dotRects:[];
    // V6.2.1: when the official background has measured dotted writing rows,
    // those physical rows are the baseline authority for BOTH live and PDF.
    // Do this before rowFit so 5/6-row forms do not get compressed into an
    // inferred 3/4-row box or vertically shifted between printed guides.
    if(dots.length&&Number(fld.h)>0){
      const r=dots[Math.min(Math.max(0,Number(index)||0),dots.length-1)];
      const pageH=phField/Number(fld.h),y=(Number(r?.[1])+Number(r?.[3]))*pageH;
      if(Number.isFinite(y))return y-SAGS_V6119_RULE_GAP;
    }
  }
  if(fmIsMultiV489(fld)&&fld.rowFit){
    const rows=Math.max(1,Math.round(Number(fld.rowCount)||1)),rowH=phField/rows;
    return py+(Math.max(0,index)+1)*rowH-SAGS_V6119_RULE_GAP;
  }
  if(legacy){
    if(!fmIsMultiV489(fld)&&(legacy.writeOnLine||legacy.aboveDots)){
      const r=Number(legacy.lineRatio);
      return Number.isFinite(r)&&r>0&&r<=1?py+phField*r-SAGS_V6119_RULE_GAP:py+phField-SAGS_V6119_RULE_GAP;
    }
  }
  return fmCanvasBaselineV490(st,py,phField,visible,lh)+Math.max(0,index)*lh;
}

// V6.1.13: the operator's multiline editor must count EXACTLY the rows that
// the published Form Manager PDF painter will use, not a hardcoded 5-row modal.
// Return copies of published metadata only; never expose or change the AD draft.
function sagsV6113PublishedEntry(pageNo,key){
  const wanted=Number(pageNo),name=String(key||'');
  for(const form of published.forms||[]){
    if(!form.legacy)continue;
    const pg=(form.pages||[]).find(p=>Number(p.sourcePage||pageNoV45(p))===wanted);
    if(!pg)continue;
    const raw=(form.fields||[]).find(f=>String(f.pageId)===String(pg.id)&&
      (String(f.bind||f.key)===name||String(f.key)===name));
    if(!raw||!fmIsMultiV489(raw)||Number(raw.w)<=.005||Number(raw.h)<=.005)return null;
    const fld=fmNormalizeFieldV487({...raw}),st=fmStyleV487(fld);
    const width=Number(pg.width)*Number(fld.w),height=Number(pg.height)*Number(fld.h);
    if(!(width>4&&height>0))return null;
    if(fld.rowFit)st.lineHeight=fmClampV487(height/(Math.max(1,Number(fld.rowCount)||1)*st.fontSize),.8,4,1.5);
    const rows=fld.rowFit?Math.max(1,Math.round(Number(fld.rowCount)||1)):
      Math.max(1,Math.floor(Math.max(st.fontSize*st.lineHeight,height-4)/Math.max(1,st.fontSize*st.lineHeight)));
    return {key:name,pageNo:wanted,formId:String(form.id),fieldKey:String(fld.key),
      rowCount:rows,boxWidth:width,boxHeight:height,width:Math.max(8,width-4),
      fontSize:st.fontSize,fontFamily:st.fontFamily,fontWeight:st.fontWeight,
      fontStyle:st.fontStyle,lineHeight:st.fontSize*st.lineHeight,
      letterSpacing:st.letterSpacing,color:st.textColor,align:st.align,
      top:st.valign==='top',isLmc:String(form.id)==='fsags421'&&String(fld.key)==='f421_lmcDetails'};
  }
  return null;
}
root.sagsV6113EntryGuide=(pageNo,key)=>sagsV6113PublishedEntry(pageNo,key);
root.sagsV6113EntryReady=()=>loadPublished();
let sagsV6115EntryMeasureCtx=null;
root.sagsV6113CheckEntry=(pageNo,key,value,entryGuide)=>{
  // Only a guide derived from the published layout for this exact open field is reusable.
  const guide=entryGuide?.pageNo===Number(pageNo)&&entryGuide?.key===String(key) ? entryGuide : sagsV6113PublishedEntry(pageNo,key);
  if(!guide)return null;
  const raw=String(value??'').replace(/\r\n?/g,'\n');
  // The PDF omits only trailing blank LMC lines (V6.1.12). The editor still
  // counts a new empty row after Enter so it never offers a fifth input row.
  const printable=guide.isLmc?raw.replace(/(?:[ \t]*\n)+[ \t]*$/,''):raw;
  // Reuse the measurement canvas; a new canvas per keystroke allocates memory.
  const ctx=sagsV6115EntryMeasureCtx||(sagsV6115EntryMeasureCtx=document.createElement('canvas').getContext('2d'));
  if(!ctx)return {guide,valid:false,rows:0,printedRows:0,reason:'Không đo được chữ trên thiết bị'};
  ctx.font=`${guide.fontStyle} ${guide.fontWeight} ${guide.fontSize}px '${guide.fontFamily}'`;
  if('letterSpacing' in ctx)ctx.letterSpacing=guide.letterSpacing+'px';
  // Match PDF soft-fit width: a row may be naturally wider than the box and is
  // rendered with horizontal glyph fitting instead of being rejected immediately.
  const softWidth=fmV6119SoftWidth(guide.width);
  const lines=fmWrapCanvasV487(ctx,printable,softWidth);
  const physical=raw===''?0:raw.split('\n').length;
  const printed=printable===''?0:lines.length;
  const rows=Math.max(physical,printed);
  const ratios=lines.filter(Boolean).map(line=>Math.min(1,guide.width/Math.max(1,fmV6119Measure(ctx,line,guide.letterSpacing))));
  const fitRatio=ratios.length?Math.min(...ratios):1;
  // Number of printable rows remains the hard guard. Width alone is not a
  // failure anymore because PDF/live render use the same non-clipping fit.
  const valid=rows<=guide.rowCount;
  return {guide,valid,rows,printedRows:printed,physicalRows:physical,fitRatio,
    softFit:fitRatio<1,reason:'Nội dung vượt số hàng của ô'};
};
// NEW UI V1: the SAME Form Manager field painter is used by AD PDF preview and
// operator PDF export. No legacy dot-line / 23pt / device layout corrections apply.
function fmPaintCanonicalPageV495(ctx, f, p, valueFor){
  if(!ctx||!f||!p)throw new Error('Thiếu Form Manager page.');
  const pw=Number(p.width),ph=Number(p.height);
  if(!(pw>0&&ph>0))throw new Error('Kích thước trang Form Manager không hợp lệ.');
  ctx.save();
  try{
    ctx.scale(ctx.canvas.width/pw,ctx.canvas.height/ph);
    for(const raw of (f.fields||[]).filter(z=>String(z.pageId)===String(p.id))){
      // Do not modify the published registry or a draft while rendering.
      const fld=fmNormalizeFieldV487({...raw});
      // FSAGS 42.1 duplicate comment was disabled by the existing live UI.
      // Its 0.5%-sized placeholder cannot be treated as a printable field.
      // Never mutate saved state; meaningful older second comments use the real box.
      const is421Comments=String(f.id)==='fsags421' && Number(p.sourcePage||pageNoV45(p))===7;
      const retiredComment=is421Comments && String(fld.key)==='f421_continuation2' &&
        Number(fld.x)===0 && Number(fld.y)===0 && Number(fld.w)<=.005 && Number(fld.h)<=.005;
      if(retiredComment)continue;
      const v0=valueFor(fld);
      let v=fld.type==='static-text'?(fld.text||fld.label||''):v0;
      if(is421Comments && String(fld.key)==='f421_continuation'){
        const oldSecond=(f.fields||[]).find(z=>String(z.key)==='f421_continuation2' && Number(z.x)===0 && Number(z.y)===0 && Number(z.w)<=.005 && Number(z.h)<=.005);
        const extra=oldSecond?String(valueFor(oldSecond)??'').trim():'';
        // N/A in the retired field is an invisible default, not a second comment.
        if(extra && !/^(?:(?:N\/A|NA|—|-|\.)\s*)+$/i.test(extra) && !String(v??'').includes(extra))
          v=[String(v??'').trim(),extra].filter(Boolean).join('\n');
        fld.rowFit=true;fld.rowCount=6; // The page already has six dotted rows.
      }
      const px=Number(fld.x)*pw,py=Number(fld.y)*ph,pwField=Number(fld.w)*pw,phField=Number(fld.h)*ph;
      if(!(Number.isFinite(px)&&Number.isFinite(py)&&pwField>0&&phField>0))throw new Error('Tọa độ Form Manager không hợp lệ: '+fld.key);
      const st=fmStyleV487(fld);
      // Row fit is based on THIS field's page, not whichever page AD last edited.
      if(fld.rowFit&&fmIsMultiV489(fld))st.lineHeight=fmClampV487(phField/(Math.max(1,Number(fld.rowCount)||1)*st.fontSize),.8,4,1.5);
      if(fld.type==='checkbox'){
        if(v){ctx.save();ctx.font=`900 ${Math.max(10,st.fontSize)}px Arial`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle='#111';ctx.fillText('✓',px+pwField/2,py+phField/2);ctx.restore()}
        continue;
      }
      if(fld.type==='signature')continue; // Existing protected image/signature path.
      if(v===undefined||v===null||v==='')continue;
      // FSAGS 42.1 preview and PDF use the same published typography.
      // V6.1.19 fits width without clipping; row count/vertical geometry remain strict.
      const strict421=String(f.id)==='fsags421' && (Number(p.sourcePage||pageNoV45(p))===6 || Number(p.sourcePage||pageNoV45(p))===7);
      // BBBT Flight No.: restore original two-line layout in full Form Manager cell.
      if(String(fld.key)==='bbbtFlight' && Number(p.sourcePage||pageNoV45(p))===4){
        const parts=String(v).split(/[\/\r\n]+/).map(s=>s.trim()).filter(Boolean);
        if(parts.length>2)throw new Error('BBBT Flight No. có hơn 2 số hiệu. Kiểm tra trước khi xuất PDF.');
        if(parts.length){
          ctx.save();try{
            ctx.beginPath();ctx.rect(px,py,pwField,phField);ctx.clip();
            ctx.font=`${st.fontStyle} ${st.fontWeight} ${st.fontSize}px '${st.fontFamily}'`;
            ctx.fillStyle=st.textColor;ctx.textAlign='left';ctx.textBaseline='alphabetic';
            const tx=px+2,available=Math.max(8,pwField-4);
            const longest=Math.max(...parts.map(s=>ctx.measureText(s).width));
            const fs=st.fontSize*Math.min(1,available/Math.max(1,longest));
            if(fs<9.5)throw new Error('Số hiệu chuyến bay BBBT quá dài để hiển thị rõ trong ô.');
            ctx.font=`${st.fontStyle} ${st.fontWeight} ${fs}px '${st.fontFamily}'`;
            if(parts.length===2){
              ctx.fillText(parts[0],tx,py+phField*.39);
              ctx.fillText(parts[1],tx,py+phField*.82);
              const width=Math.min(available,Math.max(34,Math.max(...parts.map(s=>ctx.measureText(s).width))+6));
              ctx.save();ctx.strokeStyle='#111';ctx.lineWidth=1.4;ctx.beginPath();
              ctx.moveTo(tx,py+phField*.52);ctx.lineTo(tx+width,py+phField*.52);ctx.stroke();ctx.restore();
            }else{ctx.fillText(parts[0],tx,py+(phField+fs*.70)/2)}
          }finally{ctx.restore()}
        }
        continue;
      }
      ...
      try{
        ctx.beginPath();ctx.rect(px,py,pwField,phField);ctx.clip();
        ctx.font=`${st.fontStyle} ${st.fontWeight} ${st.fontSize}px '${st.fontFamily}'`;
        if('letterSpacing' in ctx)ctx.letterSpacing=st.letterSpacing+'px';
        ctx.fillStyle=st.textColor;
        ctx.textAlign=st.align==='right'?'right':st.align==='center'?'center':'left';
        ctx.textBaseline='alphabetic';
        const tx=st.align==='right'?px+pwField-2:st.align==='center'?px+pwField/2:px+2;
        const lh=st.fontSize*st.lineHeight;
        // Blank lines at the very end of the LMC textarea are not printable ink.
        // Keep the saved value unchanged, preserve intentional interior blank rows,
        // and still reject a fifth row with any actual text.
        const rawText=String(v);
        const layoutText=(strict421&&String(fld.key)==='f421_lmcDetails')?
          rawText.replace(/\r\n?/g,'\n').replace(/(?:[ \t]*\n)+[ \t]*$/,''):rawText;
        const available=Math.max(8,pwField-4);
        const lines=(fmIsMultiV489(fld)||layoutText.includes('\n'))?
          fmWrapCanvasV487(ctx,layoutText,fmV6119SoftWidth(available)):[layoutText];
        const max=fmIsMultiV489(fld)?
          (fld.rowFit?Math.max(1,Math.round(Number(fld.rowCount)||1)):Math.max(1,Math.floor(Math.max(lh,phField-4)/Math.max(1,lh)))):
          Math.max(1,Math.floor(phField/Math.max(1,lh)));
        const visible=Math.min(lines.length,max);
        if(strict421){
          const explain=reason=>{throw new Error('FSAGS 42.1 · Trang '+Number(p.sourcePage||pageNoV45(p))+' · '+String(fld.label||fld.key)+' ('+String(fld.key)+'): '+reason+'. Dữ liệu đã nhập vẫn được giữ.');};
          // Width is no longer a rejection: each printable row is fitted like
          // the approved VJ5062-style SVG. Only a real extra row/vertical escape fails.
          if(lines.length>max)explain('Nội dung vượt số dòng của ô; hãy mở rộng ô trong Form Manager hoặc rút ngắn nội dung');
          for(let i=0;i<lines.length;i++){
            const baseline=fmV6119Baseline(fld,st,Number(p.sourcePage||pageNoV45(p)),py,phField,visible,lh,i);
            const metrics=ctx.measureText(lines[i]);
            const ascent=Number.isFinite(metrics.actualBoundingBoxAscent)?metrics.actualBoundingBoxAscent:st.fontSize*.85;
            const descent=Number.isFinite(metrics.actualBoundingBoxDescent)?metrics.actualBoundingBoxDescent:st.fontSize*.22;
            if(baseline-ascent<py-.5||baseline+descent>py+phField+.5)
              explain('Chữ vượt chiều cao ô; không được cắt dòng trong PDF');
          }
        }
        lines.slice(0,max).forEach((line,i)=>{
          const baseline=fmV6119Baseline(fld,st,Number(p.sourcePage||pageNoV45(p)),py,phField,visible,lh,i);
          fmV6119DrawLine(ctx,line,tx,baseline,st,available);
        });
      }finally{ctx.restore()}
    }
  }finally{ctx.restore()}
}
function fmManagedPageV495(pageNo){
  // Operator PDF always uses the published shared registry, never an AD-only local draft.
  // V6.3.1: a published CUSTOM form may opt into the canonical runtime/PDF path by
  // declaring page.runtimePage. This keeps its original PDF sourcePage intact.
  const registry=published;
  for(const f of registry?.forms||[]){
    for(const p of f.pages||[]){
      const runtimePage=Number(p?.runtimePage||0);
      if(!f.legacy && !runtimePage)continue;
      const logicalPage=runtimePage||Number(p.sourcePage||pageNoV45(p));
      if(Number(logicalPage)===Number(pageNo))return {form:f,page:p};
    }
  }
  return null;
}
root.sagsV495ManagedPage=pageNo=>!!fmManagedPageV495(pageNo);
root.sagsV495PageInfo=pageNo=>{
  const entry=fmManagedPageV495(pageNo);
  return entry?{
    image:String(entry.page.image||''),blobKey:!!entry.page.blobKey,
    width:Number(entry.page.width),height:Number(entry.page.height)
  }:null;
};
root.sagsV495PageImage=async pageNo=>{
  const entry=fmManagedPageV495(pageNo);
  if(!entry)throw new Error('Thiếu ảnh nền Form Manager trang '+pageNo);
  const source=await pageSrc(entry.page);
  if(!source)throw new Error('Ảnh nền Form Manager trang '+pageNo+' chưa được xuất bản.');
  return source;
};
root.sagsV495PaintCanonicalPage=(ctx,pageNo)=>{
  const entry=fmManagedPageV495(pageNo);
  if(!entry)throw new Error('Chưa có bố cục Form Manager cho trang '+pageNo+'. Không xuất PDF sai tọa độ.');
  const data=liveStateV45();
  fmPaintCanonicalPageV495(ctx,entry.form,entry.page,fld=>{
    const bind=String(fld.bind||fld.key),key=String(fld.key||'');
    return data[bind]!==undefined?data[bind]:data[key];
  });
  return true;
};
// A first visit or newly activated PWA can have an uncontrolled document even
// when a fully staged and activated worker exists. Verify that worker directly;
// never accept a waiting/installing worker, another scope or an unknown script.
async function sagsV6111PdfVerifiedWorker(){
  const sw=navigator.serviceWorker;
  if(!sw)throw new Error('Trình duyệt chưa hỗ trợ xác minh Service Worker. Lưu nháp và dùng trình duyệt hỗ trợ để xuất PDF.');
  if(sw.controller)return sw.controller;
  const scope=new URL('./',location.href);
  let reg=null;
  try{reg=await sw.getRegistration(scope.href)}catch(_e){}
  if(!reg?.active||reg.active.state!=='activated'){
    let timer;
    try{reg=await Promise.race([sw.ready.catch(()=>null),new Promise(resolve=>{timer=setTimeout(()=>resolve(null),7000)})]);}
    finally{clearTimeout(timer)}
  }
  const active=reg?.active;
  let valid=false;
  try{const script=new URL(active?.scriptURL||'');valid=!!active&&active.state==='activated'&&
    new URL(reg.scope).href===scope.href&&script.origin===scope.origin&&
    script.pathname===new URL('./service-worker.js',scope.href).pathname;}
  catch(_e){}
  if(!valid)throw new Error('Service Worker chưa kích hoạt đúng ứng dụng. Lưu nháp, kiểm tra mạng rồi mở lại trang; không xóa dữ liệu.');
  return active;
}
function fmLayoutPayloadV626(reg){
  const forms=Array.isArray(reg?.forms)?reg.forms.map(f=>({
    id:String(f?.id||''),code:String(f?.code||''),legacy:!!f?.legacy,group:String(f?.group||''),
    pages:Array.isArray(f?.pages)?f.pages:[],fields:Array.isArray(f?.fields)?f.fields:[],engine:String(f?.engine||'')
  })):[];
  return JSON.stringify({schema:Number(reg?.schema||0),forms});
}
async function fmLayoutShaV626(reg){
  const bytes=new TextEncoder().encode(fmLayoutPayloadV626(reg));
  const digest=await crypto.subtle.digest('SHA-256',bytes);
  return [...new Uint8Array(digest)].map(n=>n.toString(16).padStart(2,'0')).join('');
}
root.sagsV495BeforeExport=async()=>{
  // Verify the active, pinned release rather than the legacy runtime's V5 executable ID.
  // A newer app version does not block PDF if the canonical Form Manager registry is identical.
  ...
  if(verified?.ready!==true)
    throw new Error('Bản đang chạy chưa xác minh đủ tệp và Form Manager. Giữ nguyên dữ liệu và báo quản trị viên.');
  // Build numbers may differ briefly during a verified app update. Do not block PDF solely
  // for that mismatch: the canonical Form Manager SHA-256 is compared with the live server below.
  // Strict fetches must be network-only; a cached manifest can conceal a newly edited form.
  const [remote,manifestResponse]=await Promise.all([
    fetch('./version.json?__sags_strict=1&t='+Date.now(),{cache:'no-store'}).catch(()=>null),
    fetch('./asset-manifest.json?__sags_strict=1&t='+Date.now(),{cache:'no-store'}).catch(()=>null)
  ]);
  if(!remote?.ok)throw new Error('Không kiểm tra được bố cục Form Manager hiện hành. Dữ liệu vẫn được giữ; thử lại khi có mạng.');
  const remoteVersion=await remote.json();
  if(!remoteVersion?.build)throw new Error('Thông tin phiên bản trên máy chủ không hợp lệ.');
  if(!manifestResponse?.ok)throw new Error('Không kiểm tra được thông tin bố cục Form Manager trên máy chủ. Thử lại khi có mạng.');
  const latest=await manifestResponse.json();
  if(latest?.build!==remoteVersion.build||!latest?.assets?.['./index.html']?.sha256)
    throw new Error('Các tệp phát hành chưa đồng bộ trên máy chủ. Vui lòng thử lại sau; không tải lại khi đang nhập.');
  // V6.2.9: forms.registry.json is published mutable metadata, not an immutable
  // app-shell asset. Always refresh it from the live server before PDF export.
  await loadPublished(true);
  if(!Array.isArray(published?.forms)||!published.forms.length)
    throw new Error('Không tải được forms.registry.json hiện hành. PDF chưa được xuất; dữ liệu vẫn được giữ.');
  if(!draft?.forms?.length)mergeRegistry();
  syncLegacyToLive(false);
  const required=[1,2,4,6,7,9,10,11,12,13];
  for(const pageNo of required)if(!fmManagedPageV495(pageNo))
    throw new Error('Thiếu trang '+pageNo+' trong Form Manager; dừng xuất PDF.');
  // Live edits maintain SVG; fast export should not redraw every page.
  // Form Manager editor, old renderers and missing SVG keep the safe path.
...
async function previewCustomPageV45(f,p){
  ...
  try{
    const src=await pageSrc(p);
    if(!src)throw new Error('Trang chưa có ảnh nền.');
    const im=await new Promise((res,rej)=>{const q=new Image();q.onload=()=>res(q);q.onerror=rej;q.src=src});
    const c=document.createElement('canvas');c.width=Number(p.width||im.naturalWidth||1241);c.height=Number(p.height||im.naturalHeight||1755);
    const x=c.getContext('2d');x.drawImage(im,0,0,c.width,c.height);
    fmPaintCanonicalPageV495(x,f,p,testValueV45);
    showCanvasPreviewV45(c,`${f.name||f.id} · Trang ${pageIndex+1}`);
    status('✓ Preview và PDF xuất cùng sử dụng một hàm vẽ của Form Manager.');
  }catch(e){status('Không preview được: '+(e?.message||e),true)}
}
...
function directHandlesV627(){return ['nw','n','ne','e','se','s','sw','w'].map(d=>'<i class="v627Handle" data-resize="'+d+'" aria-hidden="true"></i>').join('')}
function directRowsV627(fld){
  if(!directEditV627||selectedKey!==fld.key||!fmIsMultiV489(fld)||!fld.rowFit)return '';
  const rows=Math.max(1,Math.min(30,Math.round(Number(fld.rowCount)||1))),p=page();
  if(rows<1||!p)return '';
  const W=Math.max(1,Number(fld.w||0)*Number(p.width||1241)),H=Math.max(1,Number(fld.h||0)*Number(p.height||1755));
  const py=Number(fld.y||0)*Number(p.height||1755),pageNo=Number(p.sourcePage||pageNoV45(p)||0),st=fmStyleV487({...fld}),lh=st.fontSize*st.lineHeight;
  const maxTest=Math.min(rows,9),parts=['<svg class="v628RowCal" viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="none" aria-hidden="true">'];
  for(let i=0;i<rows;i++){
    const baseAbs=fmV6119Baseline(fld,st,pageNo,py,H,rows,lh,i),base=Math.max(0,Math.min(H,baseAbs-py)),rule=Math.max(0,Math.min(H,base+SAGS_V6119_RULE_GAP));
    parts.push('<line class="rule" x1="0" y1="'+rule+'" x2="'+W+'" y2="'+rule+'"></line>');
    parts.push('<line class="base" x1="0" y1="'+base+'" x2="'+W+'" y2="'+base+'"></line>');
    if(i<maxTest){
      const label=(i+1)+' · CÂU NÀY ĐỂ CĂN DÒNG';
      parts.push('<text x="2" y="'+base+'" font-family="'+esc(st.fontFamily)+'" font-size="'+st.fontSize+'" font-weight="'+esc(st.fontWeight)+'" font-style="'+esc(st.fontStyle)+'" text-anchor="'+(st.align==='center'?'middle':st.align==='right'?'end':'start')+'">'+esc(label)+'</text>');
    }
  }
  parts.push('</svg>');
  return parts.join('');
}
function directGeoV627(fld){if(!directEditV627||selectedKey!==fld.key)return '';const p=page(),W=Math.max(1,Number(p?.width||1241)),H=Math.max(1,Number(p?.height||1755)),x=Math.round(Number(fld.x||0)*W),y=Math.round(Number(fld.y||0)*H),w=Math.round(Number(fld.w||0)*W),h=Math.round(Number(fld.h||0)*H);return '<small class="v627Geo">x:'+x+' y:'+y+' · '+w+'×'+h+'px</small>'}
function renderDirectModeUiV627(){const b=$('v627Direct');if(!b)return;b.classList.toggle('active',directEditV627);b.textContent=directEditV627?'🎯 TRỰC TIẾP · BẬT':'🎯 TRỰC TIẾP · TẮT';b.title=directEditV627?'Kéo field trực tiếp trên form. Kéo 8 điểm đỏ để resize. Snap 1 px; phím mũi tên 1 px, Shift = 5 px.':'Bật để chỉnh WYSIWYG trực tiếp trên ảnh biểu mẫu.'}
function toggleDirectV627(){directEditV627=!directEditV627;if(directEditV627&&testMode)testMode=false;drawBoxes();renderInspector();renderDirectModeUiV627();status(directEditV627?'🎯 CHỈNH TRỰC TIẾP: chạm field → kéo; kéo cạnh/góc đỏ để resize; mũi tên = 1 px, Shift = 5 px. Field nhiều dòng hiện guide số hàng.':'Đã tắt chỉnh trực tiếp. Bật lại để kéo/resize trên biểu mẫu.')}
async function applyAndExportV627(){if(!validateCurrentFormV487())return;try{fmNormalizeRegistryV487(draft);saveLocal();syncLegacyToLive(true);try{if(typeof draw==='function')draw()}catch(_e){}await exportRegistry();status('✓ Đã áp dụng layout nháp lên màn hình AD và xuất forms.registry.json. PDF PREVIEW dùng cùng layout này.')}catch(e){status('Không áp dụng/xuất được: '+(e?.message||e),true);alert('Không áp dụng/xuất được: '+(e?.message||e))}}
function drawBoxes(){const ov=$('v440Overlay');if(!ov)return;ov.classList.toggle('v450TestMode',testMode);ov.classList.toggle('v462TestEdit',testMode&&testEditMode);ov.classList.toggle('v462TestInteract',testMode&&!testEditMode);ov.classList.toggle('v627DirectOn',directEditV627&&!testMode);ov.innerHTML='';for(const fld of pageFields()){const b=document.createElement('div');b.className='v440Box'+(fld.key===selectedKey?' sel':'')+(selectedMany.has(fld.key)?' multi':'')+(fld.aiGenerated?' ai':'');b.dataset.key=fld.key;b.style.left=(Number(fld.x||0)*100)+'%';b.style.top=(Number(fld.y||0)*100)+'%';b.style.width=(Number(fld.w||.12)*100)+'%';b.style.height=(Number(fld.h||.035)*100)+'%';const label='<span>'+esc(fld.label||fld.key)+(fld.aiGenerated?' · AI '+Math.round(Number(fld.aiConfidence||0))+'%':'')+'</span>';const calibrating=directEditV627&&selectedKey===fld.key&&fmIsMultiV489(fld)&&fld.rowFit;const preview=testMode?(testEditMode?testHtmlV45(fld):testControlHtmlV462(fld)):(directEditV627&&!calibrating?testHtmlV45(fld):'');const handles=(directEditV627||testMode&&testEditMode)?directHandlesV627():'<i class="v440Resize" data-resize="se"></i>';b.innerHTML=label+preview+directRowsV627(fld)+directGeoV627(fld)+handles;ov.appendChild(b);bindBox(b,fld)}wireTestControlsV462(ov);for(const fld of aiPreviewV460){if(String(fld.pageId||'')!==String(page()?.id||''))continue;const b=document.createElement('div');b.className='v440Box ai-preview';b.style.left=(Number(fld.x||0)*100)+'%';b.style.top=(Number(fld.y||0)*100)+'%';b.style.width=(Number(fld.w||.12)*100)+'%';b.style.height=(Number(fld.h||.035)*100)+'%';b.innerHTML='<span>AI '+Math.round(Number(fld.aiConfidence||0))+'% · '+esc(fld.label||fld.aiText||fld.key)+'</span>';ov.appendChild(b)}}
function snap(v,axis){const p=page(),px=directEditV627?1:4,den=axis==='x'?Number(p?.width||1241):Number(p?.height||1755),step=Math.max(.0001,px/den);return Math.round(v/step)*step}
function clamp(v,a,b){return Math.max(a,Math.min(b,v))}
function bindBox(el,fld){el.onpointerdown=e=>{if(testMode&&!testEditMode)return;e.preventDefault();e.stopPropagation();if(e.shiftKey&&!e.target.dataset.resize){if(selectedMany.has(fld.key))selectedMany.delete(fld.key);else selectedMany.add(fld.key);selectedKey=fld.key;drawBoxes();renderInspector();return}selectedKey=fld.key;if(!selectedMany.has(fld.key)){selectedMany.clear();selectedMany.add(fld.key)};pushHistory();drawBoxes();renderInspector();const r=$('v440Overlay').getBoundingClientRect(),sx=e.clientX,sy=e.clientY,ox=Number(fld.x||0),oy=Number(fld.y||0),ow=Number(fld.w||.12),oh=Number(fld.h||.035),dir=String(e.target.dataset.resize||''),minW=Math.max(.005,8/Math.max(1,Number(page()?.width||1241))),minH=Math.max(.005,8/Math.max(1,Number(page()?.height||1755)));try{e.target.setPointerCapture?.(e.pointerId)}catch(_e){}const move=ev=>{const dx=(ev.clientX-sx)/r.width,dy=(ev.clientY-sy)/r.height;if(!dir){fld.x=clamp(snap(ox+dx,'x'),0,1-ow);fld.y=clamp(snap(oy+dy,'y'),0,1-oh)}else{let nx=ox,ny=oy,nw=ow,nh=oh;if(dir.includes('e'))nw=clamp(snap(ow+dx,'x'),minW,1-ox);if(dir.includes('s'))nh=clamp(snap(oh+dy,'y'),minH,1-oy);if(dir.includes('w')){nx=clamp(snap(ox+dx,'x'),0,ox+ow-minW);nw=ox+ow-nx}if(dir.includes('n')){ny=clamp(snap(oy+dy,'y'),0,oy+oh-minH);nh=oy+oh-ny}fld.x=nx;fld.y=ny;fld.w=nw;fld.h=nh;if(fmIsMultiV489(fld)&&fld.rowFit)fmApplyRowFitV490(fld)}drawBoxes();renderInspector()};const up=()=>{document.removeEventListener('pointermove',move);document.removeEventListener('pointerup',up);saveLocal();status('✓ '+fld.key+': x '+Math.round(Number(fld.x)*Number(page()?.width||1241))+' · y '+Math.round(Number(fld.y)*Number(page()?.height||1755))+' · '+Math.round(Number(fld.w)*Number(page()?.width||1241))+'×'+Math.round(Number(fld.h)*Number(page()?.height||1755))+' px')};document.addEventListener('pointermove',move);document.addEventListener('pointerup',up,{once:true})}}
...
function renderInspector(){
  const h=$('v440Inspector'),f=form(),p=page();if(!h)return;if(!f){h.innerHTML='';return}
  const fld=(f.fields||[]).find(x=>x.key===selectedKey),testHint=testMode?`<div class="v462TestHint ${testEditMode?'edit':'interact'}">${testEditMode?'🔓 LIVE TEST/EDIT: dữ liệu TEST đang hiển thị nhưng field vẫn kéo, resize và căn chữ được ngay.':'🔒 TEST TƯƠNG TÁC: vị trí field đang khóa; nhập/check/select chỉ thay dữ liệu test tạm.'}</div>`:'';
  const pagePanel=`<details class="v489InspectorSection" open><summary>🖼 TRANG · ${pageIndex+1}</summary><div class="v489InspectorBody"><div class="v440Field"><label>MÃ FORM</label><input id="v440FormId" value="${esc(f.id)}" ${f.legacy?'disabled':''}></div><div class="v440Field"><label>TÊN BIỂU MẪU</label><input id="v440FormName" value="${esc(f.name||'')}"></div><div class="v440Field"><label>MÃ HIỂN THỊ</label><input id="v440FormCode" value="${esc(f.code||'')}"></div><div class="v440Row"><button class="v440Btn primary" id="v440UploadBtn">TẢI ẢNH / PDF</button><button class="v440Btn" id="v440ExistingBtn">CHỌN ẢNH ĐÃ CÓ</button></div><input id="v440Upload" type="file" accept="image/png,image/jpeg,image/webp,application/pdf,.pdf" style="display:none"><select id="v440Existing" style="width:100%;padding:8px;border:1px solid #cbd5e1;border-radius:9px"></select><div class="v489TypeHint">${p?`${Number(p.width||0)}×${Number(p.height||0)} · ${esc(p.sha256?String(p.sha256).slice(0,12)+'…':'chưa hash')}`:'Chưa có trang'} · PDF nhiều trang sẽ tự tách thành ảnh nền.</div>${f.legacy?`<div class="v489TypeHint" style="background:#fff7ed;border-color:#fed7aa;color:#9a3412">FORM HỆ THỐNG: field mới/AI là bản nháp trên máy AD cho tới khi xuất forms.registry.json.</div>`:''}</div></details>`;
  if(!fld){h.innerHTML=`${testHint}${pagePanel}<details class="v489InspectorSection" open><summary>✏ FIELD</summary><div class="v489InspectorBody"><div class="v489FieldTitle"><b>Chưa chọn field</b><button class="v440Btn primary" id="v440AddField">+ THÊM FIELD</button></div><div class="v489TypeHint">Chạm field trên biểu mẫu để sửa. Shift+click để chọn nhiều field rồi căn hàng/cột.</div></div></details><div class="v489SaveBar"><button class="v440Btn green" id="v440Save">💾 LƯU NHÁP</button>${!f.legacy?'<button class="v440Btn danger" id="v440DeleteForm">XÓA FORM</button>':''}</div>`;wireInspector(f,p,null);return}
  fmNormalizeFieldV487(fld);const isText=['text','textarea'].includes(String(fld.type||'')),multi=fmIsMultiV489(fld),cap=fmLineCapacityV489(fld,p);
  const fieldPanel=`<details class="v489InspectorSection" open><summary>✏ FIELD · ${esc(fld.label||fld.key)}</summary><div class="v489InspectorBody"><div class="v489FieldTitle"><b>${esc(fld.key)}</b>${fld.aiGenerated?`<span class="v440Tag">AI ${Math.round(Number(fld.aiConfidence||0))}%</span>`:''}${f.legacy&&fld.customFieldV463?'<span class="v440Tag">FIELD MỚI</span>':''}<span style="flex:1"></span><button class="v440Btn primary" id="v440AddField">+ FIELD</button><button class="v440Btn" id="v440Dup">NHÂN BẢN</button><button class="v440Btn danger" id="v440DelField">XÓA</button></div><div class="v440Field"><label>KEY</label><input id="v440FldKey" value="${esc(fld.key)}"></div><div class="v440Field"><label>LABEL</label><input id="v440FldLabel" value="${esc(fld.label||'')}"></div><div class="v440Grid2"><div class="v440Field"><label>TYPE</label><select id="v440FldType">${['text','textarea','number','time','date','checkbox','select','signature','computed','static-text'].map(t=>`<option ${t===fld.type?'selected':''}>${t}</option>`).join('')}</select></div><div class="v440Field"><label>BIND → STATE</label><input id="v440FldBind" value="${esc(fld.bind||fld.key)}"></div></div>${isText?`<div class="v440Field"><label>CHẾ ĐỘ DÒNG</label><select id="v489WrapMode" class="${multi?'v489WrapGood':'v489WrapSingle'}"><option value="single" ${!multi?'selected':''}>1 DÒNG · KHÔNG WRAP</option><option value="auto" ${multi?'selected':''}>NHIỀU DÒNG · AUTO WRAP THEO KHUNG</option></select><div class="v489TypeHint">AUTO WRAP: chữ chạm mép phải sẽ tự xuống dòng. Với mẫu có các hàng chấm, dùng SỐ DÒNG MẪU + KHỚP DÒNG để chữ bám đúng từng hàng.</div></div>${multi?`<div class="v489RowFitBox"><label>SỐ DÒNG MẪU <input id="v490RowCount" type="number" min="1" max="30" step="1" value="${Math.max(1,Math.round(Number(fld.rowCount)||cap))}"></label><label class="v489RowFitCheck"><input id="v490RowFit" type="checkbox" ${fld.rowFit!==false?'checked':''}> KHỚP DÒNG VỚI CHIỀU CAO FIELD</label><div class="v628RowMode"><button class="v440Btn ${String(fld.rowBaselineMode||'')==='box'?'active':''}" id="v628AutoRows" type="button">↕ TỰ CĂN ĐỀU TRONG KHUNG</button><button class="v440Btn ${String(fld.rowBaselineMode||'')!=='box'?'active':''}" id="v628PrintedRows" type="button">↩ HÀNG PDF GỐC</button></div><div class="v489TypeHint v489WrapGood">${String(fld.rowBaselineMode||'')==='box'?'CĂN THEO KHUNG FIELD':'ƯU TIÊN HÀNG PDF GỐC'} · ${cap} dòng · test 1–9 dùng đúng baseline PDF</div></div>`:''}`:fld.type==='static-text'?`<div class="v489TypeHint v489WrapGood">STATIC TEXT luôn AUTO WRAP theo chiều rộng field · ${cap} dòng theo chiều cao hiện tại.</div>`:''}<div class="v489GeoGrid">${['x','y','w','h'].map(k=>`<div class="v440Field"><label>${k.toUpperCase()} %</label><input data-geom="${k}" value="${(Number(fld[k]||0)*100).toFixed(3)}"></div>`).join('')}</div></div></details>`;
  const typography=`<details class="v489InspectorSection" open><summary>🔤 CHỮ · FONT · DÒNG</summary><div class="v489InspectorBody"><div class="v487ControlGrid2"><div class="v440Field"><label>FONT</label><select id="v487FontFamily">${FM_FONTS_V487.map(x=>`<option ${x===fmFontV487(fld.fontFamily)?'selected':''}>${esc(x)}</option>`).join('')}</select></div><div class="v440Field"><label>SIZE PX</label><input id="v440FldFont" type="number" min="6" max="80" step="0.5" value="${Number(fld.fontSize||14)}"></div></div><div class="v487ControlGrid4"><div class="v440Field"><label>WEIGHT</label><select id="v487FontWeight">${['400','500','600','700','800','900'].map(x=>`<option ${x===String(fld.fontWeight||'700')?'selected':''}>${x}</option>`).join('')}</select></div><div class="v440Field"><label>LINE</label><input id="v487LineHeight" type="number" min="0.8" max="4" step="0.05" value="${Number(fld.lineHeight||1.08)}"></div><div class="v440Field"><label>SPACE</label><input id="v487LetterSpacing" type="number" min="-2" max="10" step="0.1" value="${Number(fld.letterSpacing||0)}"></div><div class="v440Field"><label>COLOR</label><input id="v487TextColor" type="color" value="${fmColorV487(fld.textColor)}"></div></div><div class="v487ActionGrid tight"><button class="v440Btn ${String(fld.fontStyle)==='italic'?'active':''}" id="v487Italic" type="button"><i>NGHIÊNG</i></button><button class="v440Btn ${fld.underline?'active':''}" id="v487Underline" type="button"><u>GẠCH CHÂN</u></button></div><div class="v487ActionGrid"><button class="v440Btn" id="v487CopyStyle" type="button">COPY STYLE</button><button class="v440Btn" id="v487PasteStyle" type="button">PASTE STYLE</button><button class="v440Btn" id="v487SyncStyle" type="button" style="grid-column:1/-1">ĐỒNG BỘ FIELD ĐÃ CHỌN</button></div>${fld.type==='select'?`<div class="v440Field"><label>OPTIONS · mỗi dòng 1 lựa chọn</label><textarea id="v487Options" rows="4">${esc((fld.options||[]).join('\n'))}</textarea></div>`:''}${fld.type==='static-text'?`<div class="v440Field"><label>STATIC TEXT</label><textarea id="v487StaticText" rows="3">${esc(fld.text||'')}</textarea></div>`:''}</div></details>`;
  const alignPanel=`<details class="v489InspectorSection" open><summary>📝 CĂN CHỮ TRONG FIELD</summary><div class="v489InspectorBody"><div class="v461TextAlignRow"><span>NGANG</span><div class="v487AlignGrid"><button class="v461AlignBtn ${String(fld.align||'left')==='left'?'active':''}" data-text-align="left">⬅ TRÁI</button><button class="v461AlignBtn ${String(fld.align||'left')==='center'?'active':''}" data-text-align="center">↔ GIỮA</button><button class="v461AlignBtn ${String(fld.align||'left')==='right'?'active':''}" data-text-align="right">PHẢI ➡</button></div></div><div class="v461TextAlignRow"><span>DỌC</span><div class="v487AlignGrid"><button class="v461AlignBtn ${String(fld.valign||'middle')==='top'?'active':''}" data-text-valign="top">↑ TRÊN</button><button class="v461AlignBtn ${String(fld.valign||'middle')==='middle'?'active':''}" data-text-valign="middle">↕ GIỮA</button><button class="v461AlignBtn ${String(fld.valign||'middle')==='bottom'?'active':''}" data-text-valign="bottom">↓ DƯỚI</button></div></div><button class="v461CenterBoth" id="v461CenterBoth">◎ CĂN GIỮA Ô · NGANG + DỌC</button></div></details>`;
  const multiPanel=`<details class="v489InspectorSection"><summary>📐 CĂN VỊ TRÍ NHIỀU FIELD</summary><div class="v489InspectorBody"><div class="v489TypeHint">Shift+click để chọn nhiều field. Các nút dưới tác động lên toàn bộ field đang chọn.</div><div class="v489MultiActions"><button class="v440Btn" data-nudge="-1,0">← 1px</button><button class="v440Btn" data-nudge="1,0">1px →</button><button class="v440Btn" data-nudge="0,-1">↑ 1px</button><button class="v440Btn" data-nudge="0,1">↓ 1px</button>${[['left','CĂN TRÁI'],['right','CĂN PHẢI'],['top','CĂN TRÊN'],['bottom','CĂN DƯỚI'],['midX','GIỮA X'],['midY','GIỮA Y'],['sameW','CÙNG RỘNG'],['sameH','CÙNG CAO'],['distX','DÀN NGANG'],['distY','DÀN DỌC']].map(([k,l])=>`<button class="v440Btn" data-align="${k}">${l}</button>`).join('')}</div></div></details>`;
  h.innerHTML=`${testHint}${pagePanel}${fieldPanel}${typography}${alignPanel}${multiPanel}<div class="v489SaveBar"><button class="v440Btn green" id="v440Save">💾 LƯU NHÁP</button>${!f.legacy?'<button class="v440Btn danger" id="v440DeleteForm">XÓA FORM</button>':''}</div>`;
...
$('v440FldKey').onchange=e=>{const k=fieldKey(e.target.value);if(!k||f.fields.some(x=>x!==fld&&x.key===k)){e.target.value=fld.key;return alert('KEY trống hoặc đã tồn tại.')}pushHistory();selectedMany.delete(fld.key);fld.key=k;selectedKey=k;selectedMany.add(k);if(!fld.bind)fld.bind=k;saveLocal();renderAll()};$('v440FldLabel').onchange=e=>{pushHistory();fld.label=e.target.value;saveLocal();drawBoxes()};$('v440FldType').onchange=e=>{pushHistory();const prev=String(fld.type||'');fld.type=e.target.value;if(fld.type==='textarea'&&prev!=='textarea'){fld.valign='top';fld.rowFit=true;fld.rowCount=fmInferRowsV490(fld)}fmNormalizeFieldV487(fld);saveLocal();drawBoxes();renderInspector()};if($('v489WrapMode'))$('v489WrapMode').onchange=e=>{pushHistory();const toMulti=e.target.value==='auto';fld.type=toMulti?'textarea':'text';if(toMulti){fld.valign='top';fld.rowFit=true;fld.rowCount=fmInferRowsV490(fld)}fmNormalizeFieldV487(fld);saveLocal();drawBoxes();renderInspector();status(fld.type==='textarea'?'✓ NHIỀU DÒNG: AUTO WRAP + KHỚP DÒNG đang bật. Chọn đúng SỐ DÒNG MẪU để chữ bám các hàng chấm.':'✓ 1 DÒNG: chữ không tự xuống dòng.')};if($('v490RowCount'))$('v490RowCount').onchange=e=>{pushHistory();fld.rowCount=Math.max(1,Math.min(30,Math.round(Number(e.target.value)||1)));fld.rowFit=true;fld.rowBaselineMode='box';fld.valign='top';fmApplyRowFitV490(fld);saveLocal();drawBoxes();renderInspector();status('✓ Đã tự chia đều '+fld.rowCount+' dòng trong toàn bộ chiều cao field. Test 1–9 bám đúng baseline PDF.')};if($('v490RowFit'))$('v490RowFit').onchange=e=>{pushHistory();fld.rowFit=!!e.target.checked;if(fld.rowFit)fmApplyRowFitV490(fld);saveLocal();drawBoxes();renderInspector();status(fld.rowFit?'✓ KHỚP DÒNG bật: khoảng cách dòng tự theo chiều cao field.':'LINE thủ công: có thể chỉnh LINE trong phần CHỮ · FONT · DÒNG.')};if($('v628AutoRows'))$('v628AutoRows').onclick=()=>{pushHistory();fld.rowFit=true;fld.rowBaselineMode='box';fld.valign='top';fld.rowCount=Math.max(1,Math.min(30,Math.round(Number(fld.rowCount)||fmInferRowsV490(fld))));fmApplyRowFitV490(fld);delete testValuesV462[fld.bind||fld.key];saveLocal();drawBoxes();renderInspector();status('✓ TỰ CĂN DÒNG: '+fld.rowCount+' dòng được chia đều trong toàn khung; test 1–9 và PDF dùng cùng baseline.');};if($('v628PrintedRows'))$('v628PrintedRows').onclick=()=>{pushHistory();delete fld.rowBaselineMode;delete testValuesV462[fld.bind||fld.key];saveLocal();drawBoxes();renderInspector();status('✓ Đã trả về ưu tiên hàng in/PDF gốc nếu field có dotRects.');};$('v440FldBind').onchange=e=>{pushHistory();fld.bind=fieldKey(e.target.value)||fld.key;saveLocal()};const textTargets=()=>{const a=selectedFields();return a.length?a:[fld]};const applyStyleProp=(key,val)=>{pushHistory();for(const x of textTargets()){x[key]=val;fmNormalizeFieldV487(x)}saveLocal();drawBoxes();renderInspector()};$('v440FldFont').onchange=e=>applyStyleProp('fontSize',fmClampV487(e.target.value,6,80,14));$('v487FontFamily').onchange=e=>applyStyleProp('fontFamily',fmFontV487(e.target.value));$('v487FontWeight').onchange=e=>applyStyleProp('fontWeight',String(e.target.value));$('v487LineHeight').onchange=e=>{pushHistory();for(const x of textTargets()){x.rowFit=false;x.lineHeight=fmClampV487(e.target.value,.8,4,1.08);fmNormalizeFieldV487(x)}saveLocal();drawBoxes();renderInspector();status('✓ LINE thủ công đã áp dụng; KHỚP DÒNG tự động đã tắt cho field được chọn.')};$('v487LetterSpacing').onchange=e=>applyStyleProp('letterSpacing',fmClampV487(e.target.value,-2,10,0));$('v487TextColor').onchange=e=>applyStyleProp('textColor',fmColorV487(e.target.value));$('v487Italic').onclick=()=>applyStyleProp('fontStyle',String(fld.fontStyle||'normal')==='italic'?'normal':'italic');$('v487Underline').onclick=()=>applyStyleProp('underline',!fld.underline);$('v487CopyStyle').onclick=()=>{styleClipboardV487=fmStyleV487(fld);status('✓ Đã COPY STYLE của '+fld.key)};$('v487PasteStyle').onclick=()=>{if(!styleClipboardV487)return status('Chưa COPY STYLE.',true);pushHistory();for(const x of textTargets())fmCopyStyleV487(styleClipboardV487,x);saveLocal();drawBoxes();renderInspector();status('✓ Đã PASTE STYLE cho '+textTargets().length+' field.')};$('v487SyncStyle').onclick=()=>{pushHistory();for(const x of textTargets())if(x!==fld)fmCopyStyleV487(fld,x);saveLocal();drawBoxes();renderInspector();status('✓ Đã đồng bộ typography theo field '+fld.key+'.')};if($('v487Options'))$('v487Options').onchange=e=>{pushHistory();fld.options=fmOptionsV487(e.target.value);saveLocal()};if($('v487StaticText'))$('v487StaticText').onchange=e=>{pushHistory();fld.text=String(e.target.value||'');saveLocal();drawBoxes()};document.querySelectorAll('#v440Inspector [data-text-align]').forEach(b=>b.onclick=()=>{pushHistory();for(const x of textTargets())x.align=b.dataset.textAlign;saveLocal();drawBoxes();renderInspector()});document.querySelectorAll('#v440Inspector [data-text-valign]').forEach(b=>b.onclick=()=>{pushHistory();for(const x of textTargets())x.valign=b.dataset.textValign;saveLocal();drawBoxes();renderInspector()});$('v461CenterBoth').onclick=()=>{pushHistory();for(const x of textTargets()){x.align='center';x.valign='middle'}saveLocal();drawBoxes();renderInspector()};document.querySelectorAll('#v440Inspector [data-geom]').forEach(i=>i.onchange=()=>{pushHistory();const k=i.dataset.geom,v=Number(i.value)/100;if(Number.isFinite(v))fld[k]=clamp(v,(k==='w'||k==='h')?0.005:0,1);if(k==='h'&&fmIsMultiV489(fld)&&fld.rowFit)fmApplyRowFitV490(fld);saveLocal();drawBoxes();renderInspector()});
...
async function exportCurrent(){const f=form();if(!f)return;if(!validateCurrentFormV487())return;status('Đang đóng gói form…');try{const out=clone(f),files=[];for(let i=0;i<(f.pages||[]).length;i++){const p=f.pages[i],x=await pageBlob(p);if(!x)throw new Error('Trang '+(i+1)+' chưa có ảnh.');const name=`page-${String(i+1).padStart(2,'0')}.${x.ext}`;files.push({name,data:x.blob});out.pages[i].image=name;delete out.pages[i].blobKey;out.pages[i].sha256=await sha256(x.blob);out.pages[i].bytes=x.blob.size}out.schema=2;out.engine='form-manager-v487';out.managerVersion='V6.2.8-ROW-CALIBRATION';out.exportedAt=new Date().toISOString();files.unshift({name:'form.json',data:JSON.stringify(out,null,2)+'\n'});files.push({name:'README.txt',data:'E-REPORT FORM PACKAGE V4.7.5 PDF BACKGROUND IMPORT\nUpload page images to forms/'+f.id+'/ and add the form.json metadata to forms.registry.json. PDF nguồn đã được chuyển thành ảnh page-XX.png/webp khi xuất gói; runtime không cần PDF.js để chạy form.\nRuntime JS does not need a new per-form file.\n'});download(await makeZip(files),`${f.id}-ereport-form.zip`);status('✓ Đã xuất gói ZIP: form.json + ảnh riêng, không nhúng base64.')}catch(e){status('Không xuất được: '+(e?.message||e),true)}}
async function exportRegistry(){if(!validateCurrentFormV487())return;fmNormalizeRegistryV487(draft);const out=clone(draft);for(const f of out.forms||[]){for(let i=0;i<(f.pages||[]).length;i++){const p=f.pages[i];if(p.blobKey){p.image=`./forms/${f.id}/page-${String(i+1).padStart(2,'0')}.${p.ext||'png'}`;delete p.blobKey}}}out.schema=2;out.version='V6.2.8';out.build=BUILD;out.managerVersion='V6.2.8-ROW-CALIBRATION';out.updatedAt=new Date().toISOString();download(new Blob([JSON.stringify(out,null,2)+'\n'],{type:'application/json'}),'forms.registry.json');status('✓ Đã xuất forms.registry.json từ đúng layout đang thấy trên Form Manager · màn hình và PDF dùng cùng geometry.')} 
...
function runtimeControl(fld,val){fmNormalizeFieldV487(fld);const k=esc(fld.bind||fld.key),st=fmStyleV487(fld),va=st.valign,style=fmCssV487(fld,false),flexStyle=`display:flex;align-items:${va==='top'?'flex-start':va==='bottom'?'flex-end':'center'};justify-content:${st.align==='center'?'center':st.align==='right'?'flex-end':'flex-start'};`;if(fld.type==='checkbox')return `<input data-v440-bind="${k}" type="checkbox" ${val?'checked':''}>`;if(fld.type==='textarea')return `<textarea data-v440-bind="${k}" wrap="soft" style="${style};padding:0 2px;border:0;background:transparent;resize:none;overflow:hidden;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word">${esc(val||'')}</textarea>`;if(fld.type==='select')return `<select data-v440-bind="${k}" style="${style}">${(fld.options||[]).map(x=>`<option ${String(x)===String(val)?'selected':''}>${esc(x)}</option>`).join('')}</select>`;if(fld.type==='signature')return `<button data-v440-sign="${k}" style="${style}">${val?'✓ ĐÃ KÝ':'KÝ'}</button>`;if(fld.type==='static-text')return `<div style="${style};${flexStyle};width:100%;height:100%;overflow:hidden;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word">${esc(fld.text||fld.label||'')}</div>`;return `<input data-v440-bind="${k}" type="${['number','time','date'].includes(fld.type)?fld.type:'text'}" value="${esc(val??'')}" style="${style}">`}
...
/* V4.8.6: render the live SVG through the browser SVG engine before PDF encoding.
   The AD layout editor stores display corrections in SVG transform/opacity/style.
   Direct Canvas text painting ignored those properties and could export old positions.
   This path serializes the live display layer, so PDF follows what the operator sees. */
async function sagsPdfExactSvgOverlay(svg,pageNo){
  const clone=svg.cloneNode(true);
  clone.querySelectorAll('.hit,.selected-region,.v368-layout-hit,.v1012-resize-handle').forEach(el=>el.remove());
  // Signatures remain on the Safari-safe direct-image path below.
  clone.querySelectorAll('image').forEach(el=>el.remove());
  if(root.sagsV495ManagedPage?.(pageNo)){
    clone.querySelectorAll('text.value,text.tick,foreignObject,g[data-sags-fm-check],g[data-export-field],.v373-line-render').forEach(el=>el.remove());
  }
  try{
    const sanitize=(typeof sanitizeSvgForExport==='function')?sanitizeSvgForExport:root.sanitizeSvgForExport;
    if(typeof sanitize==='function')sanitize(clone);
  }catch(e){console.warn('V4.8.6 PDF SVG sanitize fallback',e)}
  clone.querySelectorAll('text.value').forEach(t=>{
    t.setAttribute('font-family','Times New Roman');
    t.setAttribute('font-weight',t.getAttribute('font-weight')||'700');
    t.setAttribute('dominant-baseline',t.getAttribute('dominant-baseline')||'middle');
    if(t.classList.contains('manual-value')||t.classList.contains('time-value')){try{t.setAttribute('fill',String(ENTRY_COLOR||'#0057b8'))}catch(_){t.setAttribute('fill','#0057b8')}}
    if(t.classList.contains('center')&&!t.getAttribute('text-anchor'))t.setAttribute('text-anchor','middle');
    if(t.classList.contains('left')&&!t.getAttribute('text-anchor'))t.setAttribute('text-anchor','start');
  });
  clone.querySelectorAll('text.tick').forEach(t=>{
    t.setAttribute('font-family','Arial');t.setAttribute('font-weight','900');t.setAttribute('fill','#111');
  ...
  clone.setAttribute('width',String(baseW()));clone.setAttribute('height',String(baseH()));
  if(!clone.getAttribute('viewBox'))clone.setAttribute('viewBox',`0 0 ${baseW()} ${baseH()}`);
  clone.setAttribute('preserveAspectRatio','none');
  const style=document.createElementNS('http://www.w3.org/2000/svg','style');
  style.textContent=`.value{font-family:'Times New Roman',serif;font-weight:700}.manual-value,.time-value{fill:#0057b8}.left{text-anchor:start}.center{text-anchor:middle}.tick{font-family:Arial,sans-serif;font-weight:900;fill:#111;text-anchor:middle}`;
  clone.insertBefore(style,clone.firstChild);
  const xml=new XMLSerializer().serializeToString(clone);
  const loader=(typeof v479LoadSvgOverlay==='function')?v479LoadSvgOverlay:root.v479LoadSvgOverlay;
  if(typeof loader==='function')return await loader(xml,'dữ liệu Trang '+pageNo);
  const load=src=>new Promise((resolve,reject)=>{const im=new Image();im.onload=()=>resolve(im);im.onerror=()=>reject(new Error('Không dựng được dữ liệu Trang '+pageNo));im.src=src});
  const blob=new Blob([xml],{type:'image/svg+xml;charset=utf-8'}),url=URL.createObjectURL(blob);
  try{try{return await load(url)}catch(_){return await load('data:image/svg+xml;charset=utf-8,'+encodeURIComponent(xml))}}
...
function sigSummary(v){v=S(v);return v.length>1600?`${v.length}:${v.slice(0,64)}:${v.slice(-64)}`:v}
function pageFingerprint(pageNo,svg,w,h,mode){const chunks=[BUILD,mode,pageNo,w,h];for(const el of svg.querySelectorAll('text,foreignObject,line,path'))chunks.push(el.tagName,S(el.getAttribute('x')),S(el.getAttribute('y')),S(el.getAttribute('width')),S(el.getAttribute('height')),S(el.getAttribute('x1')),S(el.getAttribute('y1')),S(el.getAttribute('x2')),S(el.getAttribute('y2')),S(el.getAttribute('d')),S(el.getAttribute('font-size')),S(el.getAttribute('text-anchor')),S(el.getAttribute('transform')),S(el.getAttribute('opacity')),S(el.getAttribute('class')),S(el.getAttribute('style')),S(el.style?.display),S(el.style?.opacity),S(el.textContent));const st=stateObj();for(const f of globalFields())if(Number(f.page)===Number(pageNo)&&String(f.type)==='signature')chunks.push(f.key,sigSummary(st[f.key]));return fnv(chunks.join('|'))}
function isIOS(){return /iPad|iPhone|iPod/.test(navigator.userAgent||'')||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1)}
function isMobile(){return isIOS()||/Android|Mobile/i.test(navigator.userAgent||'')||Math.min(Number(screen?.width||9999),Number(screen?.height||9999))<900}
function b(v){return new TextEncoder().encode(v)}
function cat(parts){let n=parts.reduce((a,x)=>a+x.length,0),o=new Uint8Array(n),p=0;for(const x of parts){o.set(x,p);p+=x.length}return o}
function pdfPayload(bytes,fileName){try{return typeof v479PdfPayload==='function'?v479PdfPayload(bytes,fileName):new File([bytes],fileName,{type:'application/pdf'})}catch(_){return new Blob([bytes],{type:'application/pdf'})}}
function pdfFromJpegs(pages,fileName,landscape=false){const objects=[],count=pages.length,catalogId=1,pagesId=2,pageIds=[],imageIds=[],contentIds=[];let next=3;for(let i=0;i<count;i++){pageIds.push(next++);imageIds.push(next++);contentIds.push(next++)}objects[catalogId]=b(`<< /Type /Catalog /Pages ${pagesId} 0 R >>`);objects[pagesId]=b(`<< /Type /Pages /Count ${count} /Kids [${pageIds.map(id=>`${id} 0 R`).join(' ')}] >>`);const PW=landscape?841.89:595.28,PH=landscape?595.28:841.89;for(let i=0;i<count;i++){const img=pages[i],pId=pageIds[i],iId=imageIds[i],cId=contentIds[i];objects[pId]=b(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${PW} ${PH}] /Resources << /XObject << /Im${i+1} ${iId} 0 R >> >> /Contents ${cId} 0 R >>`);objects[iId]=cat([b(`<< /Type /XObject /Subtype /Image /Width ${img.width} /Height ${img.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${img.bytes.length} >>\nstream\n`),img.bytes,b('\nendstream')]);const content=b(`q\n${PW} 0 0 ${PH} 0 0 cm\n/Im${i+1} Do\nQ\n`);objects[cId]=cat([b(`<< /Length ${content.length} >>\nstream\n`),content,b('endstream')])}const parts=[b('%PDF-1.4\n')],offs=new Array(objects.length).fill(0);let pos=parts[0].length;for(let id=1;id<objects.length;id++){offs[id]=pos;const h=b(`${id} 0 obj\n`),t=b('\nendobj\n');parts.push(h,objects[id],t);pos+=h.length+objects[id].length+t.length}const xrefPos=pos;let xref=`xref\n0 ${objects.length}\n0000000000 65535 f \n`;for(let id=1;id<objects.length;id++)xref+=String(offs[id]).padStart(10,'0')+' 00000 n \n';xref+=`trailer\n<< /Size ${objects.length} /Root ${catalogId} 0 R >>\nstartxref\n${xrefPos}\n%%EOF`;parts.push(b(xref));return pdfPayload(cat(parts),fileName)}

const bgByteCache=(root.__SAGS_LAYER_PDF_BG_CACHE ||= new Map());
function layerCfg(landscape=false,pageNo=0){if(landscape){const w=isMobile()?1120:1260;return {w,h:Math.round(w*1055/1491),quality:.84}}if(Number(pageNo)===6||Number(pageNo)===7){const w=baseW();return {w,h:baseH(),quality:.93}}const w=isMobile()?900:1000;return {w,h:Math.round(w*baseH()/baseW()),quality:.84}}
function bgPath(pageNo){return `./pdf-bg-page${Number(pageNo)}.jpg`}
// A failed fetch must not poison the in-memory cache for every later export.
async function fetchBytes(url){
  const existing=bgByteCache.get(url);if(existing)return existing;
  const job=fetch(url,{cache:'force-cache'}).then(r=>{
    if(!r.ok)throw new Error(`Không tải được PDF background ${url} (HTTP ${r.status})`);
    return r.arrayBuffer();
  }).then(a=>new Uint8Array(a));
  bgByteCache.set(url,job);
  try{return await job}catch(e){if(bgByteCache.get(url)===job)bgByteCache.delete(url);throw e}
}
// The JPEG background is on-demand rather than a staged bootstrap asset. If
// it is temporarily unavailable, use the SAME published, verified PNG page.
// Never replace a missing background with white or an unrelated page.
async function fetchPdfBackgroundV6114(canvas){
  const url=canvas.__sagsLayeredBg,pageNo=Number(canvas.__sagsPageNo);
  try{return await fetchBytes(url)}catch(first){
    try{return await fetchBytes(url+'?__sags_pdf_retry='+Date.now())}catch(second){
      const fallback='./page'+pageNo+'.png';
      let png;
      try{png=await fetchBytes(fallback)}catch(third){
        throw new Error(`Không tải được nền PDF ${url} hoặc ảnh dự phòng ${fallback}. Giữ nháp và thử lại khi có mạng.`);
      }
      ...
      try{
        const image=await new Promise((resolve,reject)=>{
          ...
          im.onerror=()=>reject(new Error('Không đọc được ảnh nền dự phòng '+fallback));
          im.src=objectUrl;
        });
        const width=Number(canvas.__sagsLayeredBgWidth),height=Number(canvas.__sagsLayeredBgHeight);
        if(image.naturalWidth!==width||image.naturalHeight!==height)
          throw new Error('Ảnh nền dự phòng không đúng kích thước trang '+pageNo);
        ...
        if(!ctx)throw new Error('Thiết bị không thể tạo ảnh nền PDF dự phòng');
        ctx.drawImage(image,0,0,width,height);
        const jpg=await canvasToJpegBytes(scratch,.96);
        if(!(jpg?.length>100))throw new Error('Ảnh nền PDF dự phòng rỗng');
        return jpg;
      }finally{
...
async function fastPage(pageNo,landscape=false){const t0=perf(),page=document.getElementById('page'+pageNo),svg=document.getElementById('svg'+pageNo);if(!page||!svg)throw new Error('Không tìm thấy Trang '+pageNo);const cfg=layerCfg(landscape,pageNo),c=document.createElement('canvas');c.width=cfg.w;c.height=cfg.h;c.__sagsOriginalWidth=c.width;c.__sagsOriginalHeight=c.height;c.__sagsLayered=true;c.__sagsLayeredBg=bgPath(pageNo);c.__sagsLayeredBgWidth=landscape?1491:baseW();c.__sagsLayeredBgHeight=landscape?1055:baseH();c.__sagsPageNo=Number(pageNo);const ctx=c.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);
const fmPageInfo=root.sagsV495PageInfo?.(pageNo);
if(fmPageInfo && (fmPageInfo.blobKey || fmPageInfo.image!=='./page'+Number(pageNo)+'.png')){
  const source=await root.sagsV495PageImage(pageNo),bg=await loadImage(source);
  if(!(bg.naturalWidth>0&&bg.naturalHeight>0))throw new Error('Không đọc được ảnh Form Manager trang '+pageNo);
  c.__sagsLayered=false;
  ctx.drawImage(bg,0,0,c.width,c.height);
}
const sx=c.width/baseW(),sy=c.height/baseH(),t1=perf();const exactOverlay=await sagsPdfExactSvgOverlay(svg,pageNo);ctx.drawImage(exactOverlay,0,0,c.width,c.height);if(root.sagsV495ManagedPage?.(pageNo))root.sagsV495PaintCanonicalPage(ctx,pageNo);const t2=perf();await drawSignatures(ctx,pageNo,sx,sy);if(Number(pageNo)===4&&!root.__SAGS_SUPPRESS_BBBT_CXR_ON_RENDER&&stateObj().bbbtCxrNo!==undefined&&stateObj().bbbtCxrNo!==null&&stateObj().bbbtCxrNo!==''){ctx.save();ctx.font=`900 ${28*sy}px Times New Roman`;ctx.fillStyle='#b54032';ctx.textAlign='right';ctx.textBaseline='top';let n=stateObj().bbbtCxrNo;try{if(typeof formatBBBTCxrNo==='function')n=formatBBBTCxrNo(n)}catch(_){}ctx.fillText('CXR No: '+n,(baseW()-42)*sx,30*sy);ctx.restore()}const t3=perf(),row={page:Number(pageNo),layered:true,width:c.width,height:c.height,overlayMs:+(t2-t1).toFixed(1),signatureMs:+(t3-t2).toFixed(1),totalMs:+(t3-t0).toFixed(1)};const P=root.__SAGS_V450_PDF_RUN;if(P){P.pages.push(row);P.renderMs=(P.renderMs||0)+row.totalMs}return c}
async function singlePageData(c,landscape){if(c?.__sagsLayered){const is421=Number(c.__sagsPageNo)===6||Number(c.__sagsPageNo)===7;const [bg,ink]=await Promise.all([fetchPdfBackgroundV6114(c),is421?canvasToLosslessRgbV6110(c):canvasToJpegBytes(c,layerCfg(landscape,c.__sagsPageNo).quality).then(bytes=>({bytes,format:'jpeg'}))]);return {layered:true,bg,overlay:ink.bytes,overlayFormat:ink.format,bgW:c.__sagsLayeredBgWidth,bgH:c.__sagsLayeredBgHeight,ovW:c.width,ovH:c.height}}if((Number(c?.__sagsPageNo)===6||Number(c?.__sagsPageNo)===7)&&c){const ink=await canvasToLosslessRgbV6110(c);return {layered:false,img:ink.bytes,imgFormat:ink.format,w:c.width,h:c.height}}let work=c;if(isMobile()&&c&&c.width>(landscape?1260:1000)){const target=landscape?1260:1000,r=target/c.width,small=document.createElement('canvas');small.width=target;small.height=Math.round(c.height*r);const x=small.getContext('2d',{alpha:false});x.fillStyle='#fff';x.fillRect(0,0,small.width,small.height);x.drawImage(c,0,0,small.width,small.height);work=small}const img=await canvasToJpegBytes(work,landscape?.84:.82);const out={layered:false,img,w:work.width,h:work.height};if(work!==c){try{work.width=1;work.height=1}catch(_){}}return out}
// Preserve the filled-in FSAGS 42.1 glyph pixels exactly: JPEG introduces visible
// ringing around 16px text even at quality .93. Keep the existing JPEG background.
// Compress just the opaque white-backed RGB overlay; fall back to uncompressed
// RGB (larger file but still lossless) on browsers without CompressionStream.
async function canvasToLosslessRgbV6110(canvas){
  const w=canvas.width,h=canvas.height;
  const source=canvas.getContext('2d').getImageData(0,0,w,h).data;
  const rgb=new Uint8Array(w*h*3);
  for(let i=0,j=0;i<source.length;i+=4,j+=3){
    const a=source[i+3];
    if(a===255){rgb[j]=source[i];rgb[j+1]=source[i+1];rgb[j+2]=source[i+2]}
    else if(a===0){rgb[j]=rgb[j+1]=rgb[j+2]=255}
    else{const t=a/255;rgb[j]=Math.round(255*(1-t)+source[i]*t);rgb[j+1]=Math.round(255*(1-t)+source[i+1]*t);rgb[j+2]=Math.round(255*(1-t)+source[i+2]*t)}
  }
  if(typeof CompressionStream==='function'){
    try{
      const compressed=new Blob([rgb]).stream().pipeThrough(new CompressionStream('deflate'));
      return {bytes:new Uint8Array(await new Response(compressed).arrayBuffer()),format:'flate-rgb'};
    }catch(error){console.warn('Lossless PDF compression unavailable; keeping raw RGB overlay.',error)}
  }
  return {bytes:rgb,format:'raw-rgb'};
}
function imageObj(bytes,w,h,format='jpeg'){
  const filter=format==='flate-rgb'?'/Filter /FlateDecode ':format==='raw-rgb'?'':'/Filter /DCTDecode ';
  return cat([b(`<< /Type /XObject /Subtype /Image /Width ${w} /Height ${h} /ColorSpace /DeviceRGB /BitsPerComponent 8 ${filter}/Length ${bytes.length} >>\nstream\n`),bytes,b('\nendstream')]);
}
function mixedPdf(pages,fileName,landscape=false){const objects=[],catalogId=1,pagesId=2,gsId=3,pageIds=[];let next=4;const ids=[];for(const p of pages){if(p.layered){const row={pageId:next++,bgId:next++,ovId:next++,contentId:next++};ids.push(row);pageIds.push(row.pageId)}else{const row={pageId:next++,imgId:next++,contentId:next++};ids.push(row);pageIds.push(row.pageId)}}objects[catalogId]=b(`<< /Type /Catalog /Pages ${pagesId} 0 R >>`);objects[pagesId]=b(`<< /Type /Pages /Count ${pages.length} /Kids [${pageIds.map(x=>`${x} 0 R`).join(' ')}] >>`);objects[gsId]=b('<< /Type /ExtGState /BM /Multiply >>');const PW=landscape?841.89:595.28,PH=landscape?595.28:841.89;for(let i=0;i<pages.length;i++){const p=pages[i],r=ids[i];if(p.layered){objects[r.pageId]=b(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${PW} ${PH}] /Resources << /XObject << /Bg${i+1} ${r.bgId} 0 R /Ov${i+1} ${r.ovId} 0 R >> /ExtGState << /GS1 ${gsId} 0 R >> >> /Contents ${r.contentId} 0 R >>`);objects[r.bgId]=imageObj(p.bg,p.bgW,p.bgH);objects[r.ovId]=imageObj(p.overlay,p.ovW,p.ovH,p.overlayFormat||'jpeg');const content=b(`q\n${PW} 0 0 ${PH} 0 0 cm\n/Bg${i+1} Do\nQ\nq\n/GS1 gs\n${PW} 0 0 ${PH} 0 0 cm\n/Ov${i+1} Do\nQ\n`);objects[r.contentId]=cat([b(`<< /Length ${content.length} >>\nstream\n`),content,b('endstream')])}else{objects[r.pageId]=b(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${PW} ${PH}] /Resources << /XObject << /Im${i+1} ${r.imgId} 0 R >> >> /Contents ${r.contentId} 0 R >>`);objects[r.imgId]=imageObj(p.img,p.w,p.h,p.imgFormat||'jpeg');const content=b(`q\n${PW} 0 0 ${PH} 0 0 cm\n/Im${i+1} Do\nQ\n`);objects[r.contentId]=cat([b(`<< /Length ${content.length} >>\nstream\n`),content,b('endstream')])}}const parts=[b('%PDF-1.4\n')],offs=new Array(objects.length).fill(0);let pos=parts[0].length;for(let id=1;id<objects.length;id++){offs[id]=pos;const h=b(`${id} 0 obj\n`),t=b('\nendobj\n');parts.push(h,objects[id],t);pos+=h.length+objects[id].length+t.length}const xrefPos=pos;let xref=`xref\n0 ${objects.length}\n0000000000 65535 f \n`;for(let id=1;id<objects.length;id++)xref+=String(offs[id]).padStart(10,'0')+' 00000 n \n';xref+=`trailer\n<< /Size ${objects.length} /Root ${catalogId} 0 R >>\nstartxref\n${xrefPos}\n%%EOF`;parts.push(b(xref));return pdfPayload(cat(parts),fileName)}
async function layeredPdf(canvases,fileName,landscape=false){const t=perf(),out=new Array(canvases.length),step=(isIOS()||canvases.some(c=>Number(c?.__sagsPageNo)===6||Number(c?.__sagsPageNo)===7))?1:2;for(let i=0;i<canvases.length;i+=step){const batch=canvases.slice(i,i+step),rows=await Promise.all(batch.map(c=>singlePageData(c,landscape)));rows.forEach((x,j)=>out[i+j]=x);if(isIOS())await new Promise(r=>setTimeout(r,0))}const P=root.__SAGS_V450_PDF_RUN;if(P)P.encodeCoreMs=(P.encodeCoreMs||0)+(perf()-t);return mixedPdf(out,fileName,landscape)}
