/* E-REPORT/SAGS V2.2.47 · FIXED PERMANENT TOP TOOLBAR ROW */
...
(function(root){'use strict';
  ...
  body.v38-clean-workflow #v324FormActions{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:8px!important;order:10!important}
...
Phần của ${who} được ghi “BỎ QUA · KHÔNG E-FORM”, không ghi HOÀN THÀNH giả.`)}catch(e){alert('Không nhận thay được: '+S(e?.message||e))}};
  ...
  let rowRepairTimer=0;
  function ensureActionRowAttached(){
    const bar=document.querySelector('.toolbar'),row=document.getElementById('v324FormActions'),nav=document.getElementById('v38CleanNav');
    if(!bar)return;
    if(row&&row.parentElement===bar&&(!nav||row.nextElementSibling===nav))return;
    clearTimeout(rowRepairTimer);rowRepairTimer=setTimeout(()=>syncFormActions().catch(e=>console.info('Toolbar row repair',e?.message||e)),40);
  }
  function install(){ensureCss();patchList();patchMulti();patchFormHooks();setTimeout(syncFormActions,120);if(!root.__SAGS_V2247_TOOLBAR_OBSERVER){root.__SAGS_V2247_TOOLBAR_OBSERVER=new MutationObserver(ensureActionRowAttached);root.__SAGS_V2247_TOOLBAR_OBSERVER.observe(document.body,{childList:true,subtree:true})}}
