...
(function(root){'use strict';
  ...
  async function claimStateFor(item,man){const st=await sessionState(item.assignmentId),cs=U(st.claimStatus);if(cs==='PART_COMPLETED'||cs==='COMPLETED'||cs==='HANDED_OVER')return {state:'DONE',st};if(cs==='CLAIMED'&&norm(st.claimedBy||st.ownerUser)===me())return {state:'CLAIMED',st};if(firstItem(man,item)||st.workPartReady===true||st.handoverReady===true||cs==='READY')return {state:'READY',st};return {state:'WAIT',st}}
  async function decorateList(d=opDate()){
    ...
    const u=me();for(const card of host.querySelectorAll('.fwcFlight')){
      ...
      let b=card.querySelector('.v324ClaimBadge');if(!b){b=document.createElement('span');b.className='v324ClaimBadge';card.querySelector('.v38Flags,.fwcFlightTitle')?.appendChild?.(b)}if(b){b.className='v324ClaimBadge'+(cs.state==='WAIT'?' wait':cs.state==='DONE'?' done':'');b.textContent=cs.state==='CLAIMED'?'ĐANG LÀM':cs.state==='DONE'?'ĐÃ HOÀN TẤT':cs.state==='WAIT'?'CHỜ PHẦN TRƯỚC':'CHƯA NHẬN'}
    }
  ...
  async function syncFormActions(){ensureCss();const bar=document.querySelector('.toolbar');if(!bar)return;let row=document.getElementById('v324FormActions');if(!row){row=document.createElement('div');row.id='v324FormActions';row.innerHTML='<button id="v324ExportBtn" class="v324FormAction v324Export" type="button">📤 XUẤT / CHIA SẺ</button><button id="v324HandoverBtn" class="v324FormAction v324Handover" type="button"></button>';const nav=document.getElementById('v38CleanNav');if(nav)nav.insertAdjacentElement('beforebegin',row);else bar.appendChild(row);document.getElementById('v324ExportBtn').onclick=()=>{if(typeof root.openExportChoiceMenu==='function')root.openExportChoiceMenu();else alert('Chức năng Xuất/Chia sẻ chưa sẵn sàng.')}}
    let ctx=null;try{ctx=await currentHandoverContext()}catch(_){}if(!ctx){row.classList.remove('show','one');return}row.classList.add('show');row.classList.remove('one');const hb=document.getElementById('v324HandoverBtn');hb.style.display='block';hb.textContent='✓ HOÀN TẤT PHẦN CỦA TÔI';hb.title=ctx.next?`Hoàn tất phần hiện tại; phần tiếp theo sẽ sẵn sàng cho ${norm(ctx.next.user||ctx.next.targetUser)}`:'Hoàn tất phần công việc hiện tại';hb.onclick=()=>root.v324ConfirmRosterHandover?.()
  }

  root.v324ConfirmRosterHandover=async function(){let ctx;try{ctx=await currentHandoverContext();if(!ctx)throw new Error('Không xác định được phần công việc đang thực hiện.');const from=me(),to=ctx.next?norm(ctx.next.user||ctx.next.targetUser):'';const nextText=to?`\nPhần công việc tiếp theo sẽ sẵn sàng cho ${to} theo Daily Roster.`:'\nĐây là phần cuối của assignment hiện tại.';if(!confirm(`HOÀN TẤT PHẦN CỦA TÔI\n\n${rosterLabel(ctx.item)} · ${formLabel(ctx.item.formGroup)}\n\nDữ liệu hiện tại sẽ được lưu lại và phần của ${from} được đánh dấu HOÀN TẤT.${nextText}`))return;try{document.activeElement?.blur?.()}catch(_){}await new Promise(r=>setTimeout(r,30));try{root.persist?.()}catch(_){}await new Promise(r=>setTimeout(r,100));let env={};try{env=root.readFlightSessionEnvelope?.(ctx.meta.id)||{}}catch(_){}env=sanitizeEnvelope(env);const t=Date.now(),aid=S(ctx.item.assignmentId),patch={};patch[`roster_sessions/${safe(aid)}/claimStatus`]='PART_COMPLETED';patch[`roster_sessions/${safe(aid)}/workPartStatus`]='COMPLETED';patch[`roster_sessions/${safe(aid)}/completedAtMs`]=t;patch[`roster_sessions/${safe(aid)}/completedBy`]=from;patch[`roster_sessions/${safe(aid)}/completionEnvelope`]=env;patch[`roster_sessions/${safe(aid)}/updatedAtMs`]=t;let nextAid='';if(ctx.next){nextAid=S(ctx.next.assignmentId);patch[`roster_sessions/${safe(nextAid)}/engine`]='daily-roster-v2';patch[`roster_sessions/${safe(nextAid)}/schema`]=1;patch[`roster_sessions/${safe(nextAid)}/assignmentId`]=nextAid;patch[`roster_sessions/${safe(nextAid)}/ownerUser`]=to;patch[`roster_sessions/${safe(nextAid)}/formGroup`]=S(ctx.next.formGroup||env.mainForm);patch[`roster_sessions/${safe(nextAid)}/envelope`]=env;patch[`roster_sessions/${safe(nextAid)}/workPartReady`]=true;patch[`roster_sessions/${safe(nextAid)}/handoverReady`]=true;patch[`roster_sessions/${safe(nextAid)}/previousPartUser`]=from;patch[`roster_sessions/${safe(nextAid)}/previousPartCompletedAtMs`]=t;patch[`roster_sessions/${safe(nextAid)}/claimStatus`]='READY';patch[`roster_sessions/${safe(nextAid)}/updatedAtMs`]=t;patch[`roster_sessions/${safe(nextAid)}/updatedBy`]=from;}const ev=`WORK_PART_${t}_${safe(aid)}`;patch[`flight_records/${safe(ctx.d)}/${safe(ctx.item.flightId)}/workPartHistory/${safe(ev)}`]={eventId:ev,assignmentId:aid,nextAssignmentId:nextAid||null,fromUser:from,nextUser:to||null,atMs:t,type:'WORK_PART_COMPLETED',status:'COMPLETED',draftSaved:true,formGroup:S(ctx.item.formGroup),sourceColumn:S(ctx.item.sourceColumn)};patch[`flight_records/${safe(ctx.d)}/${safe(ctx.item.flightId)}/taskClaims/${safe(from)}/${safe(aid)}/status`]='PART_COMPLETED';patch[`flight_records/${safe(ctx.d)}/${safe(ctx.item.flightId)}/taskClaims/${safe(from)}/${safe(aid)}/completedAtMs`]=t;patch[`flight_records/${safe(ctx.d)}/${safe(ctx.item.flightId)}/taskClaims/${safe(from)}/${safe(aid)}/nextUser`]=to||null;await db('').update(patch);multiRemove(ctx.item.flightId,ctx.d);await addAudit('WORK_PART_COMPLETED',{flightId:S(ctx.item.flightId),flightLabel:rosterLabel(ctx.item),assignmentId:aid,nextAssignmentId:nextAid||null,fromUser:from,nextUser:to||null,formGroup:S(ctx.item.formGroup),sourceColumn:S(ctx.item.sourceColumn),draftSaved:true});alert(to?`✓ ĐÃ HOÀN TẤT PHẦN CỦA TÔI.\nDữ liệu đã lưu; ${to} sẽ thấy phần tiếp theo trong MY FLIGHT và bấm NHẬN CHUYẾN.`:'✓ ĐÃ HOÀN TẤT PHẦN CỦA TÔI.\nDữ liệu hiện tại đã được lưu.');try{root.flightWorkspaceOpenList?.(ctx.d)}catch(_){}setTimeout(()=>decorateList(ctx.d),250)}catch(e){alert('Không hoàn tất được phần công việc: '+S(e?.message||e))}};
