...
(function(root){'use strict';
 ...
 const groups=[
  ...
  {key:'monitor',title:'🛡 GIÁM SÁT & HỒ SƠ',sub:'Nhật ký, tài nguyên hệ thống và lưu trữ',items:[
    ...
    ['roleBtnPilotControl','🛡 PILOT CONTROL','Go/No-Go · Critical Data · BCP/Fallback · Issue Log · Control Audit']
  ]}
 ];
 const hideIds=['roleBtnDailyRoster','roleBtnRosterFlights','roleBtnActivity','roleBtnAcLimits','roleBtnFleet','roleBtnAccounts','roleBtnAdminBuilder','roleBtnAudit','roleBtnFirebaseUsage','roleBtnArchive','roleBtnPilotControl'];
 function ensure(){
...
`;document.head.appendChild(st)}
  ...
  function ensureCleanNav(){
    ...
    nav.innerHTML=`<button class="v38NavBtn flights" id="v38NavFlights">✈ CHUYẾN</button><button class="v38NavBtn multi" id="v38NavMulti">⇄ MULTITASK</button>${shiftAvailable?'<button class="v38NavBtn shift" id="v310ShiftNav">↔ GIAO CA</button>':''}${signAvailable?'<button class="v38NavBtn sign" id="v38NavSignature">✍ KÝ TÊN</button>':''}<span class="v38NavSpacer"></span><span id="v38FlowHint">FLIGHT WORKSPACE · V3.23</span>${isAD()?'<button class="v38NavBtn admin" id="v38NavAdmin">⚙ QUẢN LÝ</button>':''}`;
...
(function(root){'use strict';
  ...
  async function pilotAudit(event,detail,reason=''){try{const a=actor(),ctx={flightId:S(root.activeFlightSessionId||''),flightLabel:S(root.currentFlightSessionMeta?.()?.name||'')};await ref('pilot_control_v37/audit').push({schema:1,event:S(event),systemTimestamp:root.firebase?.database?.ServerValue?.TIMESTAMP||now(),clientAtMs:now(),actor:a,flightId:ctx.flightId,flightLabel:ctx.flightLabel,reason:S(reason),detail:detail||{}})}catch(e){console.warn('V3.10 audit',e)}}
 ...
 * - A/C LIMITS image AI review -> AD confirm save
...
(function(root){'use strict';
  ...
  async function audit(event,detail){try{await dbref('pilot_control_v37/audit').push({schema:1,event,systemTimestamp:root.firebase?.database?.ServerValue?.TIMESTAMP||Date.now(),clientAtMs:Date.now(),actor:{username:me(),name:S(profile().name||profile().fullName||me()),role:role(),departmentCode:depOf(profile()),groupCode:groupOf(profile())},detail:detail||{}})}catch(e){console.warn('V3.20 audit',e)}}
...
#v320RampModal,#v320AclAiModal{display:none;position:fixed;inset:0;z-index:19050;background:rgba(0,0,0,.55);align-items:center;justify-content:center;padding:12px;box-sizing:border-box}#v320RampModal.show,#v320AclAiModal.show{display:flex}.v320ModalPanel{width:min(94vw,720px);max-height:90svh;overflow:auto;background:#fff;border-radius:15px;padding:14px;box-sizing:border-box}.v320ModalTop{display:flex;justify-content:space-between;gap:10px;align-items:center}.v320ModalTop h3{margin:0;color:#0b4f91}.v320Btn{border:0;border-radius:9px;padding:9px 12px;background:#0b67b2;color:#fff;font:900 12px Arial;cursor:pointer}.v320Btn.gray{background:#e8eef3;color:#31485a}.v320Btn.green{background:#15803d}.v320Select,.v320Input{width:100%;box-sizing:border-box;padding:9px;border:1px solid #cbd7e0;border-radius:9px;background:#fff}.v320AiRows{display:grid;gap:8px;margin-top:9px}.v320AiRow{display:grid;grid-template-columns:28px 110px 115px 155px minmax(180px,1fr) 130px;gap:6px;align-items:center;border:1px solid #dce5ec;border-radius:9px;padding:7px;background:#f9fbfd}.v320AiRow input,.v320AiRow select{width:100%;box-sizing:border-box;padding:7px;border:1px solid #ccd7df;border-radius:7px;background:#fff}.v320AiStatus{margin-top:8px;padding:8px;border-radius:8px;background:#eef6ff;color:#294c68;font:700 11px/1.45 Arial;white-space:pre-wrap}
#aclSimplePanel .v320AclAiBox{margin:10px 0;padding:10px;border:2px solid #7c3aed;border-radius:12px;background:#faf7ff}.v320AclAiBox .v320Btn{background:#6d28d9}
@media(max-width:700px){#v320NaContext.show{display:grid}.v320Grid{grid-template-columns:1fr}.v320AiRow{grid-template-columns:28px 1fr 1fr}.v320AiRow .wide{grid-column:2 / 4}.v320ModalPanel{max-height:92svh}.v320RampBtn{width:100%}}
@media print{#v320NaContext,.v320RampBtn,#v320RampModal,#v320AclAiModal{display:none!important}}
`;
    document.head.appendChild(st);
  }
  ...
  async function enhanceWorkspace(fid){
    ...
    const myRamp=Object.values(man?.items||{}).filter(x=>x&&x.active!==false&&S(x.flightId)===fid&&norm(x.user||x.targetUser)===me()&&['FSAGS','FSAGS421','FSAGS551'].includes(U(x.formGroup)));
  ...
  /* A/C LIMITS image AI: review first; AD explicitly confirms save. */
  function ensureAclAiBox(){
    if(role()!=='AD')return;const panel=document.getElementById('aclSimplePanel');if(!panel||panel.querySelector('.v320AclAiBox'))return;
    const box=document.createElement('div');box.className='v320AclAiBox';box.innerHTML='<div style="font:900 13px Arial;color:#5b21b6;margin-bottom:5px">📷 ĐỌC FILE ẢNH / AI PHÂN TÍCH</div><div class="v320Sub">AI chỉ đọc và tạo PREVIEW. AD kiểm tra/sửa rồi bấm LƯU VÀO HỒ SƠ LIMIT; đây là bước lưu cuối cùng, không cần bấm LƯU LIMIT lần hai.</div><input id="v320AclAiFile" type="file" accept="image/*" style="display:none"><button class="v320Btn" type="button" id="v320AclAiPick">CHỌN ẢNH LIMIT</button><div class="v320AiStatus" id="v320AclAiInlineStatus">Chưa chọn ảnh.</div>';
    panel.querySelector('.acls-top')?.insertAdjacentElement('afterend',box);document.getElementById('v320AclAiPick').onclick=()=>document.getElementById('v320AclAiFile')?.click();document.getElementById('v320AclAiFile').onchange=e=>root.v320AclAiRead?.(e.target.files?.[0]);
  }
  const baseAclOpen=root.aclSimpleOpen;if(typeof baseAclOpen==='function')root.aclSimpleOpen=function(){const r=baseAclOpen.apply(this,arguments);setTimeout(ensureAclAiBox,30);return r};
  function ensureAclAiModal(){if(document.getElementById('v320AclAiModal'))return;ensureStyle();const d=document.createElement('div');d.id='v320AclAiModal';d.innerHTML='<div class="v320ModalPanel"><div class="v320ModalTop"><h3>AI PREVIEW · A/C LIMITS</h3><button class="v320Btn gray" onclick="v320AclAiClose()">ĐÓNG</button></div><div class="v320AiStatus" id="v320AclAiStatus">Đang phân tích...</div><div class="v320AiRows" id="v320AclAiRows"></div><div style="margin-top:10px"><button class="v320Btn green" id="v320AclAiSave" onclick="v320AclAiConfirmSave()">✓ LƯU VÀO HỒ SƠ LIMIT</button></div></div>';document.body.appendChild(d)}
  let aclAiRows=[];
  function fileDataUrl(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result||''));r.onerror=()=>reject(new Error('Không đọc được file ảnh.'));r.readAsDataURL(file)})}
  root.v320AclAiRead=async function(file){if(role()!=='AD'||!file)return;ensureAclAiModal();document.getElementById('v320AclAiModal').classList.add('show');document.getElementById('v320AclAiStatus').textContent='AI đang đọc ảnh LIMIT…';document.getElementById('v320AclAiRows').innerHTML='';try{if(typeof root.acLimitsAiParseImage!=='function')throw new Error('Module AI LIMIT chưa tải. Hãy kiểm tra ai.js/cấu hình AI.');const dataUrl=await fileDataUrl(file),out=await root.acLimitsAiParseImage(dataUrl);const date=S(out?.date)||iso(),version=S(out?.version);aclAiRows=[];for(const item of out?.items||[]){const regs=(Array.isArray(item.aircraftRegs)?item.aircraftRegs:[]).filter(Boolean);if(regs.length)for(const reg of regs)aclAiRows.push({flightNo:S(item.flightNo),acReg:S(reg),category:S(item.category)||'OTHERS',restriction:S(item.restriction),effectiveFrom:S(item.effectiveFrom||date),effectiveTo:S(item.effectiveTo||date)});else aclAiRows.push({flightNo:S(item.flightNo),acReg:S(item.acReg),category:S(item.category)||'OTHERS',restriction:S(item.restriction),effectiveFrom:S(item.effectiveFrom||date),effectiveTo:S(item.effectiveTo||date)});}aclAiRows=aclAiRows.filter(x=>x.restriction&&(x.flightNo||x.acReg));if(!aclAiRows.length)throw new Error('AI chưa tách được dòng LIMIT đủ thông tin để preview.');renderAclAiRows();document.getElementById('v320AclAiStatus').textContent=`Đọc được ${aclAiRows.length} dòng${version?' · version '+version:''}${out?.model?' · '+out.model:''}${out?.fallbackUsed?' · FALLBACK':''}. AD kiểm tra/sửa; chỉ dòng được tích mới lưu.`}catch(e){document.getElementById('v320AclAiStatus').textContent='AI LIMIT chưa đọc được: '+S(e?.message||e)}};
  function renderAclAiRows(){const host=document.getElementById('v320AclAiRows');if(!host)return;host.innerHTML=aclAiRows.map((x,i)=>`<div class="v320AiRow"><input type="checkbox" id="v320AclUse_${i}" checked><input id="v320AclFlight_${i}" value="${esc(x.flightNo)}" placeholder="Flight"><input id="v320AclReg_${i}" value="${esc(x.acReg)}" placeholder="A/C Reg"><select id="v320AclCat_${i}"><option ${U(x.category)==='APU INOP'?'selected':''}>APU INOP</option><option ${U(x.category).startsWith('HOLD')?'selected':''}>HOLD INOP/ISSUES</option><option ${U(x.category)==='SEAT INOP'?'selected':''}>SEAT INOP</option><option ${!['APU INOP','SEAT INOP'].includes(U(x.category))&&!U(x.category).startsWith('HOLD')?'selected':''}>OTHERS</option></select><input class="wide" id="v320AclText_${i}" value="${esc(x.restriction)}" placeholder="Nội dung LIMIT"><input type="date" id="v320AclDate_${i}" value="${esc(x.effectiveFrom||iso())}"></div>`).join('')}
  root.v320AclAiClose=function(){document.getElementById('v320AclAiModal')?.classList.remove('show')};
  root.v320AclAiConfirmSave=async function(){if(role()!=='AD')return;const rows=aclAiRows.map((x,i)=>({use:!!document.getElementById(`v320AclUse_${i}`)?.checked,flightNo:S(document.getElementById(`v320AclFlight_${i}`)?.value),acReg:S(document.getElementById(`v320AclReg_${i}`)?.value),category:S(document.getElementById(`v320AclCat_${i}`)?.value)||'OTHERS',restriction:S(document.getElementById(`v320AclText_${i}`)?.value),date:S(document.getElementById(`v320AclDate_${i}`)?.value)||iso()})).filter(x=>x.use&&x.restriction&&(x.flightNo||x.acReg));if(!rows.length)return alert('Không có dòng AI nào được chọn để lưu.');const b=document.getElementById('v320AclAiSave');if(b)b.disabled=true;let saved=0;try{if(typeof root.aclSaveManual!=='function')throw new Error('Engine A/C LIMITS thủ công chưa sẵn sàng.');for(const x of rows){const ids=['aclManFlight','aclManReg','aclManMode','aclManCat','aclManFrom','aclManTo','aclManText'];if(ids.some(id=>!document.getElementById(id)))throw new Error('Form A/C LIMITS nền chưa sẵn sàng.');document.getElementById('aclManFlight').value=x.flightNo;document.getElementById('aclManReg').value=x.acReg;document.getElementById('aclManMode').value=x.flightNo&&x.acReg?'BOTH':x.acReg?'REG':'FLIGHT';document.getElementById('aclManCat').value=x.category;document.getElementById('aclManFrom').value=x.date;document.getElementById('aclManTo').value=x.date;document.getElementById('aclManText').value=x.restriction;document.querySelectorAll('#aclPaneManual input[data-acl-role]').forEach(c=>c.checked=true);await root.aclSaveManual();saved++;await new Promise(r=>setTimeout(r,80));}await audit('AC_LIMITS_AI_CONFIRMED',{saved});root.v320AclAiClose();document.getElementById('v320AclAiInlineStatus').textContent=`✓ AD đã xác nhận lưu ${saved} dòng AI. Có thể mở danh sách LIMIT để kiểm tra.`;try{root.ACLSimple?.refresh?.()}catch(_){}alert(`ĐÃ LƯU ${saved} A/C LIMITS từ AI sau xác nhận của AD.`)}catch(e){alert('Không lưu được AI LIMIT: '+S(e?.message||e))}finally{if(b)b.disabled=false}};

  function sync(){ensureStyle();prepareRosterUi();refreshNa();ensureAclAiBox();const h=document.getElementById('v38FlowHint');if(h)h.textContent='FLIGHT WORKSPACE · V3.20';const nav=document.getElementById('v38NavFlights');if(nav)nav.onclick=()=>root.flightWorkspaceOpenList?.(currentOpDate())}
...
(function(root){'use strict';
  ...
  function myItems(man,fid){const u=me();return itemsOf(man).filter(x=>S(x.flightId)===S(fid)&&norm(x.user||x.targetUser)===u)}
  function laneKey(x){return [S(x?.flightId),U(x?.sourceColumn),U(x?.roleKey),U(x?.formGroup)].join('|')}
  function lane(man,item){const k=laneKey(item);return itemsOf(man).filter(x=>laneKey(x)===k)}
  ...
  function addAudit(type,detail){try{return db('pilot_control_v37/audit').push({schema:1,event:type,systemTimestamp:root.firebase?.database?.ServerValue?.TIMESTAMP||Date.now(),clientAtMs:Date.now(),actor:{username:me(),name:S(profile().name||profile().fullName||me()),role:role()},flightId:S(detail?.flightId),flightLabel:S(detail?.flightLabel),detail:detail||{}})}catch(_){}}
  ...
  .v324ClaimBadge{display:inline-flex;align-items:center;border-radius:999px;padding:3px 7px;margin-left:5px;font:900 10px Arial;background:#e7f2ff;color:#07599d}.v324ClaimBadge.wait{background:#fff3cd;color:#855a00}.v324ClaimBadge.done{background:#e8f5ed;color:#17663b}
  ...
  .fwcFlight .v324ReceiveBtn[disabled]{background:#e5eaee!important;color:#667785!important;cursor:not-allowed!important}.fwcFlight .v324ReceiveBtn.wait{background:#fff4d6!important;color:#7c5600!important}.fwcFlight .v324ReceiveBtn.done{background:#eef2f5!important;color:#5b6c79!important}
  @media(max-width:520px){#v324FormActions.show{grid-template-columns:1fr 1.2fr}.v324FormAction{font-size:10px;padding:5px 6px}}
  `;document.head.appendChild(st)}

  async function claimStateFor(item,man){const st=await sessionState(item.assignmentId);if(U(st.claimStatus)==='HANDED_OVER')return {state:'HANDED_OVER',st};if(U(st.claimStatus)==='CLAIMED'&&norm(st.claimedBy||st.ownerUser)===me())return {state:'CLAIMED',st};if(firstItem(man,item)||st.handoverReady===true||U(st.claimStatus)==='READY')return {state:'READY',st};return {state:'WAIT',st}}
  async function decorateList(d=opDate()){
    ...
    const u=me();for(const card of host.querySelectorAll('.fwcFlight')){
      ...
      const primary=mine[0],cs=await claimStateFor(primary,man);old.classList.add('v324ReceiveBtn');old.onclick=null;old.removeAttribute('onclick');old.disabled=false;old.classList.remove('wait','done');
      if(cs.state==='CLAIMED'){old.textContent='TIẾP TỤC';old.onclick=()=>root.v324ReceiveOrOpen(fid);}
      else if(cs.state==='HANDED_OVER'){old.textContent='ĐÃ GIAO CA';old.disabled=true;old.classList.add('done')}
      else if(cs.state==='WAIT'){old.textContent='CHỜ GIAO CA';old.disabled=true;old.classList.add('wait')}
      else {old.textContent='NHẬN CHUYẾN';old.onclick=()=>root.v324ReceiveOrOpen(fid);}
      let b=card.querySelector('.v324ClaimBadge');if(!b){b=document.createElement('span');b.className='v324ClaimBadge';card.querySelector('.v38Flags,.fwcFlightTitle')?.appendChild?.(b)}if(b){b.className='v324ClaimBadge'+(cs.state==='WAIT'?' wait':cs.state==='HANDED_OVER'?' done':'');b.textContent=cs.state==='CLAIMED'?'ĐANG LÀM':cs.state==='HANDED_OVER'?'ĐÃ GIAO CA':cs.state==='WAIT'?'CHỜ CA TRƯỚC':'CHƯA NHẬN'}
    }
  ...
  async function markClaim(d,fid,item){const t=Date.now(),u=me(),p=profile(),patch={};patch[`roster_sessions/${safe(item.assignmentId)}/claimStatus`]='CLAIMED';patch[`roster_sessions/${safe(item.assignmentId)}/claimedAtMs`]=t;patch[`roster_sessions/${safe(item.assignmentId)}/claimedBy`]=u;patch[`roster_sessions/${safe(item.assignmentId)}/handoverReady`]=false;patch[`roster_sessions/${safe(item.assignmentId)}/updatedAtMs`]=t;patch[`flight_records/${safe(d)}/${safe(fid)}/taskClaims/${safe(u)}/${safe(item.assignmentId)}`]={assignmentId:S(item.assignmentId),username:u,name:S(p.name||p.fullName||u),formGroup:S(item.formGroup),sourceColumn:S(item.sourceColumn),status:'CLAIMED',claimedAtMs:t,updatedAtMs:t};await db('').update(patch);await addAudit('MY_FLIGHT_CLAIMED',{flightId:fid,flightLabel:rosterLabel(item),assignmentId:S(item.assignmentId),formGroup:S(item.formGroup),sourceColumn:S(item.sourceColumn)});}
  root.v324ReceiveOrOpen=async function(fid){fid=S(fid);if(!fid)return;const d=opDate();try{const man=await manifest(d),mine=myItems(man,fid);if(!mine.length)throw new Error('Chuyến này không thuộc MY FLIGHT của tài khoản hiện tại.');let item=mine[0],cs=await claimStateFor(item,man);if(cs.state==='HANDED_OVER')throw new Error('Phần việc của bạn trên chuyến này đã bàn giao cho ca sau.');if(cs.state==='WAIT'){const prev=previousItem(man,item);throw new Error(`Ca trước ${norm(prev?.user||prev?.targetUser)||'chưa xác định'} chưa bấm XÁC NHẬN GIAO CA.`)}if(cs.state!=='CLAIMED'){await markClaim(d,fid,item);multiAdd(fid,d)}else multiAdd(fid,d);await openAssignment(item);setTimeout(()=>decorateList(d),300)}catch(e){alert('Không mở được MY FLIGHT: '+S(e?.message||e))}};
  ...
  async function syncFormActions(){ensureCss();const bar=document.querySelector('.toolbar');if(!bar)return;let row=document.getElementById('v324FormActions');if(!row){row=document.createElement('div');row.id='v324FormActions';row.innerHTML='<button id="v324ExportBtn" class="v324FormAction v324Export" type="button">📤 XUẤT / CHIA SẺ</button><button id="v324HandoverBtn" class="v324FormAction v324Handover" type="button"></button>';const nav=document.getElementById('v38CleanNav');if(nav)nav.insertAdjacentElement('beforebegin',row);else bar.appendChild(row);document.getElementById('v324ExportBtn').onclick=()=>{if(typeof root.openExportChoiceMenu==='function')root.openExportChoiceMenu();else alert('Chức năng Xuất/Chia sẻ chưa sẵn sàng.')}}
    let ctx=null;try{ctx=await currentHandoverContext()}catch(_){}if(!ctx){row.classList.remove('show','one');return}row.classList.add('show');const hb=document.getElementById('v324HandoverBtn');if(ctx.next){row.classList.remove('one');hb.style.display='block';const to=norm(ctx.next.user||ctx.next.targetUser);hb.textContent=`↪ XÁC NHẬN GIAO CA → ${to}`;hb.onclick=()=>root.v324ConfirmRosterHandover?.()}else{row.classList.add('one');hb.style.display='none'}
  }

  root.v324ConfirmRosterHandover=async function(){let ctx;try{ctx=await currentHandoverContext();if(!ctx||!ctx.next)throw new Error('Daily Roster không có người làm tiếp theo cho assignment này.');const to=norm(ctx.next.user||ctx.next.targetUser),from=me();if(!confirm(`XÁC NHẬN GIAO CA\n\n${from} → ${to}\n${rosterLabel(ctx.item)} · ${formLabel(ctx.item.formGroup)}\n\nDữ liệu hiện tại sẽ được lưu tạm và bàn giao cho người kế tiếp.`))return;try{document.activeElement?.blur?.()}catch(_){}await new Promise(r=>setTimeout(r,30));try{root.persist?.()}catch(_){}await new Promise(r=>setTimeout(r,100));let env={};try{env=root.readFlightSessionEnvelope?.(ctx.meta.id)||{}}catch(_){}env=sanitizeEnvelope(env);const t=Date.now(),nextAid=S(ctx.next.assignmentId),aid=S(ctx.item.assignmentId),patch={};patch[`roster_sessions/${safe(aid)}/claimStatus`]='HANDED_OVER';patch[`roster_sessions/${safe(aid)}/handedOverAtMs`]=t;patch[`roster_sessions/${safe(aid)}/handedOverTo`]=to;patch[`roster_sessions/${safe(aid)}/updatedAtMs`]=t;patch[`roster_sessions/${safe(nextAid)}/engine`]='daily-roster-v2';patch[`roster_sessions/${safe(nextAid)}/schema`]=1;patch[`roster_sessions/${safe(nextAid)}/assignmentId`]=nextAid;patch[`roster_sessions/${safe(nextAid)}/ownerUser`]=to;patch[`roster_sessions/${safe(nextAid)}/formGroup`]=S(ctx.next.formGroup||env.mainForm);patch[`roster_sessions/${safe(nextAid)}/envelope`]=env;patch[`roster_sessions/${safe(nextAid)}/handoverReady`]=true;patch[`roster_sessions/${safe(nextAid)}/handoverFromUser`]=from;patch[`roster_sessions/${safe(nextAid)}/handoverAtMs`]=t;patch[`roster_sessions/${safe(nextAid)}/claimStatus`]='READY';patch[`roster_sessions/${safe(nextAid)}/updatedAtMs`]=t;patch[`roster_sessions/${safe(nextAid)}/updatedBy`]=from;const ev=`ROSTER_HANDOVER_${t}_${safe(aid)}`;patch[`flight_records/${safe(ctx.d)}/${safe(ctx.item.flightId)}/rampTransferHistory/${safe(ev)}`]={eventId:ev,assignmentId:aid,nextAssignmentId:nextAid,fromUser:from,toUser:to,atMs:t,type:'ROSTER_SEQUENCE_HANDOVER',status:'DRAFT_SAVED'};patch[`flight_records/${safe(ctx.d)}/${safe(ctx.item.flightId)}/taskClaims/${safe(from)}/${safe(aid)}/status`]='HANDED_OVER';patch[`flight_records/${safe(ctx.d)}/${safe(ctx.item.flightId)}/taskClaims/${safe(from)}/${safe(aid)}/handedOverAtMs`]=t;patch[`flight_records/${safe(ctx.d)}/${safe(ctx.item.flightId)}/taskClaims/${safe(from)}/${safe(aid)}/handedOverTo`]=to;await db('').update(patch);multiRemove(ctx.item.flightId,ctx.d);await addAudit('ROSTER_SEQUENCE_HANDOVER',{flightId:S(ctx.item.flightId),flightLabel:rosterLabel(ctx.item),assignmentId:aid,nextAssignmentId:nextAid,fromUser:from,toUser:to,formGroup:S(ctx.item.formGroup),draftSaved:true});alert(`ĐÃ GIAO CA · ${from} → ${to}.\nDữ liệu hiện tại đã lưu tạm; ${to} vào MY FLIGHT → NHẬN CHUYẾN để làm tiếp.`);try{root.flightWorkspaceOpenList?.(ctx.d)}catch(_){}setTimeout(()=>decorateList(ctx.d),250)}catch(e){alert('Không giao ca được: '+S(e?.message||e))}};

  // AI PREVIEW save is the final save. After success, clear the manual form and move directly to the archive list.
  function patchAiSave(){const fn=root.v320AclAiConfirmSave;if(typeof fn!=='function'||fn.__v324)return;const w=async function(){const before=document.getElementById('v320AclAiModal')?.classList.contains('show');await fn.apply(this,arguments);if(before&&!document.getElementById('v320AclAiModal')?.classList.contains('show')){try{root.aclSimpleClear?.()}catch(_){}try{await root.ACLSimple?.refresh?.()}catch(_){}const st=document.getElementById('v320AclAiInlineStatus');if(st)st.textContent='✓ ĐÃ LƯU TRỰC TIẾP VÀO HỒ SƠ LIMIT. Không cần bấm LƯU LIMIT lần hai.';setTimeout(()=>document.getElementById('aclSList')?.scrollIntoView?.({behavior:'smooth',block:'start'}),100)}};w.__v324=1;root.v320AclAiConfirmSave=w}
...
(function(root){
  ...
  async function audit(event,detail){try{const p=profile();await db('pilot_control_v37/audit').push({schema:1,event,systemTimestamp:root.firebase?.database?.ServerValue?.TIMESTAMP||Date.now(),clientAtMs:Date.now(),actor:{username:me(),name:S(p.name||p.fullName||me()),role:role()},flightId:S(detail?.flightId),flightLabel:S(detail?.flightLabel),detail:detail||{}})}catch(e){console.info('V3.27 audit',e?.message||e)}}
...
`;document.head.appendChild(st)}
  ...
  root.v327Reassign=async function(aid){if(!canReassign())return;aid=S(aid);const item=cache.manifest?.items?.[aid];if(!item)return alert('Không tìm thấy assignment. Hãy tải lại.');const sel=document.getElementById(`v327To_${safe(aid)}`),target=norm(sel?.value),old=norm(item.user||item.targetUser);if(!target)return alert('Chọn người mới.');if(target===old)return alert('Người mới đang là người phụ trách hiện tại.');const oldP=cache.users.find(x=>norm(x.username)===old),toP=cache.users.find(x=>norm(x.username)===target);if(!toP||toP.active===false)return alert('Tài khoản người mới không ACTIVE.');if(oldP&&!sameUnit(oldP,toP))return alert('Người mới phải cùng đơn vị với người đang được phân công.');if(!canTouchAssignment(item,cache.users))return alert('Tài khoản của bạn không được phân công lại assignment này.');if(!confirm(`ĐỔI NGƯỜI PHỤ TRÁCH\n\n${item.flightRaw||item.flightName||''} · ${labelForm(item.formGroup)}\n${old} → ${target}\n\nDaily Roster kế hoạch gốc vẫn được giữ. Người mới sẽ thấy chuyến trong MY FLIGHT và bấm NHẬN CHUYẾN.`))return;
    try{const d=cache.date||opDate(),t=Date.now(),actor=me();let payload=null;try{payload=(await db(`roster_mail/${safe(old)}/items/${safe(aid)}`).once('value')).val()}catch(_){}payload=payload||{engine:'daily-roster-v2',schema:2,assignmentId:aid,opDate:d,flightRaw:S(item.flightRaw),flightName:S(item.flightName),arrFlight:S(item.arrFlight),depFlight:S(item.depFlight),sta:S(item.sta),std:S(item.std),acReg:S(item.acReg),acType:S(item.acType),route:S(item.route),formGroup:S(item.formGroup),sourceColumn:S(item.sourceColumn),roleKey:S(item.roleKey),active:true};
  ...
  function disableLegacyHandover(){
    try{root.v310ShiftOpen=function(){alert('GIAO CA có duyệt đã ngừng dùng. Với tờ RAMP: vào biểu mẫu đang làm và bấm XÁC NHẬN GIAO CA → người kế tiếp trong Daily Roster. Không cần duyệt/tiếp nhận.')}}catch(_){}
    try{if(typeof root.openHandoverMenu==='function')root.openHandoverMenu=function(){alert('GIAO CA tờ RAMP thực hiện trực tiếp trong biểu mẫu bằng XÁC NHẬN GIAO CA.')}}catch(_){}
  }
