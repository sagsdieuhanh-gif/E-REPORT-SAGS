...
`;
  ...
  /* A/C LIMITS image AI: review first; authorized AC_LIMITS user explicitly confirms save. */
  function canAclManage(){return role()==='AD'||(typeof root.v485Can==='function'&&root.v485Can('AC_LIMITS'))}
  function ensureAclAiBox(){
    if(!canAclManage())return;const panel=document.getElementById('aclSimplePanel');if(!panel||panel.querySelector('.v320AclAiBox'))return;
    const box=document.createElement('div');box.className='v320AclAiBox';box.innerHTML='<div style="font:900 13px Arial;color:#5b21b6;margin-bottom:5px">📷 ĐỌC FILE ẢNH / AI PHÂN TÍCH</div><div class="v320Sub">AI chỉ đọc và tạo PREVIEW. Người được cấp quyền LIMIT kiểm tra/sửa rồi bấm LƯU VÀO HỒ SƠ LIMIT; đây là bước lưu cuối cùng, không cần bấm LƯU LIMIT lần hai.</div><input id="v320AclAiFile" type="file" accept="image/*" style="display:none"><button class="v320Btn" type="button" id="v320AclAiPick">CHỌN ẢNH LIMIT</button><div class="v320AiStatus" id="v320AclAiInlineStatus">Chưa chọn ảnh.</div>';
...
/* V3.28 · A/C LIMITS FULL PERMISSION */
(function(root){root.__SAGS_V328_BUILD='V3.28-20260821-01';root.__SAGS_V328_HDSD='Tài khoản được AD cấp quyền A/C LIMITS có toàn bộ chức năng LIMIT tương ứng: mở module, nhập/sửa/xóa hồ sơ, đọc file ảnh bằng AI, xem PREVIEW và LƯU VÀO HỒ SƠ LIMIT. Quyền được kiểm tra bằng AC_LIMITS ở cả giao diện và thao tác; không còn khóa cứng chỉ role AD.';})(typeof window!=='undefined'?window:globalThis);
