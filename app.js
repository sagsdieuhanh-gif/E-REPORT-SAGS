...
(function(root){'use strict';
  ...
  async function repairMyManualAssignments(){
    ...
    for(const [key,rec0] of Object.entries(raw)){if(!rec0||rec0.manualCreatedV340!==true)continue;const rec={...rec0,assignmentId:S(rec0.assignmentId||key)},group=canonicalFormGroup(rec.formGroup);if(!group)continue;const date=S(rec.opDate),aid=S(rec.assignmentId),fixed={...rec,formGroup:group,engine:'DAILY_ROSTER_V1',date:displayDate(rec.date||date)};if(S(rec.engine)!=='DAILY_ROSTER_V1')patch[`roster_mail/${safe(user)}/items/${safe(key)}/engine`]='DAILY_ROSTER_V1';if(S(rec.formGroup)!==group)patch[`roster_mail/${safe(user)}/items/${safe(key)}/formGroup`]=group;if(date&&aid){patch[`roster_manifests/${safe(date)}/items/${safe(aid)}/formGroup`]=group;patch[`roster_sessions/${safe(aid)}/formGroup`]=group;if(rec.flightId){patch[`flight_records/${safe(date)}/${safe(rec.flightId)}/assignments/${safe(aid)}/formGroup`]=group;patch[`flight_records/${safe(date)}/${safe(rec.flightId)}/taskStatus/${safe(aid)}/formGroup`]=group}}try{await ensureLocalSession(fixed)}catch(e){console.info('V3.41 repair local session',e?.message||e)}repaired++}
  ...
  function injectReleaseNote(){const panel=document.querySelector('#updateInfoModal>div');if(!panel||document.getElementById('v340ReleaseNote'))return;const d=document.createElement('div');d.id='v340ReleaseNote';d.innerHTML='<p style="margin:10px 0 4px"><b>V3.41 · 21/08/2026 — SỬA MỞ BIỂU MẪU CHUYẾN TẠO THỦ CÔNG</b></p><p style="margin:4px 0">• Chuyến tạo tay dùng đúng engine mailbox và mã form chuẩn; bấm NHẬN CHUYẾN/TIẾP TỤC sẽ mở ngay đúng FSAGS 42.3, 42.1, 55.1 hoặc FSAGS 09.</p><p style="margin:4px 0">• Chuyến đã tạo ở V3.40 được tự sửa và dựng lại biểu mẫu trên máy, không cần xóa chuyến hoặc tạo lại.</p><p style="margin:10px 0 4px"><b>V3.40 · 21/08/2026 — AUTO FINAL CBTT + TẠO CHUYẾN THỦ CÔNG</b></p><p style="margin:4px 0">• Grnd_Ls tự tạo đúng tờ FINAL; thiếu A/C Type/Fleet thì giữ CHỜ MẪU, không đoán.</p><p style="margin:4px 0">• Mọi đơn vị được tự tạo phần việc khi roster chưa đổ dữ liệu; luôn dùng chung Flight Record và chặn tạo trùng/vượt phân công.</p>';panel.prepend(d)}
