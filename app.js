  ...
  function enrichRosterPatch(patch){
    ...
    for(const [k,v] of Object.entries(patch||{})){
      ...
      for(const [aid,item0] of Object.entries(v.items||{})){
        ...
        const rec=records[fid]||(records[fid]={flightId:fid,opDate:date,flightRaw:S(item.flightRaw||mail.flightRaw),flightName:S(item.flightName||mail.flightName),arrFlight:S(get('arrFlight')),depFlight:S(get('depFlight')),sta:S(get('sta')),std:S(get('std')),eta:S(get('eta')),etd:S(get('etd')),arrFlightDate:S(get('arrFlightDate')||date),depFlightDate:S(get('depFlightDate')||date),etaFlightDate:S(get('etaFlightDate')||get('arrFlightDate')||date),etdFlightDate:S(get('etdFlightDate')||get('depFlightDate')||date),staClock:S(get('staClock')),stdClock:S(get('stdClock')),etaClock:S(get('etaClock')),etdClock:S(get('etdClock')),staDayOffset:Number(get('staDayOffset')||0),stdDayOffset:Number(get('stdDayOffset')||0),etaDayOffset:Number(get('etaDayOffset')||0),etdDayOffset:Number(get('etdDayOffset')||0),staSortMinute:Number(get('staSortMinute')),stdSortMinute:Number(get('stdSortMinute')),etaSortMinute:Number(get('etaSortMinute')),etdSortMinute:Number(get('etdSortMinute')),acReg:S(get('acReg')),acType:S(get('acType')),route:S(get('route')),bay:S(get('bay')),createdFrom:'DAILY_ROSTER',createdAtMs:Date.now(),updatedAtMs:Date.now(),assignments:{}});
        rec.assignments[aid]={assignmentId:aid,user:S(item.user||mail.targetUser),originalUser:S(item.originalUser||mail.originalTargetUser),formGroup:S(item.formGroup||mail.formGroup),sourceColumn:S(item.sourceColumn||mail.sourceColumn),roleKey:S(item.roleKey||mail.roleKey),workspaceKey:S(item.workspaceKey||item.rosterWorkspaceKey||mail.workspaceKey||mail.rosterWorkspaceKey),assignmentScope:S(item.assignmentScope||mail.assignmentScope||'BOTH'),workPartOrder:Number(item.workPartOrder||mail.workPartOrder||1),workPartTotal:Number(item.workPartTotal||mail.workPartTotal||1),workPartSequenceSource:S(item.workPartSequenceSource||mail.workPartSequenceSource||item.sourceColumn||mail.sourceColumn),active:item.active!==false};
      }
...
(function(root){'use strict';
  ...
  async function openAssignment(item){if(U(item?.formGroup)==='FINAL'){try{root.flightWorkspaceClose?.()}catch(_){}if(typeof root.sagsV338OpenFinalForRoster!=='function')throw new Error('Biểu mẫu FINAL chưa sẵn sàng.');await root.sagsV338OpenFinalForRoster(item);return;}if(U(item?.formGroup)==='UNIT_TASK'){root.flightWorkspaceOpenFlight?.(S(item.flightId));return;}let meta=localMeta(item.assignmentId);if(!meta){try{root.dailyRosterRestartMailbox?.()}catch(_){}await new Promise(r=>setTimeout(r,700));meta=localMeta(item.assignmentId)}if(!meta)throw new Error('Biểu mẫu chưa đồng bộ xuống thiết bị. Hãy chờ vài giây rồi thử lại.');await syncSharedIntoLocal(item,meta);try{root.flightWorkspaceClose?.()}catch(_){}root.switchFlightSession?.(meta.id);setTimeout(syncFormActions,120)}
...
(function(root){
  ...
  function laneKey(x){return [S(x?.flightId),U(x?.sourceColumn),U(x?.roleKey),U(x?.formGroup)].join('|')}
  function activeItems(man){return Object.values(man?.items||{}).filter(x=>x&&x.active!==false)}
...
(function(root){'use strict';
  ...
  root.sagsV340CreateManualFlight=async function(){
    ...
    try{
      const date=field('v340Date'),unit=U(field('v340Unit')),formGroup=U(field('v340Form')),arr=normFlight(field('v340Arr')),dep=normFlight(field('v340Dep')),sta=timeValue(field('v340Sta')),std=timeValue(field('v340Std')),acReg=U(field('v340Reg')),acType=U(field('v340Type')),route=U(field('v340Route')),bay=U(field('v340Bay')),user=me();
      ...
      const item=existing||{assignmentId:aid,user,originalUser:user,targetUser:user,flightId:fid,opDate:date,date,flightRaw,flightName,arrFlight:arr,depFlight:dep,sta,std,arrFlightDate:date,depFlightDate:date,staSortMinute:timeScore(sta),stdSortMinute:timeScore(std),acReg,acType,route,route1:routeParts[0]||'',route3:routeParts[routeParts.length-1]||'',bay,formGroup,sourceColumn,roleKey,workPartOrder:1,workPartTotal:1,workPartSequenceSource:sourceColumn,assignmentScope:'MANUAL_SELF',manualUnit:unit,manualCreatedV340:true,manualCreatedAtMs:now,manualCreatedBy:user,active:true};
      const payload={...item,engine:'daily-roster-v2',schema:2,targetUser:user,originalTargetUser:user,sourceFile:'MANUAL_V340',publishedAtMs:now,publishedBy:user,manualCreatedV340:true,active:true};
      ...
      await db('').update(patch);try{root.dailyRosterRestartMailbox?.()}catch(_){}if(U(item.formGroup)==='FINAL'&&typeof root.sagsV340EnsureFinalForRoster==='function')await root.sagsV340EnsureFinalForRoster({...payload,...item},{open:false});try{await root.sagsTaskStatusSyncDate?.(date,true)}catch(_){}
  ...
  function injectReleaseNote(){const panel=document.querySelector('#updateInfoModal>div');if(!panel||document.getElementById('v340ReleaseNote'))return;const d=document.createElement('div');d.id='v340ReleaseNote';d.innerHTML='<p style="margin:10px 0 4px"><b>V3.40 · 21/08/2026 — AUTO FINAL CBTT + TẠO CHUYẾN THỦ CÔNG</b></p><p style="margin:4px 0">• Grnd_Ls tự tạo đúng tờ FINAL; thiếu A/C Type/Fleet thì giữ CHỜ MẪU, không đoán.</p><p style="margin:4px 0">• Mọi đơn vị được tự tạo phần việc khi roster chưa đổ dữ liệu; luôn dùng chung Flight Record và chặn tạo trùng/vượt phân công.</p>';panel.prepend(d)}
  function install(){ensureUi();injectReleaseNote();const fn=root.flightWorkspaceOpenList;if(typeof fn==='function'&&!fn.__v340){const w=function(){const r=fn.apply(this,arguments);Promise.resolve(r).finally(()=>{setTimeout(injectButton,40);setTimeout(injectButton,420)});return r};w.__v340=true;w.__v340Base=fn;root.flightWorkspaceOpenList=w;try{flightWorkspaceOpenList=w}catch(_){}}setTimeout(injectButton,80)}
  install();setTimeout(install,400);setTimeout(install,1300);
  root.__SAGS_V340_MANUAL_TEST__={normFlight,flightTokens,matchesFlight,itemUnit,timeValue,timeScore,unitForProfile};
  root.__SAGS_V340_HDSD='V3.40: mọi đơn vị có nút TẠO CHUYẾN THỦ CÔNG khi roster chưa có dữ liệu. Hệ thống dò Flight đến/đi để dùng lại một Flight Record chung, chỉ tạo assignment của tài khoản/đơn vị hiện tại, chặn vượt phân công hoặc người phụ trách đã có, và giữ assignment thủ công qua các lần đồng bộ roster cho tới khi roster thật thay thế đúng chuyến/đơn vị.';
})(typeof window!=='undefined'?window:globalThis);
