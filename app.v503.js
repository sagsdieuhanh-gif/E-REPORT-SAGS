...
/* E-Report V4.88
   ...
   - AD daily .ereport export/import viewer.
*/
(function(){
"use strict";
...
async function v488LoadSnapshot(parent){
  // Fetch only this explicitly selected flight. Independent chunks can be read
  // concurrently, but bound concurrency to avoid overwhelming mobile devices.
  const count=Number(parent?.chunkCount);
  if(!Number.isSafeInteger(count)||count<1)throw new Error("Hồ sơ thiếu thông tin số phần dữ liệu.");
  const col=initHandoverFirebase().collection(HANDOVER_COLLECTION),parts=new Array(count);
  for(let start=1;start<=count;start+=4){
    ...
    await Promise.all(Array.from({length:end-start+1},async(_,offset)=>{
      const i=start+offset,id=`${parent.__docId}__${String(i).padStart(3,"0")}`;
      const snap=await col.doc(id).get();
      if(!snap.exists)throw new Error("Thiếu chunk "+i+" của "+parent.flightName);
      parts[i-1]=String(snap.data()?.data||"");
    }));
  }
  const payload=await v488UnpackJson(parent.encoding,parts.join(""));
  return {...payload,__parent:parent};
}
...
async function v488PreviewArchivedForms(i){
  ...
  const saved=v488Clone(state),arch=v488Clone(f.envelope?.state||{}),rawMain=String(f.sessionMeta?.initialGroup||f.envelope?.mainForm||"fsags"),isVZ=String(f.envelope?.mainForm||"")==="tvjgof035"||String(f.envelope?.vzForm||"")==="tvj_gof_035"||Object.keys(arch||{}).some(k=>String(k).startsWith("vz_")),main=isVZ?"tvjgof035":rawMain;
  try{
    Object.keys(state).forEach(k=>delete state[k]);Object.assign(state,arch);const pages=main==="tvjgof035"?[15,4]:main==="fsags421"?[6,7,4]:main==="fsags551"?[9,10,4]:[1,2,4],imgs=[];for(const p of pages){const c=await renderReportPage(p);imgs.push(c.toDataURL("image/jpeg",.9));}
...
if(typeof baseOpen==="function")window.v488OpenArchiveFlight=function(i){const out=baseOpen.apply(this,arguments);sagsObserveCompletionV480(out,()=>setTimeout(()=>appendPanel(i),0));return out;};
...
async function exportSelectedPdf(i){
  ...
  try{
    const st=clone(f.envelope?.state||{});Object.keys(state).forEach(k=>delete state[k]);Object.assign(state,st||{});const rawMain=String(f.sessionMeta?.initialGroup||f.envelope?.mainForm||"fsags"),isVZ=String(f.envelope?.mainForm||"")==="tvjgof035"||String(f.envelope?.vzForm||"")==="tvj_gof_035"||Object.keys(st||{}).some(k=>String(k).startsWith("vz_")),main=isVZ?"tvjgof035":rawMain;if(typeof activeFormGroup!=="undefined")activeFormGroup=main;if(typeof currentPage!=="undefined")currentPage=main==="tvjgof035"?15:main==="fsags421"?6:main==="fsags551"?9:main==="fsags09"?11:1;
    if(isVZ&&typeof window.sagsV495BeforeExport==="function"){await window.sagsV495BeforeExport();if(!window.sagsV495ManagedPage?.(15))throw new Error("TVJ-GOF-035 chưa có canonical registry Trang 15.");}
    if(kinds.includes("ramp")){
      const nums=main==="tvjgof035"?[15]:main==="fsags421"?[6,7]:main==="fsags551"?[9,10]:main==="fsags09"?[11,12]:[1,2];for(const n of nums)pages.push(await renderReportPage(n));
    }
...
async function importWorking(file){
  if(!isAD())return deny();try{const text=await file.text();const pkg=JSON.parse(text);await validateWorking(pkg);const env=clone(pkg.flight.envelope||{}),oldMeta=clone(pkg.flight.sessionMeta||{}),now=Date.now(),id=makeFlightSessionId();let list=readFlightSessionList();const rawMain=String(oldMeta.initialGroup||env.mainForm||"fsags"),isVZ=String(env.mainForm||"")==="tvjgof035"||String(env.vzForm||"")==="tvj_gof_035"||Object.keys(env.state||{}).some(k=>String(k).startsWith("vz_")),main=isVZ?"tvjgof035":rawMain;const base=String(pkg.flight.flightName||oldMeta.name||"HỒ SƠ").trim();const meta={...oldMeta,id,name:`${base} · KHÔI PHỤC`,customName:true,initialGroup:["fsags","fsags421","fsags551","fsags09","tvjgof035"].includes(main)?main:"fsags",createdAt:now,updatedAt:now,restoredFromWorkingV34:true,restoredAtMs:now,restoredSourceDate:String(pkg.operationalDate||""),restoredMatchKeys:clone(pkg.flight.matchKeys||[])};env.mainForm=meta.initialGroup;env.activeFormGroup=meta.initialGroup;env.currentPage=meta.initialGroup==="tvjgof035"?15:meta.initialGroup==="fsags421"?6:meta.initialGroup==="fsags551"?9:meta.initialGroup==="fsags09"?11:1;if(isVZ)env.vzForm="tvj_gof_035";env.scrollY=0;list.push(meta);writeFlightSessionList(list);localStorage.setItem(flightSessionStorageKey(id),JSON.stringify(env));try{localStorage.setItem(sagsOwnedPrefix(V34_REF_PREFIX)+id,JSON.stringify({format:V34_WORK_FORMAT,references:pkg.references||{},identity:pkg.flight.identity||{},matchKeys:pkg.flight.matchKeys||[],importedAtMs:now}));}catch(e){}
...
(function(){
  ...
  function layers(){
    const q=[
      ...
      '#quickTimeModal','#fs09QuickModal','#finalFormsModal'
    ].join(',');
  ...
  function panelFor(layer){
    ...
    const preferred=layer.querySelector('.ahPanel,.fwcPanel,.finalPanel,.modal-content,.modalContent,[class*="ModalPanel"],[class*="modalPanel"],[class*="Panel"]');
...
(function(root){
  ...
  function formLabel(g){g=upper(g);return g==="FINAL"?"FINAL":g==="FSAGS54"?"FSAGS54":g==="CLC_CHECKLIST"?"CLC CHECKLIST":g==="FSAGS421"?"42.1":(g==="FSAGS551"?"55.1":(g==="FSAGS09"?"FSAGS 09":"42.3"));}
  function hashId(s){
  ...
  // DAILY_ROSTER Booking: 0/12/150 = F/C/Y; never interpret a single total as a cabin split.
  function rosterBookingParts(raw){
    const text=S(raw).replace(/\u00a0/g,' '),out={raw:text,total:'',F:'',C:'',Y:'',I:''};
    const triple=/^(\d{1,5})\s*\/\s*(\d{1,5})\s*\/\s*(\d{1,5})$/.exec(text);
    if(triple){out.F=triple[1];out.C=triple[2];out.Y=triple[3];return out;}
    if(/^\d+$/.test(text)){out.total=text;return out;}
    for(const cabin of ['F','C','Y','I']){
      const match=new RegExp(`(?:^|[\\s,;/|])${cabin}\\s*[:=\\-]?\\s*(\\d+)`,'i').exec(text);
      if(match)out[cabin]=match[1];
    }
    return out;
  }
  ...
  function rosterRecords(parsed){
    ...
    for(let i=hi+1;i<(parsed.rows||[]).length;i++){
      ...
      const base={
        ...
        booking:S(getCell(row,map,"Booking")),grndCor:corPlan.display.length?corPlan.display:corUsers,grndLd:ldUsers,grndLs:lsUsers,paxSupr:paxUsers,
        flightName:[arrFlight,depFlight].filter(Boolean).join(" / ")||upper(flightRaw)
      };
      const isVzTurn=[arrFlight,depFlight,flightRaw].some(v=>/^VZ\s*\d+/i.test(S(v)));
      ...
      const add=(u,formGroup,sourceColumn,roleKey,workPartOrder=1,workPartTotal=1,assignmentLeg="",coMeta=null)=>{
        ...
        // V6.3.18: CBTT can own multiple independent forms on the same flight.
        // Keep the legacy FINAL assignmentId stable, but namespace additional CBTT
        // checklist assignments by formGroup so they do not overwrite FINAL/each other.
        if(roleKey==="CBTT"&&formGroup!=="final")idParts.push(formGroup);
      ...
      const addCorEntries=(entries,formGroup,sourceColumn,roleKey)=>{
        // VZ uses one TVJ-GOF-035 RAMP checklist for the turnaround. 55.1 remains
        // a separate Loading assignment; only coordinator RAMP groups are remapped.
        const resolvedGroup=isVzTurn&&["fsags","fsags421","fsags423"].includes(String(formGroup).toLowerCase())?"tvjgof035":formGroup;
        ...
        ordered.forEach((g,step)=>{const rows=g.rows.slice().sort((a,b)=>(Number(a.coIndex)||0)-(Number(b.coIndex)||0)),users=rows.map(e=>e.user);rows.forEach((e,i)=>add(e.user,resolvedGroup,sourceColumn,roleKey,step+1,ordered.length,e.leg,{users,rank:i+1}))});
      };
      if(!ldUsers.length){
      ...
      // V1.82: Grnd_Ls là nguồn phân công CBTT.
      // V6.3.17: mỗi người Grnd_Ls nhận 3 nhiệm vụ độc lập trên cùng chuyến:
      // FINAL/CROSSCHECK + LOAD CONTROL CHECKLIST + CLC CHECKLIST.
      // Thứ tự username trong cùng ô vẫn là thứ tự bắt buộc nhận/làm: A / B / C => A → B → C.
      lsUsers.forEach((u,i)=>{
        add(u,"final","Grnd_Ls","CBTT",i+1,lsUsers.length);
        add(u,"FSAGS54","Grnd_Ls","CBTT",1,1);
        add(u,"clc_checklist","Grnd_Ls","CBTT",1,1);
      });
  ...
  function seedFor(rec){
    const s={},booking=rosterBookingParts(rec.booking||rec.bookingParts?.raw);
    if(rec.formGroup==="fsags421"){
      ...
      for(const c of ["F","C","Y"])if(booking[c]!=="")s["f421_booking"+c]=booking[c];
      ...
      if(["fsags","fsags423"].includes(rec.formGroup))for(const c of ["F","C","Y"])if(booking[c]!=="")s["booking"+c]=booking[c];
    }
    for(const k of Object.keys(s))if(!S(s[k]))delete s[k];
    return s;
  }
  ...
  root.__SAGS_DAILY_ROSTER_TEST__={parseXlsxBytes,parseCsvText,headerRowInfo,parseDate,parseRosterTime,fmtTime,addIsoDays,isoDayDiff,resolveEventDate,sortMinuteFor,splitFlights,usersFromCell,coAssigneeGroupsFromCell,flattenUserGroups,grndCorPlan,rosterBookingParts,allFlightRows,pvhk09SeedFor,rosterRecords,seedFor,flightIdForRoster};
  ...
  const ROSTER_DELTA_FIELDS=[
    ...
    "acReg","acType","route","route1","route3","bay","booking","formGroup","sourceColumn","roleKey","assignmentLeg","assignmentFlight",
  ...
  function rosterWorkSlotKey(x){
    const source=rosterSlotSource(x);
    // V6.3.18: Grnd_Ls owns three independent forms on one flight.
    // Include formGroup for CBTT so FINAL, Load Control and CLC are not treated
    // as replacement versions of the same work slot during safe roster merge.
    const cbttForm=source==="GRND_LS"?upper(x?.formGroup||"FINAL"):"";
    return [
      S(x?.opDate),
  ...
  function sameRosterWorkSlot(a,b){
    ...
    const sourceA=rosterSlotSource(a),sourceB=rosterSlotSource(b);
    return sourceA===sourceB
      && (sourceA!=="GRND_LS"||upper(a?.formGroup||"FINAL")===upper(b?.formGroup||"FINAL"))
...
Không ghi đè working envelope của nhân viên đang thao tác.`);
  ...
  function startPageForGroup(g){g=String(g||"").toLowerCase();return g==="tvjgof035"?15:(g==="fsags421"?6:(g==="fsags551"?9:(g==="fsags09"?11:(g==="fsags54"?16:(g==="clc_checklist"?17:1)))));}
  ...
  function scheduleSharedSync(meta,env,delay=260){
    ...
    rosterSyncTimers.set(id,setTimeout(()=>{rosterSyncTimers.delete(id);const owner=meta?.rosterOpenShared===true?'CBTT_SHARED':(currentUserProfile?.username||"");void writeSharedAssignment(id,env,owner,meta.initialGroup||env?.mainForm||"",false);},delay));
  }
  ...
  async function processMailbox(raw){
    ...
    if(created.length){showToast(`DAILY ROSTER: đã đồng bộ ${created.length} biểu mẫu chờ nhận · ${created.slice(0,3).join(", ")}${created.length>3?"…":""}`);try{renderFlightSessionList?.();}catch(e){}}
  }
...
.flightTypeEditBtn,.flightOpsBtn,button[class*="Btn"]:not(.secondary):not(.gray):not(.green):not(.danger):not(.red):not(.warn):not(.orange):not(.active):not(.on){
  background:linear-gradient(180deg,#315fa2,#244c89)!important;color:#fff!important;
...
.sagsAdminCard,.fleetManagerCard,.aclCard,.acls-step,.drField,.v320Card,.v310ShiftCard,.v342Card,.v339Card,.v338Doc{
...
(function(root){
  ...
  function wrap(name){const base=root[name];if(typeof base!=='function'||base.__v2239SigGeometry)return;const fn=function(){const out=base.apply(this,arguments);sagsObserveCompletionV480(out,()=>schedule(name==='switchFlightSession'?420:180));return out};fn.__v2239SigGeometry=true;fn.__v2239Base=base;root[name]=fn;try{if(name==='showFormGroup')showFormGroup=fn;else if(name==='switchFlightSession')switchFlightSession=fn}catch(_){}}
...
/* ===== V6.1.40 QUICK TIME STYLE OWNERSHIP =====
 * Quick Time visual styling now lives only in new-ui-v1.css.
 * Do not inject a second Quick Time stylesheet from runtime JS.
 */
(function(){
  const css=`
#v324FormActions{gap:7px!important;padding:4px 0 6px!important}
.v324FormAction{min-height:44px!important;border-radius:12px!important;padding:7px 10px!important;font:900 11px/1.1 Arial!important;box-shadow:none!important}
#v1134QuickTimeBtn{background:#edf3f8!important;color:#173e62!important;border-color:#c9d7e3!important}
...
.attachmentSourceBox{border-radius:18px!important;padding:14px!important;box-shadow:0 14px 36px rgba(0,0,0,.28)!important}
.attachmentSourceBox button{min-height:44px!important;border-radius:10px!important;margin-top:7px!important;font-size:14px!important}
`;
  function install(){let s=document.getElementById('v6140NonQuickStyle');if(!s){s=document.createElement('style');s.id='v6140NonQuickStyle';document.head.appendChild(s)}s.textContent=css}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
})();
...
function bookingParts(raw){return root.__SAGS_DAILY_ROSTER_TEST__?.rosterBookingParts?.(raw)||{raw:S(raw),total:'',F:'',C:'',Y:'',I:''}}
...
(function(root){'use strict';
 ...
 const groups=[
  {key:'ops',title:'✈ VẬN HÀNH CHUYẾN',sub:'DAILY ROSTER tự tạo chuyến và Flight Workspace',items:[
  ...
  {key:'monitor',title:'🛡 HỆ THỐNG & HỒ SƠ',sub:'Sao lưu, nhật ký, tài nguyên hệ thống và lưu trữ',items:[
    ['roleBtnActivity','💾 SAO LƯU & KHỞI TẠO','AD sao lưu/kiểm tra trước khi khởi tạo lại'],
...
async function assignedData(){
  ...
  // Worker MULTITASK reuses the same *own-account* metadata as MY FLIGHT.
  // This screen needs labels and timing only, never full flight_records or
  // roster_manifests of everyone on the shift. AD retains the full overview.
  if(role()!=='AD'){
    if(typeof root.sagsV477ManifestForWorker!=='function')throw new Error('Hộp phân công đang khởi tạo.');
    const man=await root.sagsV477ManifestForWorker(d),byFlight=new Map();
    for(const item of Object.values(man?.items||{})){
      if(!item||item.active===false||normUser(item.user||item.targetUser)!==normUser(u))continue;
      const fid=S(item.flightId);if(!fid)continue;
      const prev=byFlight.get(fid)||{},arr=S(item.arrFlight||prev.arrFlight),depFlight=S(item.depFlight||prev.depFlight);
      byFlight.set(fid,{...prev,flightId:fid,opDate:d,flightName:S(item.flightName||item.flightRaw||prev.flightName||[arr,depFlight].filter(Boolean).join(' / ')||fid),flightRaw:S(item.flightRaw||prev.flightRaw),arrFlight:arr,depFlight,route:S(item.route||prev.route),acReg:S(item.acReg||prev.acReg),acType:S(item.acType||prev.acType),bay:S(item.bay||prev.bay),sta:S(item.sta||prev.sta),std:S(item.std||prev.std),staSortMinute:Number(item.staSortMinute??prev.staSortMinute??-1),stdSortMinute:Number(item.stdSortMinute??prev.stdSortMinute??-1)});
    }
    return {d,list:[...byFlight.values()].sort((x,y)=>recordTimeScore(x,'std')-recordTimeScore(y,'std')||flightName(x).localeCompare(flightName(y),'vi'))};
  }
...
async function count(){const e=document.getElementById('fwcMultitaskCount');if(!e)return;try{if(role()!=='AD'){e.textContent=String(multiSet().size);return}e.textContent=String((await data()).list.length)}catch(_){e.textContent='0'}}
...
root.sagsV36CloseMultitask=()=>document.getElementById('fwcMultitaskModal')?.classList.remove('show');root.sagsV36SwitchFlight=f=>{multiAdd(f);root.sagsV36CloseMultitask();setCtx(opDate(),f);if(role()!=='AD'&&typeof root.sagsV477ManifestForWorker==='function'){root.flightWorkspaceOpenList?.(opDate());return}root.flightWorkspaceOpenFlight?.(f)};root.sagsV323FinishMultitask=f=>{f=S(f);const wasCurrent=selected()===f;if(wasCurrent)setCtx(opDate(),'');multiRemove(f);if(wasCurrent){root.sagsV36CloseMultitask();root.flightWorkspaceOpenList?.(opDate());setTimeout(count,80)}else{count();setTimeout(()=>root.sagsV36OpenMultitask?.(),20)}};
...
function patch(){let n=0,t=setInterval(()=>{if(typeof root.flightWorkspaceOpenFlight==='function'&&typeof root.flightWorkspaceOpenList==='function'){clearInterval(t);if(!root.flightWorkspaceOpenFlight.__v36){const f=root.flightWorkspaceOpenFlight,l=root.flightWorkspaceOpenList,c=root.flightWorkspaceClose;root.flightWorkspaceOpenFlight=function(id){const r=f.apply(this,arguments);setTimeout(()=>{sticky();document.querySelector('#fwcBody>.fwcBack')?.remove();navFlight(id)},0);return r};root.flightWorkspaceOpenFlight.__v36=1;root.flightWorkspaceOpenList=function(d){d=S(d)||today();const r=l.apply(this,arguments);navList(d);sagsObserveCompletionV480(r,()=>setTimeout(()=>navList(d),0));return r};root.flightWorkspaceClose=function(){root.sagsV36CloseMultitask();return c?.apply(this,arguments)}}sticky()}else if(++n>60)clearInterval(t)},200);n=0;t=setInterval(()=>{if(typeof root.cxCleanShowToast==='function'){clearInterval(t);if(!root.cxCleanShowToast.__v36){const b=root.cxCleanShowToast;root.cxCleanShowToast=function(m){if(S(m?.eventType)==='COMPLETE_OK'&&role()==='DH'&&show(m))return;return b.apply(this,arguments)};root.cxCleanShowToast.__v36=1}}else if(++n>60)clearInterval(t)},250)}
...
(function(){
  ...
  function boot(){
    if(install())return;
    let n=0;const t=setInterval(()=>{n++;if(install()||n>=16)clearInterval(t);},250);
  }
...
.v38NavBtn.purple{background:#5b21b6}.v38NavBtn.gray{background:#e9eef3;color:#30475b}.v38NavBtn.admin{background:#7c2d12}.v38NavSpacer{flex:1}
...
`;document.head.appendChild(st)}
  ...
  function ensureCleanNav(){
    ...
    const sig=[logged()?'1':'0',shiftAvailable?'1':'0',signAvailable?'1':'0',isAD()?'1':'0'].join('|');
    // V3.11: do not rebuild the navigation bar on polling/sync. Replacing innerHTML
    // Avoid rebuilding optional toolbar actions every few seconds; it visibly blinks on mobile.
  ...
  function patchWorkspace(){
    ...
    if(!root.flightWorkspaceOpenList.__v38){
      ...
      root.flightWorkspaceOpenList=function(d){d=S(d)||today();const r=baseList.apply(this,arguments);sagsObserveCompletionV480(r,()=>setTimeout(()=>decorateList(d),80));return r};root.flightWorkspaceOpenList.__v38=1;
 ...
 * FSAGS 42.3 / FSAGS 42.1 / FSAGS 09 / TVJ-GOF-035 (VZ).
 */
(function(root){'use strict';
  ...
  function vzContext(){
    try{
      const g=group();if(g)return g==='tvjgof035';
      const sid=sessionId(),env=(sid&&typeof readFlightSessionEnvelope==='function')?(readFlightSessionEnvelope(sid)||{}):{};
      return S(env.activeFormGroup||env.mainForm).toLowerCase()==='tvjgof035';
    }catch(_){return false}
  }
  function eligible(){
    const g=group(),r=normalizedRole(),vz=vzContext();
    if(idleVisible()&&!vz)return false;
    if(vz)return false;
...
`;
  ...
  function ensure(){
    ensureStyle();
    const toolbar=document.querySelector('.toolbar.compact-main-toolbar')||document.querySelector('.toolbar');
    if(!toolbar)return null;
    let box=document.getElementById('v313QuickContext');
    if(box&&box.parentNode!==toolbar){
    ...
    if(!box){
      ...
      document.getElementById('v313QuickContextBtn').onclick=()=>{
        if(!eligible()){refresh();return alert('NHẬP NHANH chỉ dùng khi đang mở đúng FSAGS 42.3 / 42.1 / 55.1 / 09 / TVJ-GOF-035 và tài khoản có quyền thao tác.');}
  ...
  function refresh(){
    ...
    if(hint)hint.textContent=ok?`Đang mở ${group()==='fsags'?'FSAGS 42.3':group()==='fsags421'?'FSAGS 42.1':group()==='fsags551'?'FSAGS 55.1':'FSAGS 09'} · nhập nhanh các mốc giờ của biểu mẫu này.`:'Chỉ hiện khi đang mở biểu mẫu hỗ trợ.';
  }
  function wrap(name){
    const fn=root[name];if(typeof fn!=='function'||fn.__v313QuickContext)return;
    const w=function(){const r=fn.apply(this,arguments);sagsObserveCompletionV480(r,()=>setTimeout(refresh,30));return r};
...
`;
  ...
  ['showFormGroup','showRoleHomeIdle','hideRoleHomeIdle','switchFlightSession'].forEach(name=>{const fn=root[name];if(typeof fn!=='function'||fn.__v320)return;const w=function(){const r=fn.apply(this,arguments);sagsObserveCompletionV480(r,()=>setTimeout(refreshNa,35));return r};w.__v320=1;root[name]=w;try{if(name==='showFormGroup')showFormGroup=w;else if(name==='showRoleHomeIdle')showRoleHomeIdle=w;else if(name==='hideRoleHomeIdle')hideRoleHomeIdle=w;else if(name==='switchFlightSession')switchFlightSession=w}catch(_){}});
...
(function(root){'use strict';
  const BUILD='V5.0.3B-DIRECT-ROSTER';
  const S=v=>String(v??'').trim(), U=v=>S(v).toUpperCase();
  const norm=v=>{try{return typeof root.normalizePersonalUsername==='function'?root.normalizePersonalUsername(v):U(v).replace(/\s+/g,'').replace(/[^A-Z0-9._-]/g,'_').slice(0,40)}catch(_){return U(v)}};
  const safe=v=>S(v).replace(/[.#$\[\]\/]/g,'_');
  const esc=v=>S(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  ...
  function formLabel(g){g=U(g);return g==='TVJGOF035'?'VZ Form':g==='FINAL'?'CBTT · FINAL':g==='FSAGS421'?'FSAGS 42.1':g==='FSAGS551'?'FSAGS 55.1':g==='FSAGS09'?'F/SAGS-CXR/09':g==='LOADING208'?'F/SAG-CXR/208':'FSAGS 42.3'}
  function isVzRosterItem(item,extra=''){const vals=[item?.assignmentFlight,item?.flightName,item?.flightRaw,item?.arrFlight,item?.depFlight,item?.flightId,extra].map(S);if(U(item?.formGroup)==='TVJGOF035')return true;return vals.some(v=>/(?:^|[^A-Z0-9])VZ\s*\d+(?:$|[^A-Z0-9])/i.test(v)||/^VZ\s*\d+/i.test(v))}
  function itemFormLabel(item,extra=''){return isVzRosterItem(item,extra)?'VZ Form':formLabel(item?.formGroup)}
  function isVzFlightCard(card,fid=''){
    const raw=[fid,card?.dataset?.v38Fid,card?.querySelector?.('.fwcFlightTitle')?.textContent,card?.querySelector?.('.fwcFlightMeta')?.textContent].map(S).join(' ');
    return /(?:^|[^A-Z0-9])VZ\s*\d+(?:$|[^A-Z0-9])/i.test(raw)||/^VZ\s*\d+/i.test(raw);
  }
  function forceVzReceiveLabel(card,btn,fid=''){
    try{
      if(!card||!btn||!isVzFlightCard(card,fid))return false;
      const t=S(btn.textContent);
      if(/^NHẬN\b/i.test(t)&&t!=='NHẬN VZ Form'){btn.textContent='NHẬN VZ Form';btn.dataset.vzReceiveLabel='1';return true}
    }catch(_){}
    return false;
  }
  let vzReceiveObserver=null,vzReceiveObservedHost=null,vzReceiveTimer=0;
  function enforceVzReceiveLabels(){
    const host=document.getElementById('fwcList');if(!host)return;
    for(const card of host.querySelectorAll('.fwcFlight')){
      const fid=S(card.dataset?.v38Fid);
      const btn=card.querySelector('button.v324ReceiveBtn,button[onclick*="flightWorkspaceOpenFlight"]');
      forceVzReceiveLabel(card,btn,fid);
    }
  }
  function ensureVzReceiveObserver(){
    const host=document.getElementById('fwcList');if(!host)return;
    if(vzReceiveObserver&&vzReceiveObservedHost===host)return;
    try{vzReceiveObserver?.disconnect()}catch(_){}
    vzReceiveObservedHost=host;
    vzReceiveObserver=new MutationObserver(()=>{
      clearTimeout(vzReceiveTimer);
      vzReceiveTimer=setTimeout(enforceVzReceiveLabels,0);
    });
    vzReceiveObserver.observe(host,{subtree:true,childList:true,characterData:true});
    enforceVzReceiveLabels();
  }
  ...
  async function claimStateFor(item,man){
    ...
    const completedAt=Number(st.completedAtMs||st.completionEnvelopeAtMs||0),lastClaim=Number(st.claimedAtMs||0),lastReopen=Number(st.reopenedAtMs||0);
    const recoveredCompleted=completedAt>0&&completedAt>=lastClaim&&completedAt>=lastReopen&&st.completionEnvelope&&typeof st.completionEnvelope==='object';
    // V5.0.2: a legacy co-assignee was falsely marked complete by another person's click.
    // Treat that legacy auto-skip as not yet received unless THIS account actually completed it.
    const legacyCoSkip=st.autoSkippedCoAssignee===true&&norm(st.completedBy)!==me();
    if(!legacyCoSkip&&(recoveredCompleted||ts==='COMPLETED'||cs==='PART_COMPLETED'||cs==='COMPLETED'||cs==='HANDED_OVER'))return {state:'DONE',st,taskStatus:'COMPLETED'};
    if(legacyCoSkip)return {state:'READY',st,taskStatus:'UNCLAIMED'};
    if((ts==='IN_PROGRESS'||cs==='CLAIMED')&&norm(st.claimedBy||st.ownerUser)===me())return {state:'CLAIMED',st,taskStatus:'IN_PROGRESS'};
    const coOwner=coGroupId(item)?await activeCoOwner(man,item):null;
    ...
    // V4.8.3: ownership in the current roster is authoritative. A later assigned operator
    // may open their own work even when the previous operator has not pressed HOÀN TẤT.
    // openAssignment()/workspace hydration still reuses any shared edits that already exist.
  ...
  async function decorateList(d=opDate()){
    ...
    const u=me();for(const card of host.querySelectorAll('.fwcFlight')){
      ...
      const liveFlightContext=[fid,card.dataset?.v38Fid,card.querySelector('.fwcFlightTitle')?.textContent,card.querySelector('.fwcFlightMeta')?.textContent,card.textContent].map(S).join(' ');
      const receiveFormLabel=itemFormLabel(primary,liveFlightContext);
      card.dataset.v1137PrimaryAssignment=S(primary.assignmentId);card.dataset.v1137PrimaryLeg=U(primary.assignmentLeg);old.classList.add('v324ReceiveBtn');old.onclick=null;old.removeAttribute('onclick');old.disabled=false;old.classList.remove('wait','standby','done','blocked','na','takeover','skipped');
      ...
      else if(cs.state==='CLAIMED'){old.textContent='NHẬN '+receiveFormLabel;old.onclick=()=>root.v324ReceiveOrOpen(fid,S(primary.assignmentId),d);}
      ...
      else if(cs.state==='DONE'){old.textContent='NHẬN '+receiveFormLabel;old.disabled=false;old.classList.add('done');old.onclick=()=>root.v324ReceiveOrOpen(fid,S(primary.assignmentId),d)}
      ...
      else {old.textContent='NHẬN '+receiveFormLabel;old.onclick=()=>root.v324ReceiveOrOpen(fid,S(primary.assignmentId),d);}
      if(!pbSource)forceVzReceiveLabel(card,old,fid);
    ...
    ensureVzReceiveObserver();enforceVzReceiveLabels();
  ...
  // One physical form per flight/type. Each operator has their own status and
  // immutable completion; the next roster assignee reads the latest real snapshot.
  async function latestRosterFormSnapshot(item,man){
    const form=x=>{const g=U(x?.formGroup);return g==='FSAGS'?'FSAGS423':g};
    const peers=itemsOf(man).filter(x=>S(x.assignmentId)!==S(item.assignmentId)&&S(x.flightId)===S(item.flightId)&&form(x)===form(item));
    const rows=[];
    for(const peer of peers){
      const st=await sessionState(peer.assignmentId);
      const by=norm(st.completedBy),owner=norm(peer.user||peer.targetUser);
      if(st.autoSkippedCoAssignee===true||!by||by!==owner||!workPartFinished(st)||!envelopeHasData(st.completionEnvelope))continue;
      rows.push({env:st.completionEnvelope,at:Number(st.completionEnvelopeAtMs||st.completedAtMs||0),source:'ROSTER_PEER_COMPLETION',assignmentId:S(peer.assignmentId)});
    }
    rows.sort((a,b)=>b.at-a.at);return rows[0]||null;
  }
  async function resolveHandoverEnvelope(item,man){
    ...
    const peer=await latestRosterFormSnapshot(item,man);if(peer)candidates.push({...peer,priority:5});
    if(shared?.completionEnvelope&&!shared.autoSkippedCoAssignee)candidates.push({env:shared.completionEnvelope,at:Number(shared.completionEnvelopeAtMs||shared.completedAtMs||0),source:'OWN_COMPLETION',priority:9});
    ...
    if(Number(shared?.upstreamAppliedAtMs||0)>0&&shared?.envelope&&typeof shared.envelope==='object')candidates.push({env:shared.envelope,at:Number(shared.envelopeUpdatedAtMs||shared.upstreamAppliedAtMs||0),source:'UPSTREAM_RECONCILED',priority:7});
  ...
  async function syncSharedIntoLocal(item,meta,man){
    ...
    try{
      ...
      const isImmutable=['HANDOVER_SNAPSHOT','PREVIOUS_COMPLETION','ROSTER_PEER_COMPLETION','OWN_COMPLETION'].includes(resolved.source);
      ...
      if(resolved.source==='ROSTER_PEER_COMPLETION'&&localHasData){
        const currentSession=await sessionState(item.assignmentId);
        const baseline=currentSession?.rosterPeerBaselineEnvelope||null;
        const lastPeer=Number(currentSession?.rosterPeerSnapshotAtMs||0);
        if(incomingAt&&lastPeer>=incomingAt)return false;
        // Without a baseline, retain edited local values. On later revisions use a
        // three-way field merge, so a newer A cannot overwrite B's own entries.
        const merged=reconcileUpstreamRevision(local,{baselineEnvelope:baseline,envelope:resolved.env,atMs:incomingAt,fromAssignmentId:resolved.assignmentId});
        env=merged.env;
        if(merged.conflicts.length)alert('Người cùng chuyến đã cập nhật '+merged.conflicts.length+' ô bạn cũng đã sửa. Giữ giá trị trên máy bạn; hãy đối chiếu: '+merged.conflicts.slice(0,8).join(', '));
        // Do not mark snapshot consumed if Firebase failed; retry safely on next open.
        await db(`roster_sessions/${safe(item.assignmentId)}`).update({rosterPeerBaselineEnvelope:sanitizeEnvelope(resolved.env),rosterPeerSnapshotAtMs:incomingAt,rosterPeerSourceAssignmentId:resolved.assignmentId});
      }
  ...
  // Re-completed A may have posted a pending correction while B was editing.
  // Read from RTDB, merge by field, and do not discard B's independently edited values.
  const pendingUpstream=await applyPendingUpstreamToLocal(item,meta,man);
  if(pendingUpstream?.conflicts?.length){
    alert('A vừa sửa dữ liệu. Đã đồng bộ '+pendingUpstream.applied+' ô; '+pendingUpstream.conflicts.length+' ô cả A và B đều sửa được giữ theo bản B để tránh mất dữ liệu. Hãy đối chiếu các ô: '+pendingUpstream.conflicts.slice(0,8).join(', '));
  }
  ...
  if(typeof root.switchFlightSession!=='function')throw new Error('Bộ mở biểu mẫu chưa sẵn sàng. Dữ liệu chuyến vẫn được giữ nguyên.');
  await root.switchFlightSession(meta.id,true);
  if(S(root.currentFlightSessionMeta?.()?.id)!==S(meta.id)){
    throw new Error('Không xác nhận được biểu mẫu đã mở. Hãy kiểm tra dữ liệu máy và thử lại; không nhập vào biểu mẫu trắng.');
  }
  ...
  async function acquireCoGroupClaim(d,item,man,{allowCompleted=false}={}){
    ...
    // Legacy V5 may leave a CLAIMED group lock after its owner has completed.
    // Only release that exact stale generation; never steal a live operator's lock.
    let priorLock=null,completedOldLock=false;
    try{
      priorLock=(await lockRef.once('value')).val()||null;
      if(U(priorLock?.status)==='CLAIMED'&&norm(priorLock?.claimedBy)!==u&&S(priorLock?.claimedAssignmentId)){
        const ownerSt=await sessionState(priorLock.claimedAssignmentId);
        const finishedAt=Math.max(Number(ownerSt.completedAtMs||0),Number(ownerSt.completionEnvelopeAtMs||0));
        completedOldLock=workPartFinished(ownerSt)&&finishedAt>0&&finishedAt>=Number(priorLock.claimedAtMs||0)&&finishedAt>=Number(ownerSt.reopenedAtMs||0);
      }
    }catch(_){completedOldLock=false;}
    let result=null;
    try{result=await lockRef.transaction(cur=>{cur=cur&&typeof cur==='object'?cur:{};const status=U(cur.status),owner=norm(cur.claimedBy);if(status==='CLAIMED'&&owner&&owner!==u){
      if(!completedOldLock||S(cur.claimedAssignmentId)!==S(priorLock?.claimedAssignmentId)||Number(cur.updatedAtMs||0)!==Number(priorLock?.updatedAtMs||0)||Number(cur.claimedAtMs||0)!==Number(priorLock?.claimedAtMs||0))return;
    }return {schema:1,groupId:gid,status:'CLAIMED',claimedBy:u,claimedAssignmentId:aid,claimedAtMs:t,updatedAtMs:t,opDate:S(d),flightId:S(item.flightId),formGroup:S(item.formGroup),sourceColumn:S(item.sourceColumn)};});}catch(e){throw new Error('Không khóa được nhóm hỗ trợ: '+S(e?.message||e))}
  ...
  async function markClaim(d,fid,item,man){
    ...
    patch[`roster_sessions/${safe(aid)}/claimStatus`]='CLAIMED';patch[`roster_sessions/${safe(aid)}/taskStatusV333`]='IN_PROGRESS';patch[`roster_sessions/${safe(aid)}/taskAvailabilityV333`]='ACTIVE';patch[`roster_sessions/${safe(aid)}/taskStatusUpdatedAtMs`]=t;patch[`roster_sessions/${safe(aid)}/claimedAtMs`]=t;patch[`roster_sessions/${safe(aid)}/claimedBy`]=u;patch[`roster_sessions/${safe(aid)}/handoverReady`]=false;patch[`roster_sessions/${safe(aid)}/autoSkippedCoAssignee`]=false;patch[`roster_sessions/${safe(aid)}/completedAtMs`]=null;patch[`roster_sessions/${safe(aid)}/completedBy`]=null;patch[`roster_sessions/${safe(aid)}/workPartStatus`]='IN_PROGRESS';patch[`roster_sessions/${safe(aid)}/coClaimedBy`]=co.gid?u:null;patch[`roster_sessions/${safe(aid)}/coClaimedAssignmentId`]=co.gid?aid:null;patch[`roster_sessions/${safe(aid)}/updatedAtMs`]=t;
    // No peer mutation: every roster assignee keeps an independent claim/completion status.
  ...
  // V5.0.1B: do not overwrite an already-completed successor when the previous
  // operator corrects and completes their own work again. Reconcile only the
  // upstream fields changed since B's original handover; protect B's edits.
  function workPartFinished(st){
    const status=[st?.claimStatus,st?.workPartStatus,st?.taskStatusV333,st?.taskStatus].map(U);
    return status.some(v=>['PART_COMPLETED','COMPLETED','HANDED_OVER'].includes(v));
  }
  function successorHasStarted(st){
    return workPartFinished(st)||[st?.claimStatus,st?.workPartStatus,st?.taskStatusV333].map(U).some(v=>['CLAIMED','IN_PROGRESS','STANDBY'].includes(v))||Number(st?.claimedAtMs||0)>0||Number(st?.completedAtMs||0)>0;
  }
  function reconcileUpstreamRevision(localEnvelope,revision){
    const out=clone(localEnvelope||{}),base=revision?.baselineEnvelope?.state||{},newState=revision?.envelope?.state||{};
    ...
    const hasBaseline=revision?.baselineEnvelope&&typeof revision.baselineEnvelope.state==='object';
    for(const key of new Set([...Object.keys(base),...Object.keys(newState)])){
      if(/attachment/i.test(key)||same(base[key],newState[key]))continue;
      const current=out.state[key];
      if((hasBaseline&&same(current,base[key]))||(!hasBaseline&&(current===undefined||current===null||current===''))){
        if(Object.prototype.hasOwnProperty.call(newState,key))out.state[key]=clone(newState[key]);
        else delete out.state[key];
        applied++;
      }else if(!same(current,newState[key]))conflicts.push(key);
    }
    out.rosterUpstreamRevisionAtMs=Number(revision?.atMs||0);
    out.rosterUpstreamFromAssignmentId=S(revision?.fromAssignmentId);
    out.rosterUpstreamConflictFields=conflicts.slice(0,100);
    return {env:out,conflicts,applied,missingBaseline:!hasBaseline};
  }
  async function applyPendingUpstreamToLocal(item,meta,man){
    const aid=S(item?.assignmentId);if(!aid||!meta||typeof root.flightSessionStorageKey!=='function')return null;
    const st=await sessionState(aid),revision=st?.upstreamRevision,revAt=Number(revision?.atMs||0);
    if(!revAt||revAt<=Number(st?.upstreamAppliedAtMs||0))return null;
    const local=root.readFlightSessionEnvelope?.(meta.id)||{};
    // If B has not yet saved any business data, use the latest A snapshot.
    const r=reconcileUpstreamRevision(local,revision);
    const env=mergeRosterSeedDefaults(item,r.env,local).env;
    env.mainForm=S(item.formGroup||env.mainForm||'fsags');env.activeFormGroup=env.mainForm;
    env.rosterAssignmentId=aid;
    localStorage.setItem(root.flightSessionStorageKey(meta.id),JSON.stringify(env));
    await db(`roster_sessions/${safe(aid)}`).update({envelope:sanitizeEnvelope(env),envelopeUpdatedAtMs:Date.now(),upstreamAppliedAtMs:revAt,upstreamConflicts:r.conflicts,upstreamReviewedBy:me()});
    return r;
  }
  async function restoreOwnCompletedEnvelopeForReopen(item,man,cs){
    const aid=S(item?.assignmentId);if(!aid)return null;
    const st=await sessionState(aid),candidates=[];
    ...
    let env=clone(pick.env);
    let upstreamReview=null;
    const revision=st?.upstreamRevision,revAt=Number(revision?.atMs||0);
    if(revAt>Number(st?.upstreamAppliedAtMs||0)){
      upstreamReview=reconcileUpstreamRevision(env,revision);
      env=upstreamReview.env;
    }
    const peer=await latestRosterFormSnapshot(item,man);
    if(peer&&peer.at>Number(st?.rosterPeerSnapshotAtMs||0)){
      const merged=reconcileUpstreamRevision(env,{baselineEnvelope:st?.rosterPeerBaselineEnvelope||null,envelope:peer.env,atMs:peer.at,fromAssignmentId:peer.assignmentId});
      env=merged.env;
      upstreamReview=merged;
      await db(`roster_sessions/${safe(aid)}`).update({rosterPeerBaselineEnvelope:sanitizeEnvelope(peer.env),rosterPeerSnapshotAtMs:peer.at,rosterPeerSourceAssignmentId:peer.assignmentId});
    }
    env.rosterAssignmentId=aid;env.mainForm=S(item?.formGroup||env.mainForm||meta?.initialGroup||'fsags');env.activeFormGroup=env.mainForm;env.currentPage=Number(env.currentPage)||1;env.scrollY=0;env.rosterReopenRestoredAtMs=Date.now();env.rosterReopenRestoreSource=pick.source;
    ...
    return {env,source:pick.source,meta,upstreamReview,upstreamRevisionAtMs:upstreamReview?revAt:0};
  }
...
${rosterLabel(item)} · ${itemFormLabel(item)}
...
Phần của ${who} được ghi “BỎ QUA · KHÔNG E-FORM”, không ghi HOÀN THÀNH giả.`)}catch(e){alert('Không nhận thay được: '+S(e?.message||e))}};
  ...
  root.v324ReceiveOrOpen=async function(fid,requestedAid='',requestedDate=''){
    fid=S(fid);requestedAid=S(requestedAid);
    if(!fid)return false;
    ...
    try{
      ...
      const exact=requestedAid?mine.find(x=>S(x.assignmentId)===requestedAid):null;
      if(requestedAid&&!exact)throw new Error('Phân công đã thay đổi hoặc không còn thuộc tài khoản này. Tải lại danh sách công việc; không tự mở nhầm form khác.');
      const picked=exact?{item:exact,cs:await claimStateFor(exact,man)}:await preferredMyAssignment(man,mine);
      if(!picked)throw new Error('Không xác định được phần công việc cần mở.');let item=picked.item,cs=picked.cs;if(cs.state==='STANDBY')throw new Error('Một người cùng nhóm đang chỉnh sửa biểu mẫu này. Khi họ hoàn tất, bấm NHẬN lại để lấy dữ liệu mới từ Firebase.');if(cs.state==='CO_DONE')cs={...cs,state:'READY'};if(cs.state==='SKIPPED')throw new Error('Phần công việc này đã được người phía sau NHẬN THAY và được ghi BỎ QUA · KHÔNG E-FORM.');if(cs.state==='DONE'){if(!confirm(`MỞ LẠI CHỈNH SỬA\n\n${rosterLabel(item)} · ${itemFormLabel(item)}\n\nPhần công việc này đã HOÀN TẤT. Bạn có chắc muốn mở lại để chỉnh sửa?`))return;const t=Date.now(),u=me(),aid=S(item.assignmentId),patch={},co=await acquireCoGroupClaim(d,item,man,{allowCompleted:true});const restored=await restoreOwnCompletedEnvelopeForReopen(item,man,cs);if(restored?.env){patch[`roster_sessions/${safe(aid)}/envelope`]=sanitizeEnvelope(restored.env);patch[`roster_sessions/${safe(aid)}/reopenRestoreSource`]=S(restored.source);patch[`roster_sessions/${safe(aid)}/reopenRestoreAtMs`]=t;}if(restored?.upstreamRevisionAtMs){patch[`roster_sessions/${safe(aid)}/upstreamAppliedAtMs`]=restored.upstreamRevisionAtMs;patch[`roster_sessions/${safe(aid)}/upstreamConflicts`]=restored.upstreamReview?.conflicts||[];patch[`roster_sessions/${safe(aid)}/upstreamReviewedBy`]=u;}
      patch[`roster_sessions/${safe(aid)}/claimStatus`]='CLAIMED';patch[`roster_sessions/${safe(aid)}/workPartStatus`]='IN_PROGRESS';patch[`roster_sessions/${safe(aid)}/taskStatusV333`]='IN_PROGRESS';patch[`roster_sessions/${safe(aid)}/taskStatusUpdatedAtMs`]=t;patch[`roster_sessions/${safe(aid)}/reopenedAtMs`]=t;patch[`roster_sessions/${safe(aid)}/reopenedBy`]=u;patch[`roster_sessions/${safe(aid)}/completedAtMs`]=null;patch[`roster_sessions/${safe(aid)}/completedBy`]=null;patch[`roster_sessions/${safe(aid)}/updatedAtMs`]=t;patch[`flight_records/${safe(d)}/${safe(fid)}/taskClaims/${safe(u)}/${safe(aid)}/status`]='CLAIMED';patch[`flight_records/${safe(d)}/${safe(fid)}/taskClaims/${safe(u)}/${safe(aid)}/taskStatus`]='IN_PROGRESS';patch[`flight_records/${safe(d)}/${safe(fid)}/taskClaims/${safe(u)}/${safe(aid)}/reopenedAtMs`]=t;patch[`flight_records/${safe(d)}/${safe(fid)}/taskClaims/${safe(u)}/${safe(aid)}/completedAtMs`]=null;patch[`roster_sessions/${safe(aid)}/coClaimedBy`]=co.gid?u:null;patch[`roster_sessions/${safe(aid)}/coClaimedAssignmentId`]=co.gid?aid:null;/* No peer-status writes on reopening: only the active roster assignment is changed. */ const ev=`WORK_PART_REOPEN_${t}_${safe(aid)}`;patch[`flight_records/${safe(d)}/${safe(fid)}/workPartHistory/${safe(ev)}`]={eventId:ev,assignmentId:aid,fromUser:u,atMs:t,type:'WORK_PART_REOPENED',status:'IN_PROGRESS',formGroup:S(item.formGroup),sourceColumn:S(item.sourceColumn),restoreSource:S(restored?.source||'')};await db('').update(patch);if(restored?.upstreamReview?.conflicts?.length)alert('A và B cùng chỉnh sửa '+restored.upstreamReview.conflicts.length+' ô. Để tránh mất dữ liệu, bản B được giữ ở các ô trùng. Vui lòng đối chiếu: '+restored.upstreamReview.conflicts.slice(0,8).join(', '));await addAudit('WORK_PART_REOPENED',{flightId:fid,flightLabel:rosterLabel(item),assignmentId:aid,fromUser:u,formGroup:S(item.formGroup),sourceColumn:S(item.sourceColumn),restoreSource:S(restored?.source||'')});cs={state:'CLAIMED',st:{...(cs.st||{}),claimStatus:'CLAIMED',taskStatusV333:'IN_PROGRESS',envelope:restored?.env||cs.st?.envelope},taskStatus:'IN_PROGRESS'};}if(cs.state==='WAIT'){const prev=previousItem(man,item);throw new Error(`Phần công việc trước của ${norm(prev?.user||prev?.targetUser)||'chưa xác định'} chưa hoàn tất.`)}if(cs.state!=='CLAIMED'){await markClaim(d,fid,item,man);multiAdd(fid,d)}else multiAdd(fid,d);await openAssignment(item,man);setTimeout(()=>decorateList(d),300)}catch(e){alert('Không mở được MY FLIGHT: '+S(e?.message||e))}};

  async function currentRosterWorkContext(){const meta=activeMeta(),aid=S(meta?.rosterAssignmentId);if(!aid)return null;const d=S(meta.rosterOpDate)||opDate(),man=await manifest(d,S(meta.rosterFlightId)),item=itemsOf(man).find(x=>S(x.assignmentId)===aid);if(!item||norm(item.user||item.targetUser)!==me())return null;const st=await sessionState(aid);return {d,man,item,next:null,skippedSameUser:[],coAssigneePeers:[],meta,st}}
  ...
  root.v1113ExportCurrentQr=()=>{alert('Không còn QR bàn giao theo tên người. Người có tên trong Daily Roster tự bấm NHẬN biểu mẫu.');return false};
  ...
  function v1134VzContext(){
    try{
      ...
      let env={};
      try{const sid=S(typeof activeFlightSessionId!=='undefined'?activeFlightSessionId:root.activeFlightSessionId);if(sid&&typeof root.readFlightSessionEnvelope==='function')env=root.readFlightSessionEnvelope(sid)||{}}catch(_){}
      return S(env.activeFormGroup||env.mainForm).toLowerCase()==='tvjgof035';
    }catch(_){return false}
  }
  function v1134QuickAllowed(){
    ...
    if(v1134VzContext()){
      try{
        ...
        if(S(meta?.rosterAssignmentId))return true;
      }catch(_){}
      if(r==='AD'||r==='DH')return true;
      try{
        const quick=(typeof ov.QUICK_TIME==='boolean')?ov.QUICK_TIME:(typeof root.v485Can==='function'&&root.v485Can('QUICK_TIME',profile,sess.role));
        const form=typeof root.v485Can==='function'&&root.v485Can('FSAGS423',profile,sess.role);
        return !!(quick||form);
      }catch(_){return false}
    }
  ...
  function v1134OpenQuickTime(){
    ...
    const g=v1134ActiveGroup();
    if(g==='fsags54'||g==='clc_checklist'){
      try{if(typeof root.sagsOpenGrndLsQuick==='function')return root.sagsOpenGrndLsQuick(g);return alert('NHẬP NHANH checklist chưa tải xong. Vui lòng thử lại sau giây lát.')}catch(e){return alert('Không mở được NHẬP NHANH checklist: '+S(e?.message||e))}
    }
  ...
  function v324HomeIdleVisible(){
    const home=document.getElementById('roleHomeIdle');if(!home)return false;
    try{return home.getAttribute('aria-hidden')==='false'&&getComputedStyle(home).display!=='none'&&getComputedStyle(home).visibility!=='hidden'}catch(_){return home.getAttribute('aria-hidden')==='false'}
  }
  function v324HideFormActionsAtHome(row,hb,qb,qt){
    if(!v324HomeIdleVisible())return false;
    row?.classList.remove('show','one','two','three');
    if(hb)hb.style.display='none';if(qb)qb.style.display='none';if(qt)qt.style.display='none';
    return true;
  }
  async function syncFormActions(){
    ...
    if(!row){
      ...
      row.innerHTML='<button id="v324ExportBtn" class="v324FormAction v324Export" type="button" title="Xuất / Chia sẻ">📤 XUẤT</button><button id="v1134QuickTimeBtn" class="v324FormAction" type="button" style="display:none" title="Nhập nhanh các mốc giờ">⏱ NHẬP NHANH</button><button id="v324HandoverBtn" class="v324FormAction v324Handover" type="button" style="display:none" title="Hoàn tất phần của tôi">✓ HOÀN TẤT</button><button id="v1113QrFormBtn" class="v324FormAction v324Qr" type="button" style="display:none">▣ XUẤT QR</button>';
    ...
    row.classList.remove('show','one','two','three');
    // Home/idle has no form context. Do not resurrect action buttons from a stale
    // activeMeta left by the previous flight, and skip RTDB context reads entirely.
    if(v324HideFormActionsAtHome(row,hb,qb,qt))return;
    const quickAllowed=v1134QuickAllowed();
    if(qt){qt.style.display=quickAllowed?'inline-flex':'none';qt.textContent='⏱ NHẬP NHANH';qt.onclick=v1134OpenQuickTime;}
    ...
    // OPEN SHARED 54/94: XUAT + NHAP NHANH are immediate; no legacy handover lookup.
    const metaNow=activeMeta(),groupNow=v1134ActiveGroup();
    if(metaNow?.rosterOpenShared===true&&['fsags54','loadcontrol_checklist','clc_checklist','fsags94','fsags94_clc'].includes(groupNow)){
      row.classList.add('show',quickAllowed?'two':'one');return;
    }
    let ctx=null;try{ctx=await currentHandoverContext()}catch(_){}
    if(v324HideFormActionsAtHome(row,hb,qb,qt))return;
    if(ctx){
      row.classList.add('show',quickAllowed?'three':'two');
      if(hb){hb.style.display='block';hb.textContent='✓ HOÀN TẤT';hb.title='Hoàn tất phần của tôi; người tiếp theo tự nhận theo Daily Roster';hb.setAttribute('aria-label','Hoàn tất phần của tôi');hb.onclick=()=>root.v324ConfirmRosterHandover?.()}
    ...
    if(v324HideFormActionsAtHome(row,hb,qb,qt))return;
    if(done){
      row.classList.add('show',quickAllowed?'two':'one');
      if(qb)qb.style.display='none';
      return
    }
    if(activeMeta()||quickAllowed)row.classList.add('show',quickAllowed?'two':'one')
  }


  // V5.0.3B: HOÀN TẤT only this roster assignment. There is no named handover.
  root.v324ConfirmRosterHandover=async function(){
    try{
      const ctx=await currentHandoverContext();
      if(!ctx)throw new Error('Không xác định được phần việc đang mở hoặc không còn quyền chỉnh sửa.');
      if(!confirm('HOÀN TẤT '+itemFormLabel(ctx.item)+'\n\n'+rosterLabel(ctx.item)+'\n\nLưu biểu mẫu lên Firebase và hoàn tất phần của bạn?'))return false;
      document.activeElement?.blur?.();
      await Promise.resolve(root.persist?.());
      await new Promise(resolve=>setTimeout(resolve,100));
      const raw=root.readFlightSessionEnvelope?.(ctx.meta.id)||null;
      if(!raw||!envelopeHasData(raw))throw new Error('Chưa đọc được dữ liệu biểu mẫu đã nhập trên máy; không đánh dấu HOÀN TẤT.');
      const env=sanitizeEnvelope(raw),t=Date.now(),aid=S(ctx.item.assignmentId),fid=S(ctx.item.flightId),actor=me();
      if(!aid||!fid||norm(ctx.item.user||ctx.item.targetUser)!==actor)throw new Error('Phân công đã thay đổi, vui lòng tải lại MY FLIGHT.');
      const current=await sessionState(aid);
      if(norm(current.claimedBy||current.ownerUser)!==actor||U(current.claimStatus)!=='CLAIMED')throw new Error('Quyền nhận phần việc đã thay đổi. Bản nháp trên máy vẫn được giữ.');
      const base=`roster_sessions/${safe(aid)}`;
      const patch={
        [`${base}/claimStatus`]:'PART_COMPLETED',
        [`${base}/workPartStatus`]:'COMPLETED',
        [`${base}/taskStatusV333`]:'COMPLETED',
        [`${base}/taskAvailabilityV333`]:'COMPLETED',
        [`${base}/taskStatusUpdatedAtMs`]:t,
        [`${base}/completedAtMs`]:t,
        [`${base}/completedBy`]:actor,
        [`${base}/completionEnvelope`]:env,
        [`${base}/completionEnvelopeAtMs`]:t,
        [`${base}/envelope`]:env,
        [`${base}/envelopeUpdatedAtMs`]:t,
        [`${base}/autoSkippedCoAssignee`]:false,
        [`${base}/updatedAtMs`]:t,
        [`flight_records/${safe(ctx.d)}/${safe(fid)}/taskClaims/${safe(actor)}/${safe(aid)}/status`]:'PART_COMPLETED',
        [`flight_records/${safe(ctx.d)}/${safe(fid)}/taskClaims/${safe(actor)}/${safe(aid)}/taskStatus`]:'COMPLETED',
        [`flight_records/${safe(ctx.d)}/${safe(fid)}/taskClaims/${safe(actor)}/${safe(aid)}/completedAtMs`]:t,
        [`roster_mail/${safe(actor)}/items/${safe(aid)}/claimStatus`]:'PART_COMPLETED',
        [`roster_mail/${safe(actor)}/items/${safe(aid)}/taskStatusV333`]:'COMPLETED',
        [`roster_mail/${safe(actor)}/items/${safe(aid)}/taskAvailabilityV333`]:'COMPLETED',
        [`roster_mail/${safe(actor)}/items/${safe(aid)}/updatedAtMs`]:t
      };
      const historyId='DONE_'+t+'_'+safe(aid);
      patch[`${base}/completionHistory/${safe(historyId)}`]={atMs:t,by:actor,envelope:env};
      patch[`flight_records/${safe(ctx.d)}/${safe(fid)}/workPartHistory/${safe(historyId)}`]={eventId:historyId,assignmentId:aid,fromUser:actor,atMs:t,type:'WORK_PART_COMPLETED',status:'COMPLETED',formGroup:S(ctx.item.formGroup),sourceColumn:S(ctx.item.sourceColumn)};
      const gid=coGroupId(ctx.item);
      // Release a co-assignee lock ONLY if this operator still owns exactly this generation.
      // Never rewrite another member's roster_sessions record.
      if(gid){
        const lockRef=db(`roster_co_claims/${safe(ctx.d)}/${safe(gid)}`),lock=(await lockRef.once('value')).val()||null;
        if(U(lock.status)==='CLAIMED'&&norm(lock.claimedBy)===actor&&S(lock.claimedAssignmentId)===aid){
          // Release using an owner/assignment-matching transaction AFTER our own completion is committed.
        }
      }
      // Firebase update must succeed before any success message or next user's access.
      await db('').update(patch);
      if(gid)try{
        const lockRef=db(`roster_co_claims/${safe(ctx.d)}/${safe(gid)}`);
        await lockRef.transaction(cur=>cur&&U(cur.status)==='CLAIMED'&&norm(cur.claimedBy)===actor&&S(cur.claimedAssignmentId)===aid?{...cur,status:'COMPLETED',completedAtMs:t,updatedAtMs:t}:undefined);
      }catch(e){console.warn('V5.0.2: group lock release retry on next receive',e)}
      root.sagsV477InvalidateQueueStatus?.();multiRemove(fid,ctx.d);
      try{await addAudit('ROSTER_OWN_FORM_COMPLETED',{flightId:fid,assignmentId:aid,fromUser:actor,formGroup:S(ctx.item.formGroup),savedOnFirebase:true})}catch(_){}
      alert('✓ ĐÃ HOÀN TẤT '+itemFormLabel(ctx.item)+'.\nDữ liệu đã được Firebase xác nhận lưu. Người có tên trong Daily Roster tự bấm NHẬN để mở tiếp.');
      setTimeout(()=>syncFormActions(),120);setTimeout(()=>decorateList(ctx.d),250);
      return true;
    }catch(e){alert('Chưa hoàn tất hoặc chưa đồng bộ được Firebase: '+S(e?.message||e)+'\nBản nháp trên máy vẫn được giữ.');return false;}
  };

  function patchList(){if(typeof root.flightWorkspaceOpenList==='function'&&!root.flightWorkspaceOpenList.__v324){const b=root.flightWorkspaceOpenList;const w=function(d){d=S(d)||opDate();const r=b.apply(this,arguments);sagsObserveCompletionV480(r,()=>{setTimeout(()=>decorateList(d),130);setTimeout(()=>decorateList(d),520)});return r};w.__v324=1;root.flightWorkspaceOpenList=w}}
  ...
  function patchFormHooks(){for(const name of ['switchFlightSession','showFormGroup','showRoleHomeIdle','hideRoleHomeIdle']){const fn=root[name];if(typeof fn!=='function'||fn.__v324)continue;const w=function(){const r=fn.apply(this,arguments);sagsObserveCompletionV480(r,()=>setTimeout(syncFormActions,70));return r};w.__v324=1;root[name]=w;try{if(name==='switchFlightSession')switchFlightSession=w;else if(name==='showFormGroup')showFormGroup=w;else if(name==='showRoleHomeIdle')showRoleHomeIdle=w;else if(name==='hideRoleHomeIdle')hideRoleHomeIdle=w}catch(_){}}}
...
(function(root){
  ...
  function ready(){const def=root.v485RoleDefaults||(typeof v485RoleDefaults==='function'?v485RoleDefaults:null);return !!document.querySelector('script[data-phase="control"][src*="app.v501.js"]')&&typeof root.v485Can==='function'&&typeof def==='function'&&typeof root.applyRoleUI==='function';}
  function extras(){
    if(role()==='AD')return [];
...
`;
  ...
  function sync(){
    ...
    const shift=document.getElementById('v310ShiftNav');if(shift)shift.style.display='none';
  ...
  let tries=0;const t=setInterval(()=>{if(install()||++tries>=32)clearInterval(t)},250);
})(typeof window!=='undefined'?window:globalThis);
...
(function(root){
  ...
  function labelForm(g){g=U(g);return g==='TVJGOF035'?'VZ Form':g==='FINAL'?'CBTT · FINAL':g==='FSAGS421'?'FSAGS 42.1':g==='FSAGS551'?'FSAGS 55.1':g==='FSAGS09'?'F/SAGS-CXR/09':g==='LOADING208'?'F/SAGS-CXR/208':'FSAGS 42.3'}
...
`;document.head.appendChild(st)}
  ...
  function install(){if(root.__SAGS_V327_INSTALLED===BUILD){syncNav();return true}if(!installPermission())return false;root.__SAGS_V327_INSTALLED=BUILD;ensureCss();ensureModal();disableLegacyHandover();syncNav();
    ...
    return true}
  let tries=0;const timer=setInterval(()=>{if(install()||++tries>=48)clearInterval(timer)},250);setTimeout(()=>install(),0);setTimeout(()=>install(),700);
})(typeof window!=='undefined'?window:globalThis);
...
(function(root){
  'use strict';
  const BUILD='V3.33-20260915-DATA-SAVER';
  ...
  function availability(st,item,man,allSessions){const status=derive(st,item,man,allSessions);if(status===STATUS.COMPLETED)return 'COMPLETED';if(status===STATUS.BLOCK)return 'BLOCKED';if(status===STATUS.NOT_APPLICABLE)return 'NOT_APPLICABLE';const owner=norm(item?.user||item?.targetUser||st?.ownerUser),coOwner=norm(st?.coClaimedBy);if(coGroupId(item)&&coOwner&&owner&&coOwner!==owner)return 'STANDBY';/* V4.8.3: no hard WAITING_PREVIOUS gate between separately assigned work parts. */return status===STATUS.IN_PROGRESS?'ACTIVE':'READY'}
  ...
  const LITE_SESSION_FIELDS=['taskStatusV333','taskStatus','taskAvailabilityV333','claimStatus','workPartStatus','ownerUser','coClaimedBy','skippedNoEform','autoSkippedByNextUser','skippedBy','skippedAtMs','taskStatusUpdatedAtMs','updatedAtMs'];
  async function readLiteSession(aid){const out={};await Promise.all(LITE_SESSION_FIELDS.map(async k=>{try{const x=await db(`roster_sessions/${safe(aid)}/${k}`).once('value');const v=x.val();if(v!==null&&v!==undefined)out[k]=v}catch(_){}}));return out}
  async function readOldTask(date,fid,aid){if(!fid)return null;try{return (await db(`flight_records/${safe(date)}/${safe(fid)}/taskStatus/${safe(aid)}`).once('value')).val()||null}catch(_){return null}}
  async function syncDate(date=opDate(),force=false){date=S(date)||opDate();if(!date||typeof root.sagsV470Ref!=='function')return {ok:false,count:0};const mark=`sagsTaskStatusV333:${date}`;if(!force){try{if(sessionStorage.getItem(mark)===BUILD)return {ok:true,skipped:true,count:0}}catch(_){}}
    // V4.2.53 DATA SAVER: never read the global roster_sessions tree or full flight_records day.
    // Only read compact status leaves for assignments that actually exist in today's manifest.
    const ms=await db(`roster_manifests/${safe(date)}`).once('value'),man=ms.val()||{},items=activeItems(man),sessions={},oldTasks={},patch={},now=Date.now();let changed=0;
    await Promise.all(items.map(async item=>{const aid=S(item.assignmentId);if(!aid)return;sessions[aid]=await readLiteSession(aid)}));
    await Promise.all(items.map(async item=>{const aid=S(item.assignmentId);if(!aid)return;const fid=itemFlightId(man,item,date);if(fid)oldTasks[aid]=await readOldTask(date,fid,aid)}));
    for(const item of items){const aid=S(item.assignmentId);if(!aid)continue;const st=sessions[aid]||{},sum=summary(item,st,man,sessions,date),fid=itemFlightId(man,item,date);if(S(st.taskStatusV333)!==sum.status){patch[`roster_sessions/${safe(aid)}/taskStatusV333`]=sum.status;patch[`roster_sessions/${safe(aid)}/taskStatusUpdatedAtMs`]=now;changed++}if(S(st.taskAvailabilityV333)!==sum.availability){patch[`roster_sessions/${safe(aid)}/taskAvailabilityV333`]=sum.availability;changed++}if(fid){const old=oldTasks[aid];if(!sameSummary(old,sum)){sum.updatedAtMs=now;patch[`flight_records/${safe(date)}/${safe(fid)}/taskStatus/${safe(aid)}`]=sum;changed++}}
    }
    if(Object.keys(patch).length)await db('').update(patch);try{sessionStorage.setItem(mark,BUILD)}catch(_){}root.__SAGS_V333_LAST_SYNC={date,atMs:now,assignments:items.length,changes:changed,dataSaver:true};return {ok:true,count:items.length,changes:changed}
  }
  ...
  function install(){if(root.__SAGS_V333_INSTALLED===BUILD)return true;root.__SAGS_V333_INSTALLED=BUILD;wrap('dailyRosterPublish',async r=>{if(r===true&&!root.__SAGS_ROSTER_LAST_DELTA?.noChange)await syncDate(S(document.getElementById('drManageDate')?.value)||opDate(),true)},'__v333');wrap('flightWorkspaceOpenList',async(_,args)=>{await syncDate(S(args?.[0])||opDate(),false)},'__v333');wrap('v324ReceiveOrOpen',async()=>{await syncDate(opDate(),false)},'__v333');wrap('v324ConfirmRosterHandover',async()=>{await syncDate(opDate(),true)},'__v333');wrap('v327Reassign',async()=>{await syncDate(opDate(),true)},'__v333');setTimeout(()=>syncDate(opDate(),false).catch(()=>{}),800);return true}
  install();setTimeout(install,350);setTimeout(install,1200);
})(typeof window!=='undefined'?window:globalThis);
...
(function(root){'use strict';
  ...
  const canonicalFormGroup=v=>({FSAGS:'fsags',FSAGS423:'fsags',FSAGS421:'fsags421',FSAGS551:'fsags551',FSAGS09:'fsags09',TVJGOF035:'tvjgof035',TVJ_GOF_035:'tvjgof035',FINAL:'final',UNIT_TASK:'unit_task',FSAGS54:'FSAGS54',CLC_CHECKLIST:'clc_checklist'}[U(v)]||'');
  ...
  function install(){ensureUi();const fn=root.flightWorkspaceOpenList;if(typeof fn==='function'&&!fn.__v340){const w=function(){const r=fn.apply(this,arguments);sagsObserveCompletionV480(r,()=>{setTimeout(injectButton,40);setTimeout(injectButton,420)});return r};w.__v340=true;w.__v340Base=fn;root.flightWorkspaceOpenList=w;try{flightWorkspaceOpenList=w}catch(_){}}setTimeout(injectButton,80)}
  install();setTimeout(install,400);setTimeout(install,1300);/* V4.2.55 DATA SAVER: không quét toàn bộ roster_mail khi startup. repairMyManualAssignments vẫn được gọi khi người dùng chọn đồng bộ/sửa thủ công. */
 ...
 * One persistent place for unfinished approvals, handoffs, crosscheck and manual-flight recovery.
...
const state={user:"",tab:"all",items:[],sources:{approval:[],cross:[],manual:[]},loadedAt:{},pendingSources:new Set(),pendingAll:false,errors:[],loading:false,connected:null,lastRefresh:0,refreshTimer:0,unsubs:[]};
...
function loadCache(){try{const x=JSON.parse(localStorage.getItem(cacheKey())||"{}"),saved=Number(x.savedAtMs||0);state.items=(Array.isArray(x.items)?x.items:[]).filter(v=>["APPROVAL","HANDOFF","CROSS","MANUAL"].includes(String(v?.source||""))&&afterReset(v));state.lastRefresh=saved;state.sources.approval=state.items.filter(v=>v.source==="APPROVAL");state.sources.handoff=state.items.filter(v=>v.source==="HANDOFF");state.sources.cross=state.items.filter(v=>v.source==="CROSS");state.sources.manual=state.items.filter(v=>v.source==="MANUAL");for(const key of Object.keys(state.sources))state.loadedAt[key]=saved}catch(_){state.items=[]}}
...
const LOADERS={approval:loadApproval,cross:loadCross,manual:loadManual};
const WORK_SOURCES=["approval","cross","manual"];
...
root.sagsV342RetrySync=async function(silent=false){if(navigator.onLine===false){if(!silent)alert("Thiết bị đang mất mạng. Dữ liệu cục bộ vẫn được giữ; hãy thử lại khi có kết nối.");return false}const text=document.getElementById("v342SyncText");if(text)text.textContent="ĐANG THỬ ĐỒNG BỘ LẠI…";try{await root.sagsV341RepairManualAssignments?.();try{root.dailyRosterRestartMailbox?.()}catch(_){}await refresh(true,WORK_SOURCES);if(!silent)alert("✓ Đã kiểm tra kết nối và đồng bộ lại các nguồn công việc.");return true}catch(e){if(!silent)alert("Chưa đồng bộ lại được: "+S(e?.message||e));return false}};

function install(){ensureUi();ensureButton();watchToolbar();const u=me();if(u!==state.user){state.user=u;state.sources={approval:[],cross:[],manual:[]};state.loadedAt={};state.pendingSources.clear();state.pendingAll=false;clearLegacyCache();loadCache();startRealtime();/* V4.2.55 DATA SAVER: panel CẦN XỬ LÝ đang ẩn, không tự tải approval/cross/roster_mail ở startup. Chỉ refresh khi panel được mở hoặc người dùng yêu cầu đồng bộ. */if(document.getElementById('v342Modal')?.classList.contains('show'))scheduleRefresh(450,WORK_SOURCES)}else{updateBadge();renderSync()}}
...
root.__SAGS_V342_TEST__={ACTION_RESET_AT_MS,ACTION_RESET_LABEL,itemAtMs,afterReset,activeItem,overdue,statusLabel,ageLabel,dueLabel,loadCross,loadManual,sourceKeys,WORK_SOURCES};
})(typeof window!=="undefined"?window:globalThis);
  ...
  try{
    ...
    if(!proto.get.__v390Perf){
      ...
      const getWrap=function(...args){
        ...
        if(path){
          inflight.set(key,p);
          // A detached finally() would create another rejected promise on read
          // failure; clean up on either outcome without changing the caller's p.
          void p.then(()=>{if(inflight.get(key)===p)inflight.delete(key);},
...
`;document.head.appendChild(st);
  ...
  const baseApply=root.applyRoleUI;if(typeof baseApply==='function'&&!baseApply.__v1111qr){const w=function(){const r=baseApply.apply(this,arguments);sagsObserveCompletionV480(r,scheduleConsume);return r};w.__v1111qr=1;root.applyRoleUI=w;try{applyRoleUI=w}catch(_){}}
...
/* V6.3.25: legacy checklist modal removed. F/SAGS-CXR/54 and /94 are now native pages 16/17 in grnd-ls-checklists.v6325.js. */
