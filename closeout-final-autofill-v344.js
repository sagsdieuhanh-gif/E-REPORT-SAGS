/* E-REPORT/SAGS V3.44 · Approved PVHK closeout -> durable CBTT FINAL autofill
 * The approved closeout is indexed by DATE + departure FLIGHT so CBTT can
...
function allPayloadFlights(p){const i=p?.identity||{},f=p?.f09||{};return [...new Set([...(Array.isArray(i.flights)?i.flights:[]),f.f09_fltBefore,f.f09_fltAfter].map(flight).filter(Boolean))]}
function targetFlights(p){const dep=flight(p?.f09?.f09_fltAfter);if(dep)return [dep];const fs=allPayloadFlights(p);return fs.length?[fs[fs.length-1]]:[]}
function payloadRevision(p){return Math.max(1,Number(p?.closeoutNo||p?.revisionNo||1)||1)}
function payloadKey(p){return payloadDate(p)+"|"+(targetFlights(p)[0]||"")}
function cachePayload(p){if(!p)return;const box=readInbox(),no=payloadRevision(p),dt=payloadDate(p);for(const f of targetFlights(p)){if(!dt||!f)continue;const key=dt+"|"+f,old=box[key]||{},versions=Array.isArray(old.versions)?old.versions.filter(x=>payloadRevision(x)!==no):[];versions.push(clone(p));versions.sort((a,b)=>payloadRevision(a)-payloadRevision(b)||Number(a?.submittedAtMs||0)-Number(b?.submittedAtMs||0));const latest=versions[versions.length-1]||clone(p);box[key]={latest,versions:versions.slice(-5),updatedAtMs:Number(latest?.submittedAtMs||Date.now())}}writeInbox(box)}
...
function recordIdentity(rec,data){return {dateToken:dateToken(data?.date||rec?.dateToken||rec?.opDate),flight:flight(data?.flight||rec?.flightToken),reg:reg(data?.acreg||rec?.acRegToken)}}
function matches(rec,data,p){const a=recordIdentity(rec,data),dt=payloadDate(p),fs=targetFlights(p);if(!a.dateToken||!a.flight||a.dateToken!==dt||!fs.includes(a.flight))return false;const pr=reg(p?.identity?.acRegToken||p?.f09?.f09_regn);return !(a.reg&&pr&&a.reg!==pr)}
...
function ensureFinalRecord(list,p){const dt=payloadDate(p),flt=targetFlights(p)[0]||"";if(!dt||!flt)return {record:null,created:false,resolved:false};let rec=list.find(r=>{const x=readRecordData(r);return matches(r,x.data,p)})||list.find(r=>S(r.closeoutAutoKeyV344)===dt+"|"+flt),created=false,resolved=false;const form=formForPayload(p),now=Date.now();
  if(!rec){if(typeof root.makeFinalSheetId!=="function")return {record:null,created:false,resolved:false};rec={id:root.makeFinalSheetId(),name:flt+(form?" · "+formLabel(form):" · CHỜ MẪU"),form:form||"",airline:form?formLabel(form):"CHỜ MẪU",flightSessionId:"",dateToken:dt,flightToken:flt,acRegToken:reg(p?.identity?.acRegToken||p?.f09?.f09_regn),createdAt:now,updatedAt:now,autoCreatedFromCloseoutV344:true,closeoutAutoKeyV344:dt+"|"+flt};list.push(rec);created=true}
  ...
  const {all,data}=readRecordData(rec),seed=identitySeed(p,rec.form);for(const [k,v] of Object.entries(seed))if(S(v)&&!S(data[k]))data[k]=v;all[rec.form]=data;try{localStorage.setItem(root.finalSheetDataKey(rec.id),JSON.stringify(all))}catch(_){}rec.dateToken=dt;rec.flightToken=flt;if(!S(rec.acRegToken))rec.acRegToken=reg(p?.identity?.acRegToken||p?.f09?.f09_regn);rec.updatedAt=now;return {record:rec,created,resolved}}
...
function applyAll(p,{notify=true,createMissing=true}={}){if(!p||!["CBTT","AD"].includes(role()))return {matched:0,changed:0,conflicts:0,created:0,pending:0};cachePayload(p);let list=[];try{list=root.readFinalSheetList?.()||[]}catch(_){return {matched:0,changed:0,conflicts:0,created:0,pending:0}}let created=0,pending=0;if(createMissing){const e=ensureFinalRecord(list,p);if(e.created)created++;if(e.record?.templatePendingV340)pending++}
  let matched=0,changed=0,conflicts=0;for(const rec of list){const r=applyRecord(rec,p);if(!r.matched)continue;matched++;if(r.changed)changed++;if(r.pendingTemplate)pending++;conflicts+=r.conflicts.length+(r.locked?1:0);refreshActive(rec,r)}if(matched||created)try{root.writeFinalSheetList?.(list)}catch(_){}
  if(notify&&(changed||created)){let msg=changed?`✓ KẾT SỔ LẦN ${payloadRevision(p)} ĐÃ TỰ ĐIỀN VÀO ${changed} TỜ FINAL.`:`✓ ĐÃ TẠO FINAL CHỜ DỮ LIỆU KẾT SỔ.`;if(pending&&!changed)msg="✓ ĐÃ NHẬN KẾT SỔ · FINAL ĐANG CHỜ XÁC ĐỊNH ĐÚNG MẪU.";try{root.showToast?.(msg)}catch(_){}}return {matched,changed,conflicts,created,pending}}
...
async function durableForRecord(rec){if(typeof root.sagsV470Ref!=="function")return null;const {data}=readRecordData(rec),id=recordIdentity(rec,data);if(!id.dateToken||!id.flight)return null;const snap=await root.sagsV470Ref(`${INDEX_ROOT}/${safe(id.dateToken)}/${safe(id.flight)}`).once("value"),v=snap.val?.()||null;return v?.payload||v||null}
async function firestoreForRecord(rec){const {data}=readRecordData(rec),id=recordIdentity(rec,data);if(!id.dateToken||!id.flight||typeof root.initHandoverFirebase!=="function")return null;const db=root.initHandoverFirebase(),arr=[];if(id.reg&&typeof root.fs09MakeMatchKey==="function"){const key=root.fs09MakeMatchKey(id.dateToken,id.flight,id.reg,"CXR");if(key){const snap=await db.collection(collectionName()).where("matchKeys","array-contains",key).limit(12).get();snap.forEach(doc=>{const d=doc.data()||{};if(U(d.kind)==="FSAGS09_CLOSEOUT")arr.push(d)})}}
  if(!arr.length){const snap=await db.collection(collectionName()).where("kind","==","fsags09_closeout").limit(80).get();snap.forEach(doc=>{const d=doc.data()||{};if(payloadDate(d)===id.dateToken&&targetFlights(d).includes(id.flight))arr.push(d)})}return arr.sort((a,b)=>payloadRevision(b)-payloadRevision(a)||Number(b?.submittedAtMs||0)-Number(a?.submittedAtMs||0))[0]||null}
async function syncRecord(id,{notify=true}={}){const rec=(root.readFinalSheetList?.()||[]).find(x=>S(x.id)===S(id));if(!rec)return false;let found=false;const cached=cachedForRecord(rec);if(cached){applyAll(cached,{notify,createMissing:false});found=true}try{const d=await durableForRecord(rec);if(d){applyAll(d,{notify,createMissing:false});found=true;return true}}catch(e){console.info("V3.44 durable closeout",e?.message||e)}try{const cloud=await firestoreForRecord(rec);if(cloud){applyAll(cloud,{notify,createMissing:false});found=true}}catch(e){console.info("V3.44 closeout query",e?.message||e)}return found}
async function syncAll({notify=false}={}){if(!["CBTT","AD"].includes(role()))return 0;const list=root.readFinalSheetList?.()||[],seen=new Set();let n=0;for(const rec of list){const {data}=readRecordData(rec),id=recordIdentity(rec,data),k=id.dateToken+"|"+id.flight;if(!id.dateToken||!id.flight||seen.has(k))continue;seen.add(k);if(await syncRecord(rec.id,{notify}))n++}return n}
async function syncAssignedCloseouts({notify=false}={}){if(role()!=="CBTT"||!me()||typeof root.sagsV470Ref!=="function")return 0;let items={};try{items=(await root.sagsV470Ref(`roster_mail/${safe(me())}/items`).once("value")).val?.()||{}}catch(e){console.info("V3.44 assigned FINAL mailbox",e?.message||e);return 0}const wanted=new Map();for(const x of Object.values(items)){if(!x||x.active===false||U(x.formGroup)!=="FINAL")continue;const dt=dateToken(x.opDate||x.depFlightDate||x.date),flt=flight(x.depFlight||x.flightToken||x.flightRaw);if(dt&&flt)wanted.set(dt+"|"+flt,{dt,flt})}let n=0;for(const {dt,flt} of wanted.values())try{const snap=await root.sagsV470Ref(`${INDEX_ROOT}/${safe(dt)}/${safe(flt)}`).once("value"),v=snap.val?.()||null,p=v?.payload||v||null;if(p){applyAll(p,{notify,createMissing:true});n++}}catch(e){console.info("V3.44 assigned closeout",dt,flt,e?.message||e)}return n}
