...
function workSlotKey(date,x){const src=slotSource(x),form=src==='GRND_LS'?canonicalForm(x):'';return [S(date),flightIdentity(x),src,form,U(x?.assignmentLeg)||'TURN',Number(x?.workPartOrder||1)].join('|')}
...
function canonicalForm(x){const g=U(x?.formGroup);if(g==='FSAGS'||g==='FSAGS423')return 'FSAGS423';if(g==='LOADCONTROL_CHECKLIST')return 'LOADCONTROL_CHECKLIST';if(g==='CLC_CHECKLIST')return 'CLC_CHECKLIST';return g||'FORM'}
...
function cardHtml(g,date){
  ...
  const actions=''; // One form button per type; never add a second generic receive/continue button.
  return `<div class="v1199Card" data-fkey="${esc(g.key)}"><div class="v1199Title">${esc(flightLabel(x))}</div><div class="v1199Meta">${esc(route)}${route?' · ':''}A/C ${esc(ac)} · STA ${esc(sta)} · STD ${esc(std)}</div><div class="v1199Tasks">${taskPills(g,date)}</div>${g.pushback?`<div class="v1199OwnerNote">PUSHBACK ${esc(g.pushback)}</div>`:''}${actions}</div>`;
}
