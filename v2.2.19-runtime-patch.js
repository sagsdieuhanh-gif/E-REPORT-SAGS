...
@media(max-width:640px){#sagsAlignLibraryList{grid-template-columns:1fr}.sagsLibTools{grid-template-columns:1fr}.sagsLibPanel{padding:10px}}
`;
  ...
  function ensureUi(){
    ...
    if(!$('sagsAlignLibrary')){
      ...
      $('sagsLibClose').onclick=closeLibrary;$('sagsLibRefresh').onclick=renderList;$('sagsLibSearch').oninput=renderList;
  ...
  function renderList(){
    ...
    if(!items.length){list.innerHTML='<div class="sagsLibEmpty"><b>Chưa tìm thấy biểu mẫu phù hợp.</b><br>Mở một chuyến bất kỳ hoặc bấm QUÉT LẠI. Các mẫu nhúng trong ứng dụng sẽ tự xuất hiện khi đã nạp.</div>';return}
  ...
  root.sagsFormLibraryList=()=>libraryItems().map(x=>({kind:x.kind,label:x.label,fieldCount:x.fieldCount||0,disabled:!!x.disabled}));
