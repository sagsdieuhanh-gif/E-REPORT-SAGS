...
async function initModels(force=false){
  ...
  if(!conf.apiKey||!conf.appId)throw new Error("cấu hình Firebase chưa có đủ Web App config của e-report-sags (thiếu apiKey/appId).");
...
function classifyAiError(e,stage=""){
  ...
  if(st==="AI_TIMEOUT"||/timeout has expired|timed? out|deadline exceeded|aborterror/.test(x)){
    ...
    d={...d,code:"AI-400-API_KEY_INVALID",where:"cấu hình Firebase → apiKey",summary:"API key Firebase Web không hợp lệ hoặc không đúng Web App E-REPORT.",action:"Kiểm tra Web App config của project e-report-sags."};
...
window.sagsAiRunDiagnostics=async()=>{
  ...
  try{
    ...
    if(!conf.apiKey||!conf.appId)throw taggedError("CONFIG_FIREBASE",new Error("Thiếu apiKey/appId trong cấu hình Firebase."));
