...
window.__SAGS_RUNTIME_BUILD__="V5.0.3B-HF1-LAYERED-PDF-AD-RESET-VISIBLE";
...
function mergeRegistry(){const local=isAD()?loadLocal():null;draft=local?.forms?.length?local:clone(published);draft.schema=2;draft.version='V4.8.10';draft.build=BUILD;draft.forms=Array.isArray(draft.forms)?draft.forms:[];hydrateLegacyForms();fmNormalizeRegistryV487(draft);syncLegacyToLive(false);return draft}
...
async function previewCustomPageV45(f,p){status('Đang dựng PDF preview…');try{const src=await pageSrc(p);if(!src)throw new Error('Trang chưa có ảnh nền.');const im=await new Promise((res,rej)=>{const q=new Image();q.onload=()=>res(q);q.onerror=rej;q.src=src}),c=document.createElement('canvas');c.width=Number(p.width||im.naturalWidth||1241);c.height=Number(p.height||im.naturalHeight||1755);const x=c.getContext('2d');x.drawImage(im,0,0,c.width,c.height);for(const raw of (f.fields||[]).filter(z=>z.pageId===p.id)){const fld=fmNormalizeFieldV487(raw),v0=testValueV45(fld),v=fld.type==='static-text'?(fld.text||fld.label||''):v0,px=Number(fld.x||0)*c.width,py=Number(fld.y||0)*c.height,pw=Number(fld.w||.1)*c.width,ph=Number(fld.h||.03)*c.height,st=fmStyleV487(fld);if(fld.type==='checkbox'){if(v){x.save();x.font=`900 ${Math.max(10,st.fontSize)}px Arial`;x.textAlign='center';x.textBaseline='middle';x.fillStyle='#111';x.fillText('✓',px+pw/2,py+ph/2);x.restore()}continue}if(fld.type==='signature')continue;x.save();x.beginPath();x.rect(px,py,pw,ph);x.clip();x.font=`${st.fontStyle} ${st.fontWeight} ${st.fontSize}px '${st.fontFamily}'`;x.fillStyle=st.textColor;x.textAlign=st.align==='right'?'right':st.align==='center'?'center':'left';x.textBaseline='alphabetic';const tx=st.align==='right'?px+pw-2:st.align==='center'?px+pw/2:px+2,lh=st.fontSize*st.lineHeight,lines=(fmIsMultiV489(fld)||String(v??'').includes('\n'))?fmWrapCanvasV487(x,String(v??''),Math.max(8,pw-4)):[String(v??'')],max=fmIsMultiV489(fld)?fmLineCapacityV489(fld,p):Math.max(1,Math.floor(ph/Math.max(1,lh))),visible=Math.min(lines.length,max),first=fmCanvasBaselineV490(st,py,ph,visible,lh);lines.slice(0,max).forEach((line,i)=>{x.fillText(line,tx,first+i*lh);if(st.underline&&line){const w=x.measureText(line).width,ux=st.align==='right'?tx-w:st.align==='center'?tx-w/2:tx;x.fillRect(ux,first+i*lh+2,w,Math.max(1,st.fontSize/16))}});x.restore()}showCanvasPreviewV45(c,`${f.name||f.id} · Trang ${pageIndex+1}`);status('✓ PDF preview dùng cùng tọa độ + typography của Form Manager.')}catch(e){status('Không preview được: '+(e?.message||e),true)}}
...
function layerCfg(landscape=false){if(landscape){const w=isMobile()?1120:1260;return {w,h:Math.round(w*1055/1491),quality:.84}}const w=isMobile()?900:1000;return {w,h:Math.round(w*baseH()/baseW()),quality:.84}}
function bgPath(pageNo){return `./pdf-bg-page${Number(pageNo)}.jpg`}
async function fetchBytes(url){if(bgByteCache.has(url))return bgByteCache.get(url);const job=fetch(url,{cache:'force-cache'}).then(r=>{if(!r.ok)throw new Error(`Không tải được PDF background ${url}`);return r.arrayBuffer()}).then(a=>new Uint8Array(a));bgByteCache.set(url,job);return job}
async function fastPage(pageNo,landscape=false){const t0=perf(),page=document.getElementById('page'+pageNo),svg=document.getElementById('svg'+pageNo);if(!page||!svg)throw new Error('Không tìm thấy Trang '+pageNo);const cfg=layerCfg(landscape),c=document.createElement('canvas');c.width=cfg.w;c.height=cfg.h;c.__sagsOriginalWidth=c.width;c.__sagsOriginalHeight=c.height;c.__sagsLayered=true;c.__sagsLayeredBg=bgPath(pageNo);c.__sagsLayeredBgWidth=landscape?1491:baseW();c.__sagsLayeredBgHeight=landscape?1055:baseH();c.__sagsPageNo=Number(pageNo);const ctx=c.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);const sx=c.width/baseW(),sy=c.height/baseH(),t1=perf();const exactOverlay=await sagsPdfExactSvgOverlay(svg,pageNo);ctx.drawImage(exactOverlay,0,0,c.width,c.height);const t2=perf();await drawSignatures(ctx,pageNo,sx,sy);if(Number(pageNo)===4&&!root.__SAGS_SUPPRESS_BBBT_CXR_ON_RENDER&&stateObj().bbbtCxrNo!==undefined&&stateObj().bbbtCxrNo!==null&&stateObj().bbbtCxrNo!==''){ctx.save();ctx.font=`900 ${28*sy}px Times New Roman`;ctx.fillStyle='#b54032';ctx.textAlign='right';ctx.textBaseline='top';let n=stateObj().bbbtCxrNo;try{if(typeof formatBBBTCxrNo==='function')n=formatBBBTCxrNo(n)}catch(_){}ctx.fillText('CXR No: '+n,(baseW()-42)*sx,30*sy);ctx.restore()}const t3=perf(),row={page:Number(pageNo),layered:true,width:c.width,height:c.height,overlayMs:+(t2-t1).toFixed(1),signatureMs:+(t3-t2).toFixed(1),totalMs:+(t3-t0).toFixed(1)};const P=root.__SAGS_V450_PDF_RUN;if(P){P.pages.push(row);P.renderMs=(P.renderMs||0)+row.totalMs}return c}
async function singlePageData(c,landscape){if(c?.__sagsLayered){const [bg,overlay]=await Promise.all([fetchBytes(c.__sagsLayeredBg),canvasToJpegBytes(c,layerCfg(landscape).quality)]);return {layered:true,bg,overlay,bgW:c.__sagsLayeredBgWidth,bgH:c.__sagsLayeredBgHeight,ovW:c.width,ovH:c.height}}let work=c;if(isMobile()&&c&&c.width>(landscape?1260:1000)){const target=landscape?1260:1000,r=target/c.width,small=document.createElement('canvas');small.width=target;small.height=Math.round(c.height*r);const x=small.getContext('2d',{alpha:false});x.fillStyle='#fff';x.fillRect(0,0,small.width,small.height);x.drawImage(c,0,0,small.width,small.height);work=small}const img=await canvasToJpegBytes(work,landscape?.84:.82);const out={layered:false,img,w:work.width,h:work.height};if(work!==c){try{work.width=1;work.height=1}catch(_){}}return out}
function imageObj(bytes,w,h){return cat([b(`<< /Type /XObject /Subtype /Image /Width ${w} /Height ${h} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${bytes.length} >>\nstream\n`),bytes,b('\nendstream')])}
function mixedPdf(pages,fileName,landscape=false){const objects=[],catalogId=1,pagesId=2,gsId=3,pageIds=[];let next=4;const ids=[];for(const p of pages){if(p.layered){const row={pageId:next++,bgId:next++,ovId:next++,contentId:next++};ids.push(row);pageIds.push(row.pageId)}else{const row={pageId:next++,imgId:next++,contentId:next++};ids.push(row);pageIds.push(row.pageId)}}objects[catalogId]=b(`<< /Type /Catalog /Pages ${pagesId} 0 R >>`);objects[pagesId]=b(`<< /Type /Pages /Count ${pages.length} /Kids [${pageIds.map(x=>`${x} 0 R`).join(' ')}] >>`);objects[gsId]=b('<< /Type /ExtGState /BM /Multiply >>');const PW=landscape?841.89:595.28,PH=landscape?595.28:841.89;for(let i=0;i<pages.length;i++){const p=pages[i],r=ids[i];if(p.layered){objects[r.pageId]=b(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${PW} ${PH}] /Resources << /XObject << /Bg${i+1} ${r.bgId} 0 R /Ov${i+1} ${r.ovId} 0 R >> /ExtGState << /GS1 ${gsId} 0 R >> >> /Contents ${r.contentId} 0 R >>`);objects[r.bgId]=imageObj(p.bg,p.bgW,p.bgH);objects[r.ovId]=imageObj(p.overlay,p.ovW,p.ovH);const content=b(`q\n${PW} 0 0 ${PH} 0 0 cm\n/Bg${i+1} Do\nQ\nq\n/GS1 gs\n${PW} 0 0 ${PH} 0 0 cm\n/Ov${i+1} Do\nQ\n`);objects[r.contentId]=cat([b(`<< /Length ${content.length} >>\nstream\n`),content,b('endstream')])}else{objects[r.pageId]=b(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${PW} ${PH}] /Resources << /XObject << /Im${i+1} ${r.imgId} 0 R >> >> /Contents ${r.contentId} 0 R >>`);objects[r.imgId]=imageObj(p.img,p.w,p.h);const content=b(`q\n${PW} 0 0 ${PH} 0 0 cm\n/Im${i+1} Do\nQ\n`);objects[r.contentId]=cat([b(`<< /Length ${content.length} >>\nstream\n`),content,b('endstream')])}}const parts=[b('%PDF-1.4\n')],offs=new Array(objects.length).fill(0);let pos=parts[0].length;for(let id=1;id<objects.length;id++){offs[id]=pos;const h=b(`${id} 0 obj\n`),t=b('\nendobj\n');parts.push(h,objects[id],t);pos+=h.length+objects[id].length+t.length}const xrefPos=pos;let xref=`xref\n0 ${objects.length}\n0000000000 65535 f \n`;for(let id=1;id<objects.length;id++)xref+=String(offs[id]).padStart(10,'0')+' 00000 n \n';xref+=`trailer\n<< /Size ${objects.length} /Root ${catalogId} 0 R >>\nstartxref\n${xrefPos}\n%%EOF`;parts.push(b(xref));return pdfPayload(cat(parts),fileName)}
async function layeredPdf(canvases,fileName,landscape=false){const t=perf(),out=new Array(canvases.length),step=isIOS()?1:2;for(let i=0;i<canvases.length;i+=step){const batch=canvases.slice(i,i+step),rows=await Promise.all(batch.map(c=>singlePageData(c,landscape)));rows.forEach((x,j)=>out[i+j]=x);if(isIOS())await new Promise(r=>setTimeout(r,0))}const P=root.__SAGS_V450_PDF_RUN;if(P)P.encodeCoreMs=(P.encodeCoreMs||0)+(perf()-t);return mixedPdf(out,fileName,landscape)}
