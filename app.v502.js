...
window.sagsShiftReadArchive=async function(day,known){
 const q=await initHandoverFirebase().collection(HANDOVER_COLLECTION).where("archiveDayKey","==",day).get(),parents=[];
 q.forEach(d=>{const x=d.data()||{};if(x.kind===V488_ARCHIVE_KIND)parents.push({...x,__docId:d.id})});
 const out=[];
 for(const parent of parents){
  const p=await v488LoadSnapshot(parent),m=p.sessionMeta||{},st=p.envelope?.state||{},fid=String(m.rosterFlightId||m.flightId||''),arr=String(st.fltBefore||st.f421_fltBefore||''),dep=String(st.fltAfter||st.f421_fltAfter||''),reg=String(p.identity?.acRegToken||st.regn||st.f421_regn||'').toUpperCase();
  const hit=[...known,...out].find(r=>r.opDate===day&&((fid&&r.flightId===fid)||((arr||dep)&&r.arrFlight===arr&&r.depFlight===dep&&String(r.acReg||'').toUpperCase()===reg)));
  if(hit)continue;
  out.push({flightId:fid||parent.__docId,opDate:day,arrFlight:arr,depFlight:dep,flightName:p.flightName||parent.flightName,acReg:reg,sta:st.sta||st.f421_sta||'',std:st.std||st.f421_std||'',etd:st.etd||st.f421_etd||'',route1:st.route1||st.f421_route1||'',route3:st.route3||st.f421_route3||'',assignments:{},_ramp:{chockOn:st.h5Start||st.f421_h5Start||'',doorClose:st.h21Start||st.f421_h21Start||'',pushback:st.h24Start||st.f421_h24Start||''},_source:'Hồ sơ lưu trữ '+parent.__docId});
 }
 return out;
};
...
(function(root){'use strict';
  const BUILD='V5.0.2B-DIRECT-ROSTER';
  const S=v=>String(v??'').trim(), U=v=>S(v).toUpperCase();
  const norm=v=>{try{return typeof root.normalizePersonalUsername==='function'?root.normalizePersonalUsername(v):U(v).replace(/\s+/g,'').replace(/[^A-Z0-9._-]/g,'_').slice(0,40)}catch(_){return U(v)}};
  const safe=v=>S(v).replace(/[.#$\[\]\/]/g,'_');
  const esc=v=>S(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  ...
  async function decorateList(d=opDate()){
    ...
    const u=me();for(const card of host.querySelectorAll('.fwcFlight')){
      ...
      else if(cs.state==='CLAIMED'){old.textContent='NHẬN '+formLabel(primary.formGroup);old.onclick=()=>root.v324ReceiveOrOpen(fid,S(primary.assignmentId),d);}
      ...
      else if(cs.state==='DONE'){old.textContent='NHẬN '+formLabel(primary.formGroup);old.disabled=false;old.classList.add('done');old.onclick=()=>root.v324ReceiveOrOpen(fid,S(primary.assignmentId),d)}
      ...
      else {old.textContent='NHẬN '+formLabel(primary.formGroup);old.onclick=()=>root.v324ReceiveOrOpen(fid,S(primary.assignmentId),d);}
...
Phần của ${who} được ghi “BỎ QUA · KHÔNG E-FORM”, không ghi HOÀN THÀNH giả.`)}catch(e){alert('Không nhận thay được: '+S(e?.message||e))}};
  ...
  // V5.0.2B: HOÀN TẤT only this roster assignment. There is no named handover.
...
function install(){ensureUi();ensureButton();watchToolbar();const u=me();if(u!==state.user){state.user=u;state.sources={approval:[],cross:[],rs:[],manual:[]};state.loadedAt={};state.pendingSources.clear();state.pendingAll=false;clearLegacyCache();loadCache();startRealtime();/* V4.2.55 DATA SAVER: panel CẦN XỬ LÝ đang ẩn, không tự tải approval/cross/R&S/roster_mail ở startup. Chỉ refresh khi panel được mở hoặc người dùng yêu cầu đồng bộ. */if(document.getElementById('v342Modal')?.classList.contains('show'))scheduleRefresh(450,WORK_SOURCES)}else{updateBadge();renderSync()}}
