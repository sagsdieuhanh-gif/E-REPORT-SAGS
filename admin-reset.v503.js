...
async function requireAdmin(){
  if(!['AD','ADMIN'].includes(String(root.currentRole||'').toUpperCase()))throw Error('Đăng nhập AD trước khi thao tác.');
  const user=uid();if(!user?.uid)throw Error('Chưa có phiên Firebase Authentication để xác minh AD.');
