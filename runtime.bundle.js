...
window.__SAGS_RUNTIME_BUILD__="V4.8.3A-TURBO-CACHE-FLEX-WORKFLOW";
...
(function(root){
'use strict';
const BUILD='V4.8.3A-TURBO-CACHE-FLEX-WORKFLOW';if(root.__SAGS_V450_FAST_PDF===BUILD)return;root.__SAGS_V450_FAST_PDF=BUILD;
...
function pageFingerprint(pageNo,svg,w,h,mode){const chunks=[BUILD,mode,pageNo,w,h];for(const el of svg.querySelectorAll('text,foreignObject,line'))chunks.push(el.tagName,S(el.getAttribute('x')),S(el.getAttribute('y')),S(el.getAttribute('x1')),S(el.getAttribute('y1')),S(el.getAttribute('x2')),S(el.getAttribute('y2')),S(el.getAttribute('font-size')),S(el.getAttribute('text-anchor')),S(el.style?.display),S(el.textContent));const st=stateObj();for(const f of globalFields())if(Number(f.page)===Number(pageNo)&&String(f.type)==='signature')chunks.push(f.key,sigSummary(st[f.key]));return fnv(chunks.join('|'))}
...
const jpegCache=(root.__SAGS_PDF_JPEG_CACHE ||= new Map());
function turboCfg(landscape=false){const mobile=isMobile();if(landscape){const w=mobile?1260:1491;return {mobile,w,h:Math.round(w*1055/1491),quality:mobile?.80:.86}}const w=mobile?1000:baseW();return {mobile,w,h:Math.round(w*baseH()/baseW()),quality:mobile?.80:.84}}
function cacheKey(base,quality){return `${base}|q=${Number(quality).toFixed(2)}`}
function pruneCache(){while(jpegCache.size>18){const k=jpegCache.keys().next().value;jpegCache.delete(k)}}
async function fastPage(pageNo,landscape=false){const t0=perf(),page=document.getElementById('page'+pageNo),bg=page?.querySelector('img'),svg=document.getElementById('svg'+pageNo);if(!page||!bg||!svg)throw new Error('Không tìm thấy Trang '+pageNo);const cfg=turboCfg(landscape),keyBase=`T:${pageFingerprint(pageNo,svg,cfg.w,cfg.h,'turbo')}`,key=cacheKey(keyBase,cfg.quality);if(Number(pageNo)!==4&&jpegCache.has(key)){const hit=jpegCache.get(key),c=document.createElement('canvas');c.width=1;c.height=1;c.__sagsCachedPdfPage=hit;c.__sagsPdfCacheKey=key;c.__sagsOriginalWidth=hit.width;c.__sagsOriginalHeight=hit.height;const row={page:Number(pageNo),cacheHit:true,totalMs:+(perf()-t0).toFixed(1)};const P=root.__SAGS_V450_PDF_RUN;if(P){P.pages.push(row);P.cacheHits=(P.cacheHits||0)+1}return c}await new Promise((res,rej)=>{if(bg.complete&&bg.naturalWidth)return res();const a=bg.onload,e=bg.onerror;bg.onload=x=>{try{a?.call(bg,x)}catch(_){}res()};bg.onerror=x=>{try{e?.call(bg,x)}catch(_){}rej(new Error('Không tải được nền Trang '+pageNo))}});const c=document.createElement('canvas');c.width=cfg.w;c.height=cfg.h;c.__sagsOriginalWidth=cfg.w;c.__sagsOriginalHeight=cfg.h;c.__sagsPdfCacheKey=Number(pageNo)===4?'':key;const ctx=c.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);const t1=perf();ctx.drawImage(bg,0,0,c.width,c.height);const t2=perf(),sx=c.width/baseW(),sy=c.height/baseH();drawSvgDirect(ctx,svg,sx,sy);drawBbbtTicks(ctx,pageNo,sx,sy);const t3=perf();await drawSignatures(ctx,pageNo,sx,sy);if(Number(pageNo)===4&&!root.__SAGS_SUPPRESS_BBBT_CXR_ON_RENDER&&stateObj().bbbtCxrNo!==undefined&&stateObj().bbbtCxrNo!==null&&stateObj().bbbtCxrNo!==''){ctx.save();ctx.font=`900 ${28*sy}px Times New Roman`;ctx.fillStyle='#b54032';ctx.textAlign='right';ctx.textBaseline='top';let n=stateObj().bbbtCxrNo;try{if(typeof formatBBBTCxrNo==='function')n=formatBBBTCxrNo(n)}catch(_){}ctx.fillText('CXR No: '+n,(baseW()-42)*sx,30*sy);ctx.restore()}const t4=perf(),row={page:Number(pageNo),cacheHit:false,width:c.width,height:c.height,bgMs:+(t2-t1).toFixed(1),overlayMs:+(t3-t2).toFixed(1),signatureMs:+(t4-t3).toFixed(1),totalMs:+(t4-t0).toFixed(1)};const P=root.__SAGS_V450_PDF_RUN;if(P){P.pages.push(row);P.renderMs=(P.renderMs||0)+row.totalMs;P.cacheMisses=(P.cacheMisses||0)+1}return c}
function downscaleForEncode(src,landscape){if(!isMobile()||!src||src.__sagsPdfCacheKey||src.__sagsCachedPdfPage)return src;const cfg=turboCfg(landscape);if(src.width<=cfg.w)return src;const r=cfg.w/src.width,c=document.createElement('canvas');c.width=cfg.w;c.height=Math.max(1,Math.round(src.height*r));const x=c.getContext('2d',{alpha:false});x.fillStyle='#fff';x.fillRect(0,0,c.width,c.height);x.drawImage(src,0,0,c.width,c.height);return c}
async function encodeOne(src,landscape){if(src?.__sagsCachedPdfPage)return src.__sagsCachedPdfPage;const cfg=turboCfg(landscape),work=downscaleForEncode(src,landscape),key=src?.__sagsPdfCacheKey||'';if(key&&jpegCache.has(key)){const P=root.__SAGS_V450_PDF_RUN;if(P)P.cacheHits=(P.cacheHits||0)+1;return jpegCache.get(key)}const bytes=await canvasToJpegBytes(work,cfg.quality),out={bytes,width:work.width,height:work.height};if(key){jpegCache.set(key,out);pruneCache()}if(work!==src){try{work.width=1;work.height=1}catch(_){}}return out}
async function turboPdf(canvases,fileName,landscape=false){const t=perf(),out=new Array(canvases.length),step=isIOS()?1:2;for(let i=0;i<canvases.length;i+=step){const batch=canvases.slice(i,i+step),encs=await Promise.all(batch.map(c=>encodeOne(c,landscape)));encs.forEach((x,j)=>out[i+j]=x);if(isIOS())await new Promise(r=>setTimeout(r,0))}const P=root.__SAGS_V450_PDF_RUN;if(P)P.encodeCoreMs=(P.encodeCoreMs||0)+(perf()-t);return pdfFromJpegs(out,fileName,landscape)}
...
root.canvasesToPdfFile=canvasesToPdfFile=async function(canvases,fileName){return turboPdf(canvases,fileName,false)};root.canvasesToLandscapePdfFile=canvasesToLandscapePdfFile=async function(canvases,fileName){return turboPdf(canvases,fileName,true)};
const baseSend=root.sendReport;if(typeof baseSend==='function'&&!baseSend.__sagsV482Turbo){const send=async function(kind='all'){const P={build:BUILD,mode:'TURBO_CACHE',kind:S(kind),startedAt:Date.now(),pages:[],renderMs:0,encodeCoreMs:0,cacheHits:0,cacheMisses:0,totalMs:0};root.__SAGS_V450_PDF_RUN=P;const t=perf();try{return await baseSend.apply(this,arguments)}finally{P.totalMs=perf()-t;P.finishedAt=Date.now();root.__SAGS_V450_PDF_PERF=P;try{localStorage.setItem('sags.v450.pdfPerf',JSON.stringify(P))}catch(_){}root.__SAGS_V450_PDF_RUN=null;try{const st=document.getElementById('exportStatus');if(st&&P.totalMs)st.textContent+=` · TURBO ${(P.totalMs/1000).toFixed(2)}s · cache ${P.cacheHits}/${P.cacheHits+P.cacheMisses}`}catch(_){}}};send.__sagsV482Turbo=true;send.__sagsOriginal=baseSend;root.sendReport=send;try{sendReport=send}catch(_){}}
...
console.info('E-REPORT/SAGS V4.8.3A Turbo Cache PDF active');
})(typeof window!=='undefined'?window:globalThis);
