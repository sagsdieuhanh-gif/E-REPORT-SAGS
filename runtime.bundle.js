...
window.__SAGS_RUNTIME_BUILD__="V4.8.7B-LAYERED-PDF-FLEX-WORKFLOW-FORM-MANAGER-PRO";
...
const BUILD='V4.8.7B-LAYERED-PDF-FLEX-WORKFLOW-FORM-MANAGER-PRO',REG_URL='./forms.registry.json',LOCAL_KEY='sagsFormRegistryDraftV450',OLD_LOCAL_KEY='sagsFormRegistryDraftV440',DB_NAME='sags-form-assets-v440',STORE='assets';
...
function mergeRegistry(){const local=isAD()?loadLocal():null;draft=local?.forms?.length?local:clone(published);draft.schema=2;draft.version='V4.8.7';draft.build=BUILD;draft.forms=Array.isArray(draft.forms)?draft.forms:[];hydrateLegacyForms();fmNormalizeRegistryV487(draft);syncLegacyToLive(false);return draft}
...
function fmWrapCanvasV487(ctx,text,maxWidth){const out=[];for(const para of String(text??'').replace(/\r\n/g,'\n').split('\n')){if(!para){out.push('');continue}const words=para.split(/\s+/).filter(Boolean);let line='';for(const w of words){const test=line?line+' '+w:w;if(!line||ctx.measureText(test).width<=maxWidth)line=test;else{out.push(line);line=w}}out.push(line)}return out}
async function previewCustomPageV45(f,p){status('Đang dựng PDF preview…');try{const src=await pageSrc(p);if(!src)throw new Error('Trang chưa có ảnh nền.');const im=await new Promise((res,rej)=>{const q=new Image();q.onload=()=>res(q);q.onerror=rej;q.src=src}),c=document.createElement('canvas');c.width=Number(p.width||im.naturalWidth||1241);c.height=Number(p.height||im.naturalHeight||1755);const x=c.getContext('2d');x.drawImage(im,0,0,c.width,c.height);for(const raw of (f.fields||[]).filter(z=>z.pageId===p.id)){const fld=fmNormalizeFieldV487(raw),v=testValueV45(fld),px=Number(fld.x||0)*c.width,py=Number(fld.y||0)*c.height,pw=Number(fld.w||.1)*c.width,ph=Number(fld.h||.03)*c.height,st=fmStyleV487(fld);if(fld.type==='checkbox'){if(v){x.save();x.font=`900 ${Math.max(10,st.fontSize)}px Arial`;x.textAlign='center';x.textBaseline='middle';x.fillStyle='#111';x.fillText('✓',px+pw/2,py+ph/2);x.restore()}continue}if(fld.type==='signature')continue;x.save();x.beginPath();x.rect(px,py,pw,ph);x.clip();x.font=`${st.fontStyle} ${st.fontWeight} ${st.fontSize}px '${st.fontFamily}'`;x.fillStyle=st.textColor;x.textAlign=st.align==='right'?'right':st.align==='center'?'center':'left';x.textBaseline='alphabetic';const tx=st.align==='right'?px+pw-2:st.align==='center'?px+pw/2:px+2,lh=st.fontSize*st.lineHeight,lines=(fld.type==='textarea'||String(v??'').includes('\n'))?fmWrapCanvasV487(x,String(v??''),Math.max(8,pw-4)):[String(v??'')],max=Math.max(1,Math.floor(ph/Math.max(1,lh)));let first;if(st.valign==='top')first=py+st.fontSize;else if(st.valign==='bottom')first=py+ph-(Math.min(lines.length,max)-1)*lh-2;else first=py+ph/2-((Math.min(lines.length,max)-1)*lh)/2+st.fontSize*.34;lines.slice(0,max).forEach((line,i)=>{x.fillText(line,tx,first+i*lh);if(st.underline&&line){const w=x.measureText(line).width,ux=st.align==='right'?tx-w:st.align==='center'?tx-w/2:tx;x.fillRect(ux,first+i*lh+2,w,Math.max(1,st.fontSize/16))}});x.restore()}showCanvasPreviewV45(c,`${f.name||f.id} · Trang ${pageIndex+1}`);status('✓ PDF preview dùng cùng tọa độ + typography của Form Manager.')}catch(e){status('Không preview được: '+(e?.message||e),true)}}
...
function runtimeControl(fld,val){fmNormalizeFieldV487(fld);const k=esc(fld.bind||fld.key),st=fmStyleV487(fld),va=st.valign,style=fmCssV487(fld,false),flexStyle=`display:flex;align-items:${va==='top'?'flex-start':va==='bottom'?'flex-end':'center'};justify-content:${st.align==='center'?'center':st.align==='right'?'flex-end':'flex-start'};`;if(fld.type==='checkbox')return `<input data-v440-bind="${k}" type="checkbox" ${val?'checked':''}>`;if(fld.type==='textarea')return `<textarea data-v440-bind="${k}" style="${style}">${esc(val||'')}</textarea>`;if(fld.type==='select')return `<select data-v440-bind="${k}" style="${style}">${(fld.options||[]).map(x=>`<option ${String(x)===String(val)?'selected':''}>${esc(x)}</option>`).join('')}</select>`;if(fld.type==='signature')return `<button data-v440-sign="${k}" style="${style}">${val?'✓ ĐÃ KÝ':'KÝ'}</button>`;if(fld.type==='static-text')return `<div style="${style};${flexStyle};width:100%;height:100%;overflow:hidden;white-space:pre-wrap">${esc(fld.text||fld.label||'')}</div>`;return `<input data-v440-bind="${k}" type="${['number','time','date'].includes(fld.type)?fld.type:'text'}" value="${esc(val??'')}" style="${style}">`}
...
(function(root){
'use strict';
const BUILD='V4.8.7B-LAYERED-PDF-FLEX-WORKFLOW-FORM-MANAGER-PRO';if(root.__SAGS_V450_FAST_PDF===BUILD)return;root.__SAGS_V450_FAST_PDF=BUILD;
...
console.info('E-REPORT/SAGS V4.8.7B Layered PDF active');
})(typeof window!=='undefined'?window:globalThis);
