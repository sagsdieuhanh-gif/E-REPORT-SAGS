/* E-REPORT/SAGS V4.4.0 · CONSOLIDATED RUNTIME + FORM MANAGER */
...
const BUILD='V4.4.4-FORM-MANAGER-VISIBLE',REG_URL='./forms.registry.json',LOCAL_KEY='sagsFormRegistryDraftV440',DB_NAME='sags-form-assets-v440',STORE='assets';
const $=id=>document.getElementById(id),esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const clone=v=>{try{return JSON.parse(JSON.stringify(v))}catch(_){return v}};
let published={schema:1,forms:[]},draft=null,currentId='',pageIndex=0,selectedKey='',selectedMany=new Set(),history=[],future=[],objectUrls=[];
...
function saveLocal(){try{localStorage.setItem(LOCAL_KEY,JSON.stringify(draft||{schema:1,forms:[]}))}catch(e){console.warn('V4.4 form local save',e)}}
function loadLocal(){try{return JSON.parse(localStorage.getItem(LOCAL_KEY)||'null')}catch(_){return null}}
async function loadPublished(force=false){if(published.forms?.length&&!force)return published;try{const r=await fetch(REG_URL+(force?'?t='+Date.now():''),{cache:force?'no-store':'default'});if(r.ok)published=await r.json()}catch(e){console.info('V4.4 form registry offline',e?.message||e)}return published}
function mergeRegistry(){const local=loadLocal();if(local?.forms?.length)draft=local;else draft=clone(published);draft.schema=1;draft.version='V4.4.4';draft.build=BUILD;draft.forms=Array.isArray(draft.forms)?draft.forms:[];return draft}
...
function renderAll(){renderList();renderPages();renderEditor();renderInspector();$('v440Undo').disabled=!history.length;$('v440Redo').disabled=!future.length}
function renderList(){const h=$('v440FormList');if(!h)return;h.innerHTML=(draft?.forms||[]).map(f=>`<button class="v440FormItem ${f.id===currentId?'active':''}" data-id="${esc(f.id)}"><div>${esc(f.name||f.code||f.id)} ${f.legacy?'<span class="v440Tag">LEGACY</span>':'<span class="v440Tag">CUSTOM</span>'}</div><small>${esc(f.code||f.id)} · ${(f.pages||[]).length} trang · ${(f.fields||[]).length} field</small></button>`).join('')||'<div class="v440Empty">Chưa có form.</div>';h.querySelectorAll('[data-id]').forEach(b=>b.onclick=()=>{currentId=b.dataset.id;pageIndex=0;selectedKey='';selectedMany.clear();history=[];future=[];renderAll()})}
...
function drawBoxes(){const ov=$('v440Overlay');if(!ov)return;ov.innerHTML='';for(const fld of pageFields()){const b=document.createElement('div');b.className='v440Box'+(fld.key===selectedKey?' sel':'')+(selectedMany.has(fld.key)?' multi':'');b.dataset.key=fld.key;b.style.left=(Number(fld.x||0)*100)+'%';b.style.top=(Number(fld.y||0)*100)+'%';b.style.width=(Number(fld.w||.12)*100)+'%';b.style.height=(Number(fld.h||.035)*100)+'%';b.innerHTML=`<span>${esc(fld.label||fld.key)}</span><i class="v440Resize" data-resize="1"></i>`;ov.appendChild(b);bindBox(b,fld)}}
...
function bindBox(el,fld){el.onpointerdown=e=>{e.preventDefault();e.stopPropagation();if(e.shiftKey){if(selectedMany.has(fld.key))selectedMany.delete(fld.key);else selectedMany.add(fld.key);selectedKey=fld.key;drawBoxes();renderInspector();return}selectedKey=fld.key;if(!selectedMany.has(fld.key)){selectedMany.clear();selectedMany.add(fld.key)};pushHistory();const r=$('v440Overlay').getBoundingClientRect(),sx=e.clientX,sy=e.clientY,ox=Number(fld.x||0),oy=Number(fld.y||0),ow=Number(fld.w||.12),oh=Number(fld.h||.035),resize=!!e.target.dataset.resize;el.setPointerCapture?.(e.pointerId);const move=ev=>{const dx=(ev.clientX-sx)/r.width,dy=(ev.clientY-sy)/r.height;if(resize){fld.w=clamp(snap(ow+dx,'x'),.008,1-ox);fld.h=clamp(snap(oh+dy,'y'),.008,1-oy)}else{fld.x=clamp(snap(ox+dx,'x'),0,1-ow);fld.y=clamp(snap(oy+dy,'y'),0,1-oh)}drawBoxes();renderInspector()};const up=()=>{document.removeEventListener('pointermove',move);document.removeEventListener('pointerup',up);saveLocal();renderList()};document.addEventListener('pointermove',move);document.addEventListener('pointerup',up,{once:true});renderInspector()}}
...
function alignMany(kind){const a=selectedFields();if(a.length<2)return alert('Shift+click để chọn ít nhất 2 field.');pushHistory();if(kind==='left'){const v=Math.min(...a.map(x=>Number(x.x||0)));a.forEach(x=>x.x=v)}if(kind==='top'){const v=Math.min(...a.map(x=>Number(x.y||0)));a.forEach(x=>x.y=v)}if(kind==='midX'){const v=a.reduce((s,x)=>s+Number(x.x||0)+Number(x.w||0)/2,0)/a.length;a.forEach(x=>x.x=clamp(v-Number(x.w||0)/2,0,1-Number(x.w||0)))}if(kind==='midY'){const v=a.reduce((s,x)=>s+Number(x.y||0)+Number(x.h||0)/2,0)/a.length;a.forEach(x=>x.y=clamp(v-Number(x.h||0)/2,0,1-Number(x.h||0)))}saveLocal();drawBoxes();renderInspector()}
...
async function exportCurrent(){const f=form();if(!f)return;status('Đang đóng gói form…');try{const out=clone(f),files=[];for(let i=0;i<(f.pages||[]).length;i++){const p=f.pages[i],x=await pageBlob(p);if(!x)throw new Error('Trang '+(i+1)+' chưa có ảnh.');const name=`page-${String(i+1).padStart(2,'0')}.${x.ext}`;files.push({name,data:x.blob});out.pages[i].image=name;delete out.pages[i].blobKey;out.pages[i].sha256=await sha256(x.blob);out.pages[i].bytes=x.blob.size}out.schema=1;out.engine='generic-v440';out.exportedAt=new Date().toISOString();files.unshift({name:'form.json',data:JSON.stringify(out,null,2)+'\n'});files.push({name:'README.txt',data:'E-REPORT FORM PACKAGE V4.4.4\nUpload page images to forms/'+f.id+'/ and add the form.json metadata to forms.registry.json. Runtime JS does not need a new per-form file.\n'});download(await makeZip(files),`${f.id}-ereport-form.zip`);status('✓ Đã xuất gói ZIP: form.json + ảnh riêng, không nhúng base64.')}catch(e){status('Không xuất được: '+(e?.message||e),true)}}
async function exportRegistry(){const out=clone(draft),custom=(out.forms||[]).filter(f=>!f.legacy);for(const f of custom){for(let i=0;i<(f.pages||[]).length;i++){const p=f.pages[i];if(p.blobKey){p.image=`./forms/${f.id}/page-${String(i+1).padStart(2,'0')}.${p.ext||'png'}`;delete p.blobKey}}}out.version='V4.4.4';out.build=BUILD;out.updatedAt=new Date().toISOString();download(new Blob([JSON.stringify(out,null,2)+'\n'],{type:'application/json'}),'forms.registry.json');status('✓ Đã xuất forms.registry.json.')} 
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{loadPublished().then(()=>injectRuntimeButton());},900),{once:true});else setTimeout(()=>loadPublished().then(()=>injectRuntimeButton()),900);root.addEventListener('pageshow',()=>setTimeout(injectRuntimeButton,700),{passive:true});
})(typeof window!=='undefined'?window:globalThis);
