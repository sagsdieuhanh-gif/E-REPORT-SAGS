/* E-REPORT/SAGS V4.5.0 TEST · FORM MANAGER SINGLE SOURCE + FAST PDF */
...
const BUILD='V4.5.0-FORM-MANAGER-FAST-PDF-TEST',REG_URL='./forms.registry.json',LOCAL_KEY='sagsFormRegistryDraftV450',OLD_LOCAL_KEY='sagsFormRegistryDraftV440',DB_NAME='sags-form-assets-v440',STORE='assets';
const $=id=>document.getElementById(id),esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const clone=v=>{try{return JSON.parse(JSON.stringify(v))}catch(_){return v}};
let published={schema:2,forms:[]},draft=null,currentId='',pageIndex=0,selectedKey='',selectedMany=new Set(),history=[],future=[],objectUrls=[],testMode=false;
...
function hydrateLegacyForms(){const gs=globalFieldsV45();if(!draft?.forms||!gs.length)return draft;for(const fm of draft.forms){if(!fm?.legacy)continue;fm.engine='form-manager-v450';fm.fields=Array.isArray(fm.fields)?fm.fields:[];const pageMap=new Map((fm.pages||[]).map(p=>[pageNoV45(p),p.id]));const existing=new Map(fm.fields.map(x=>[String(x.key),x]));for(const g of gs){const pid=pageMap.get(Number(g.page));if(!pid||!g?.key)continue;let x=existing.get(String(g.key));if(!x){x={key:String(g.key),label:String(g.label||g.key),type:mapFieldTypeV45(g),bind:String(g.key),pageId:pid,x:Number(g.vx??g.x??0),y:Number(g.vy??g.y??0),w:Number(g.vw??g.w??.1),h:Number(g.vh??g.h??.03),fontSize:Number(g.font||14),fontWeight:700,align:String(g.align||((g.leftValue)?'left':'center')),sourcePage:Number(g.page),sourceType:String(g.type||'text'),generatedFromLegacy:true};fm.fields.push(x);existing.set(String(g.key),x)}else{x.pageId=x.pageId||pid;x.sourcePage=Number(g.page);x.sourceType=x.sourceType||String(g.type||'text');if(!Number.isFinite(Number(x.fontSize)))x.fontSize=Number(g.font||14);if(!x.align)x.align=String(g.align||'left')}}}return draft}
function syncLegacyToLive(redraw=false){const gs=globalFieldsV45();if(!gs.length)return false;const source=draft?.forms?.length?draft:published?.forms?.length?published:null;if(!source)return false;const byKey=new Map(gs.map(g=>[String(g.key),g]));for(const fm of source.forms||[]){if(!fm?.legacy)continue;for(const x of fm.fields||[]){const g=byKey.get(String(x.key));if(!g)continue;g.vx=Number(x.x);g.vy=Number(x.y);g.vw=Number(x.w);g.vh=Number(x.h);if(Number.isFinite(Number(x.fontSize)))g.font=Number(x.fontSize);g.align=String(x.align||'left');g.leftValue=g.align==='left';if(g.leftValue)g.manualInset=0}}if(redraw){try{if(typeof draw==='function')draw()}catch(_){}}return true}
...
function mergeRegistry(){const local=isAD()?loadLocal():null;draft=local?.forms?.length?local:clone(published);draft.schema=2;draft.version='V4.5.0';draft.build=BUILD;draft.forms=Array.isArray(draft.forms)?draft.forms:[];hydrateLegacyForms();syncLegacyToLive(false);return draft}
...
function renderAll(){renderList();renderPages();renderEditor();renderInspector();$('v440Undo').disabled=!history.length;$('v440Redo').disabled=!future.length;if($('v450Test'))$('v450Test').textContent=testMode?'✏ QUAY LẠI CHỈNH':'👁 TEST HIỂN THỊ';renderPerfV45()}
...
async function renderEditor(){clearUrls();const ed=$('v440Editor'),f=form(),p=page();if(!ed)return;if(!f){ed.className='v440Empty';ed.innerHTML='Chọn biểu mẫu.';return}if(!p){ed.className='v440Empty';ed.innerHTML='Biểu mẫu chưa có trang. Bấm + TRANG.';return}const src=await pageSrc(p);ed.className='';if(!src){ed.innerHTML='<div class="v440Empty">Chưa có ảnh nền. Chọn TẢI ẢNH ở bảng bên phải.</div>';return}ed.innerHTML=`<div class="v440EditorWrap" id="v440Canvas"><img id="v440Bg" src="${esc(src)}" alt="Form background"><div class="v440Overlay" id="v440Overlay"></div></div>`;const img=$('v440Bg');img.onload=()=>drawBoxes();if(img.complete)drawBoxes()}
function testValueV45(fld){const st=liveStateV45(),key=fld.bind||fld.key,v=st?.[key];if(v!==undefined&&v!==null&&String(v)!=='')return v;if(fld.type==='checkbox')return true;if(fld.type==='date')return '16/09/2026';if(fld.type==='time')return '08:25';if(fld.type==='number')return '123';if(fld.type==='signature')return '';return fld.type==='textarea'?'NỘI DUNG TEST\nDÒNG 2':'TEST'}
function testHtmlV45(fld){const v=testValueV45(fld),fs=Math.max(7,Number(fld.fontSize||14)),al=String(fld.align||'left');if(fld.type==='signature'){if(typeof v==='string'&&v.startsWith('data:image/'))return `<div class="v450TestValue" style="justify-content:center"><img src="${esc(v)}" style="max-width:100%;max-height:100%;object-fit:contain"></div>`;return '<div class="v450TestValue" style="justify-content:center;color:#64748b">CHỮ KÝ</div>'}if(fld.type==='checkbox')return `<div class="v450TestValue" style="justify-content:center;font-size:${fs}px">${v?'✓':''}</div>`;const just=al==='center'?'center':al==='right'?'flex-end':'flex-start';return `<div class="v450TestValue" style="justify-content:${just};text-align:${esc(al)};font:700 ${fs}px/1.08 'Times New Roman';">${esc(v)}</div>`}
function toggleTestV45(){testMode=!testMode;drawBoxes();renderAll()}
...
async function previewCustomPageV45(f,p){status('Đang dựng PDF preview…');try{const src=await pageSrc(p);if(!src)throw new Error('Trang chưa có ảnh nền.');const im=await new Promise((res,rej)=>{const x=new Image();x.onload=()=>res(x);x.onerror=rej;x.src=src}),c=document.createElement('canvas');c.width=Number(p.width||im.naturalWidth||1241);c.height=Number(p.height||im.naturalHeight||1755);const x=c.getContext('2d');x.drawImage(im,0,0,c.width,c.height);for(const fld of (f.fields||[]).filter(z=>z.pageId===p.id)){const v=testValueV45(fld),px=Number(fld.x||0)*c.width,py=Number(fld.y||0)*c.height,pw=Number(fld.w||.1)*c.width,ph=Number(fld.h||.03)*c.height;if(fld.type==='checkbox'){if(v){x.font=`900 ${Math.max(10,Number(fld.fontSize||14))}px Arial`;x.textAlign='center';x.textBaseline='middle';x.fillStyle='#111';x.fillText('✓',px+pw/2,py+ph/2)}continue}if(fld.type==='signature')continue;x.save();x.beginPath();x.rect(px,py,pw,ph);x.clip();x.font=`700 ${Math.max(7,Number(fld.fontSize||14))}px 'Times New Roman'`;x.fillStyle='#003b8e';x.textAlign=fld.align==='right'?'right':fld.align==='center'?'center':'left';x.textBaseline='middle';const tx=fld.align==='right'?px+pw:fld.align==='center'?px+pw/2:px;x.fillText(String(v??''),tx,py+ph/2);x.restore()}showCanvasPreviewV45(c,`${f.name||f.id} · Trang ${pageIndex+1}`);status('✓ PDF preview dùng cùng tọa độ Form Manager.')}catch(e){status('Không preview được: '+(e?.message||e),true)}}
...
function drawBoxes(){const ov=$('v440Overlay');if(!ov)return;ov.classList.toggle('v450TestMode',testMode);ov.innerHTML='';for(const fld of pageFields()){const b=document.createElement('div');b.className='v440Box'+(fld.key===selectedKey?' sel':'')+(selectedMany.has(fld.key)?' multi':'');b.dataset.key=fld.key;b.style.left=(Number(fld.x||0)*100)+'%';b.style.top=(Number(fld.y||0)*100)+'%';b.style.width=(Number(fld.w||.12)*100)+'%';b.style.height=(Number(fld.h||.035)*100)+'%';b.innerHTML=`<span>${esc(fld.label||fld.key)}</span>${testMode?testHtmlV45(fld):''}<i class="v440Resize" data-resize="1"></i>`;ov.appendChild(b);bindBox(b,fld)}}
...
function bindBox(el,fld){el.onpointerdown=e=>{if(testMode)return;e.preventDefault();e.stopPropagation();if(e.shiftKey){if(selectedMany.has(fld.key))selectedMany.delete(fld.key);else selectedMany.add(fld.key);selectedKey=fld.key;drawBoxes();renderInspector();return}selectedKey=fld.key;if(!selectedMany.has(fld.key)){selectedMany.clear();selectedMany.add(fld.key)};pushHistory();const r=$('v440Overlay').getBoundingClientRect(),sx=e.clientX,sy=e.clientY,ox=Number(fld.x||0),oy=Number(fld.y||0),ow=Number(fld.w||.12),oh=Number(fld.h||.035),resize=!!e.target.dataset.resize;el.setPointerCapture?.(e.pointerId);const move=ev=>{const dx=(ev.clientX-sx)/r.width,dy=(ev.clientY-sy)/r.height;if(resize){fld.w=clamp(snap(ow+dx,'x'),.008,1-ox);fld.h=clamp(snap(oh+dy,'y'),.008,1-oy)}else{fld.x=clamp(snap(ox+dx,'x'),0,1-ow);fld.y=clamp(snap(oy+dy,'y'),0,1-oh)}drawBoxes();renderInspector()};const up=()=>{document.removeEventListener('pointermove',move);document.removeEventListener('pointerup',up);saveLocal();renderList()};document.addEventListener('pointermove',move);document.addEventListener('pointerup',up,{once:true});renderInspector()}}
...
const up=$('v440Upload');$('v440UploadBtn').onclick=()=>up.click();up.onchange=async()=>{const file=up.files?.[0];if(!file||!p)return;status('Đang đọc/tối ưu ảnh…');try{pushHistory();const x=await optimizeImage(file),key=`${f.id}/${p.id}/${Date.now()}`;if(p.blobKey)await assetDel(p.blobKey);await assetPut({key,blob:x.blob,name:file.name,type:x.blob.type,size:x.blob.size,sha256:x.hash,width:x.meta.width,height:x.meta.height,updatedAtMs:Date.now()});p.blobKey=key;p.image='';p.width=x.meta.width;p.height=x.meta.height;p.sha256=x.hash;p.ext=x.ext;saveLocal();status(`✓ Ảnh ${Math.round(x.blob.size/1024)} KB · ${x.meta.width}×${x.meta.height}`);renderAll()}catch(e){status('Không đọc được ảnh: '+(e?.message||e),true)}};
...
function addField(){const f=form(),p=page();if(!f||!p)return alert('Cần có trang trước.');if(f.legacy)return alert('Form hệ thống: chỉnh đầy đủ field hiện có tại đây. Muốn thêm KEY nghiệp vụ mới, hãy tạo FORM MỚI để tránh phá luồng nhập liệu hiện hữu.');const key=fieldKey(prompt('KEY field, ví dụ std hoặc cargoStart:','field'+((f.fields||[]).length+1)));if(!key)return;if(f.fields.some(x=>x.key===key))return alert('KEY đã tồn tại.');pushHistory();const fld={key,label:key,type:'text',bind:key,pageId:p.id,x:.35,y:.35,w:.20,h:.035,fontSize:14,align:'left'};f.fields=f.fields||[];f.fields.push(fld);selectedKey=key;selectedMany=new Set([key]);saveLocal();renderAll()}
function duplicateField(fld){const f=form();if(!f||!fld)return;pushHistory();let base=fld.key+'_copy',k=base,n=2;while(f.fields.some(x=>x.key===k))k=base+n++;const x={...clone(fld),key:k,bind:k,label:(fld.label||fld.key)+' copy',x:clamp(Number(fld.x||0)+.012,0,.95),y:clamp(Number(fld.y||0)+.012,0,.95)};f.fields.push(x);selectedKey=k;selectedMany=new Set([k]);saveLocal();renderAll()}
...
function runtimeControl(fld,val){const k=esc(fld.bind||fld.key),style=`font-size:${Number(fld.fontSize||14)}px;text-align:${esc(fld.align||'left')}`;if(fld.type==='checkbox')return `<input data-v440-bind="${k}" type="checkbox" ${val?'checked':''}>`;if(fld.type==='textarea')return `<textarea data-v440-bind="${k}" style="${style}">${esc(val||'')}</textarea>`;if(fld.type==='select')return `<select data-v440-bind="${k}" style="${style}">${(fld.options||[]).map(x=>`<option ${String(x)===String(val)?'selected':''}>${esc(x)}</option>`).join('')}</select>`;if(fld.type==='signature')return `<button data-v440-sign="${k}" style="${style}">${val?'✓ ĐÃ KÝ':'KÝ'}</button>`;if(fld.type==='static-text')return `<div style="${style};width:100%;height:100%;overflow:hidden">${esc(fld.text||fld.label||'')}</div>`;return `<input data-v440-bind="${k}" type="${['number','time','date'].includes(fld.type)?fld.type:'text'}" value="${esc(val??'')}" style="${style}">`}
...
/* ===== V4.5.0 TEST · DIRECT CANVAS FAST PDF RENDERER =====
...
(function(root){
'use strict';
const BUILD='V4.5.0-FORM-MANAGER-FAST-PDF-TEST';if(root.__SAGS_V450_FAST_PDF===BUILD)return;root.__SAGS_V450_FAST_PDF=BUILD;
...
console.info('E-REPORT/SAGS V4.5.0 TEST Fast PDF active');
})(typeof window!=='undefined'?window:globalThis);
