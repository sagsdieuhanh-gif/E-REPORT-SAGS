window.__SAGS_RUNTIME_BUILD__="V4.6.0-AI-FORM-ASSIST";
/* E-REPORT/SAGS V4.5.3 · CUMULATIVE UI FIX · FORM MANAGER + FAST PDF */
...
const BUILD='V4.6.0-AI-FORM-ASSIST',REG_URL='./forms.registry.json',LOCAL_KEY='sagsFormRegistryDraftV450',OLD_LOCAL_KEY='sagsFormRegistryDraftV440',DB_NAME='sags-form-assets-v440',STORE='assets';
...
function mergeRegistry(){const local=isAD()?loadLocal():null;draft=local?.forms?.length?local:clone(published);draft.schema=2;draft.version='V4.6.0';draft.build=BUILD;draft.forms=Array.isArray(draft.forms)?draft.forms:[];hydrateLegacyForms();syncLegacyToLive(false);return draft}
...
function renderAll(){renderList();renderPages();renderEditor();renderInspector();$('v440Undo').disabled=!history.length;$('v440Redo').disabled=!future.length;if($('v450Test'))$('v450Test').textContent=testMode?'✏ QUAY LẠI CHỈNH':'👁 TEST HIỂN THỊ';renderPerfV45();renderZoomUi()}
...
(function(root){
'use strict';
const BUILD='V4.6.0-AI-FORM-ASSIST';if(root.__SAGS_V450_FAST_PDF===BUILD)return;root.__SAGS_V450_FAST_PDF=BUILD;
...
console.info('E-REPORT/SAGS V4.5.1 TEST Fast PDF active');
})(typeof window!=='undefined'?window:globalThis);
